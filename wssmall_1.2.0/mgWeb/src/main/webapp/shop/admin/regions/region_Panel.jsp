...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
		...
		<tr parentid="${region.p_region_id }" id="${region.region_id }">
			...
			<div style="padding-left: ${region.region_grade * 15 + 30 }px;">
			
			<img status="closed" src="images/<c:if test="${region.childnum>0 }">sitemapclosed.gif</c:if><c:if test="${region.childnum==0 }">sitemapopened.gif</c:if>" 
			   id="${region.region_id }"  name="imgTree"
			   <c:if test="${region.childnum>0 }">class="imgTree"</c:if> /> 
