...
<table class="form-table"  cellspacing="0" cellpadding="0" border="0" width="100%">
	...
	<tbody>
		<tr>
			<td>${coupons.cpns_name }</td>
			<td>${coupons.cpns_prefix }</td>
			<td><c:if test="${coupons.cpns_type == 0 }">一张可无限使用</c:if><c:if test="${coupons.cpns_type == 1 }">多张只适用一次</c:if></td>
			<td>${coupons.cpns_point}</td>
			<td><c:if test="${coupons.cpns_status == 1 }">启用</c:if><c:if test="${coupons.cpns_status == 0 }">禁用</c:if></td>
			<td>${fn:substring(promotion.pmt_time_begin  , 0 , 10)}</td>
			<td>${fn:substring(promotion.pmt_time_end  , 0 , 10)} </td>
		</tr>
	</tbody>
</table>
</div>
...
<table class="form-table"  cellspacing="0" cellpadding="0" border="0" width="100%">
	<tbody>
		...
		<tr>
			<th>订单优惠条件：</th>
			<td><span>${promotion.order_money_from }</span><b>≤</b>订单金额 ＜<span>9999999</span>
			</td>
		</tr>
		</c:if>
...
<table class="form-table"  cellspacing="0" cellpadding="0" border="0" width="100%">
	<tbody>
		<tr>
			...
			<td><c:if test="${promotion.pmt_ifcoupon==1 }">是</c:if>
			<c:if test="${promotion.pmt_ifcoupon==0 }">否</c:if>
		</tr>
		<tr>
			<th>活动规则开始时间:</th>
			<td>
			${fn:substring(promotion.pmt_time_begin , 0 , 10)}
			</td>
		</tr>
		<tr>
			<th>活动规则结束时间:</th>
			<td>
			${fn:substring(promotion.pmt_time_end , 0 , 10)}
			</td>
		</tr>
	</tbody>
</table>
