...
var addCommunity={
        ...
        addSelectCommunity : function(){
    		...
    		var html = $("##developbody #developNode").html();
			...
			//$("input[type='checkbox'][name='communityCodes']:checked").each(function() {
    			  var tr = "<tr>";
    			  var develop_id = $("#inputname").val();
    			  
    			  var select = $("##developbody #developNode input[id='"+develop_id+"']");
    			  
   			      if(select && select.length>0){
   				     alert("发展渠道       "+"["+develop_id+"]已经存在，不能重复添加!");
				     return ;
			      }
    			  
				  tr += "<td><input type='hidden' name='develop_id' id='"+develop_id+"' value='"+develop_id+"'/>"+develop_id+"</td>";
				  tr += "<td><a href='javascript:;'><img  class='delete' src='${ctx}/shop/admin/images/transparent.gif' > </a></td>";
				  tr += "</tr>";
				  html += tr;
				  
			//});
    		$("##developbody #developNode").html(html);
