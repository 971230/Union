...
var addCommunity={
        ...
        addSelectCommunity : function(){
    		...
    		var html = $("##staffidbody #staffidNode").html();
			...
			//$("input[type='checkbox'][name='communityCodes']:checked").each(function() {
    			  var tr = "<tr>";
    			  var staff_id = $("#inputname").val();
    			  
    			  var select = $("##staffidbody #staffidNode input[id='"+staff_id+"']");
   			      if(select && select.length>0){
   				     alert("工号       "+"["+staff_id+"]已经存在，不能重复添加!");
				     return ;
			      }
    			  
				  tr += "<td><input type='hidden' name='staff_id' id='"+staff_id+"' value='"+staff_id+"'/>"+staff_id+"</td>";
				  tr += "<td><a href='javascript:;'><img  class='delete' src='${ctx}/shop/admin/images/transparent.gif' > </a></td>";
				  tr += "</tr>";
				  html += tr;
				  
			//});
    		$("##staffidbody #staffidNode").html(html);
