...
var addCommunity={
        ...
        addSelectCommunity : function(){
    		...
    		var html = $("##officebody #officeNode").html();
			...
			//$("input[type='checkbox'][name='communityCodes']:checked").each(function() {
    			  var tr = "<tr>";
    			  var office_id = $("#inputname").val();
    			  
    			  var select = $("##officebody #officeNode input[id='"+office_id+"']");
   			      if(select && select.length>0){
   				     alert("受理渠道       "+"["+office_id+"]已经存在，不能重复添加!");
				     return ;
			      }
    			  
				  tr += "<td><input type='hidden' name='office_id' id='"+office_id+"' value='"+office_id+"'/>"+office_id+"</td>";
				  tr += "<td><a href='javascript:;'><img  class='delete' src='${ctx}/shop/admin/images/transparent.gif' > </a></td>";
				  tr += "</tr>";
				  html += tr;
				  
			//});
    		$("##officebody #officeNode").html(html);
