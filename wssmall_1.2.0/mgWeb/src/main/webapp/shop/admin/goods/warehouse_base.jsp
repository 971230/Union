			...
			<table cellspacing="0" cellpadding="0" border="0" width="100%">
				<tbody>
					...
					<tr>
						...
						<td>
							<input type="radio" name="warehouse.attr_inline_type" <c:if test="${warehouse.attr_inline_type eq 'T' }">checked</c:if> checked value="1" dataType="" />
							线上
							<input type="radio" name="warehouse.attr_inline_type" <c:if test="${warehouse.attr_inline_type eq 'F' }">checked</c:if>  value="2" dataType="" />
							线下
					...
					<tr>
						...
						<td>
							<input type="radio" name="warehouse.deliver_goods_attr" <c:if test="${warehouse.deliver_goods_attr eq 'T' }">checked</c:if> checked value="1" dataType="" />
							发货仓库
							<input type="radio" name="warehouse.deliver_goods_attr" <c:if test="${warehouse.deliver_goods_attr eq 'F' }">checked</c:if>  value="2" dataType="" />
							备货仓库
					...
					<tr>
						...
						<td>
							<input type="radio" name="warehouse.attribution" <c:if test="${warehouse.attribution eq 'T' }">checked</c:if> checked value="1" dataType="" />
							自建仓库
							<input type="radio" name="warehouse.attribution" <c:if test="${warehouse.attribution eq 'F' }">checked</c:if>  value="2" dataType="" />
							第三方仓库
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<textarea cols="80" rows="6" name="warehouse.remarks" value="${warehouse.remarks }" style="width: 437px; height: 92px;"></textarea>
						</td>
					</tr>
					<tr>
						<th>
...
$(function (){
    ...
    //修改数据保存
      ...
      $("#ediWareBaseBtn").click(function() {
            var  url = ctx+ "/shop/supplier/warehouse!edit.do?ajax=yes";
	...
	$("#house_name").blur(function() {
		...
		url = ctx
				+ "/shop/supplier/supplier!isExits.do?house_name="
				+ house_name + "&ajax=yes";
	...
	$("#house_code").blur(function() {
		...
		url = ctx
				+ "/shop/supplier/supplier!isExits.do?house_code="
				+ house_code + "&ajax=yes";
