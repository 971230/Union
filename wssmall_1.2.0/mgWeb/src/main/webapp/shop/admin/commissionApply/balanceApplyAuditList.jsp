...
<form method="POST"  id="userform" action="<%=request.getContextPath() %>/shop/admin/commissionApply!balanceApplyAuditList.do">
...
<grid:grid from="webpage"  formId="userform">
	...
	<grid:body item="payApply">
		...
		<c:if test="${payApply.status==1 }">
			<a title ="结算审核" id="forwardBalanceApplyAudit" list_id=${payApply.list_id} apply_id = ${payApply.apply_id } apply_price=${payApply.apply_price } href="javascript:;" class="p_prted">审核</a>
			</c:if>
		</grid:cell>
	</grid:body>
</grid:grid>
...
<div id="balanceApplyAuditDlg"></div>
<!-- userAdmin!delete.do?id=${userAdmin.userid }" onclick="javascript:return confirm('确认删除此管理员吗？')" -->
 <script type="text/javascript">
$(function(){
  Eop.Dialog.init({id:"balanceApplyAuditDlg", modal:false, title:"结算审核", width:"700px"});

  $("#forwardBalanceApplyAudit").click(function(){
     var apply_id = $(this).attr("apply_id");
     var list_id = $(this).attr("list_id");
     var apply_price = $(this).attr("apply_price");
     var url = ctx + "/shop/admin/commissionApply!forwardBalanceApplyAudit.do?ajax=yes";
     $("#balanceApplyAuditDlg").load(url,{apply_id:apply_id,list_id:list_id,apply_price:apply_price});
     Eop.Dialog.open("balanceApplyAuditDlg");
  });
});
</script>
