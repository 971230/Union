...
<div class="btnWarp">
	...
	<c:if test="${(orderInfo.status==0 || orderInfo.status==1 || orderInfo.status==2) && orderInfo.order_record_status!=99}">
	...
	<!-- 新的订单处理流程 -->
	<c:if test="${orderInfo.status!=-10 && toDo!=null && orderInfo.order_record_status!=99}">
		<a href="javascript:void(0);" id="order_return_visit_btn_b" order_id="${orderInfo.order_id }" todo_id="${toDo.list_id }" class="dobtn" style="margin-right:5px;">回访</a>
		<c:if test="${toDo.flow_type=='rob' }">
			<a href="javascript:void(0);" name="BUTTON_ORDER_ROB" order_id="${orderInfo.order_id }" class="dobtn" style="margin-right:5px;">抢单</a>
		</c:if>
		<c:if test="${toDo.flow_type=='dispatch' }">
			<a href="javascript:void(0);" name="BUTTON_ORDER_DISPATCH" class="dobtn" style="margin-right:5px;">分派</a>
		</c:if>
