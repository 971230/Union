...
//		
	...
	init:function(){
		...
		if (hidden_lan_id.value != '') {
			$("#cloud_query_form [name='lan_id']").val(hidden_lan_id.value); // 记录选中发布区域
		}
		if (hidden_acc_type.value != '') {
			$("#cloud_query_form [attr_code='DC_ACC_TYPE']").val(hidden_acc_type.value); // 记录选中号码段
		}
		
//		$("#cloud_query_form [attr='begin_nbr']").each(function(i){
			if (begin_nbr != "") 
				$("#cloud_query_form [attr='begin_nbr']").val(begin_nbr);
			else {
				if (hidden_b_acc_type.value != "") {
					$("#cloud_query_form [attr='begin_nbr']").val(hidden_b_acc_type.value); //记录选中号码段开始号码
				}
			}
//		})
//		
//		$("#cloud_query_form [attr='end_nbr']").each(function(i){
			if (end_nbr != "") 
				$("#cloud_query_form [attr='end_nbr']").val(end_nbr);
			else {
				if (hidden_e_acc_type.value != "") {
					$("#cloud_query_form [attr='end_nbr']").val(hidden_e_acc_type.value);  //记录选中号码段结束号码
				}
			}
//			
//		})
		
		$("#cloud_query_form [name='lan_id']").unbind("change").bind("change",function(){
			hidden_lan_id.value =  $(this).val();
		});
		
		$("#cloud_query_form [attr_code='DC_ACC_TYPE']").unbind("change").bind("change",function(){
