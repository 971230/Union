var FindCatListByParentId = {
	...
	initClk : function() {
		$("#confirmSelectBtn").bind("click",function(){
			var catid;
			var typeid = $("#box_0").find("li.selected").attr("typeid");
			...
			var div4 = $("#box_126").find("li.selected").attr("catid");
			if(div4==undefined||div4=="undefined"){
			...
			if(catid==undefined||catid=="undefined"){
				...
				$("#goodsCatInputDialog").attr("typeid",typeid);
				$("#searchGoodsCatidInput").val(catid);
				$("#up").show();
				
				Eop.Dialog.close("productTypeQuickBtn_dialog");			
	...
	loadFirstNode : function() {
		var me = this;
		$.ajax({
			url : "cat!findCatListByParentIdEcs.do?ajax=yes&parentid=0&type="+type,
			type : "post",
			dataType : "json",
	...
	dataDeal : function(datas,divId) {
		...
		for(var i=0;i<datas.length;i++){
			$obj.append('<li catid="'+datas[i].cat_id+'" typeid="'+datas[i].type_id+'" parentid="'+datas[i].parent_id+'" catname="'+datas[i].name+'" class="">'+datas[i].name+'</li>');
		}
		$obj.each(function(i,data){
			$(data).find("li").bind("click",function() {
				...
				var fordiv = $("#box_126").find("li.selected").html();
	...
	findNextNodes : function(catid,divid) {
		var me = this;
		if(divid!="box_none"){//点击第四级菜单时不再请求 
			$.ajax({
