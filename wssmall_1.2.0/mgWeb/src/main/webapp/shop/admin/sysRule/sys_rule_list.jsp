...
<form method="post" id='rule_list_form' action="sysRule!listRule.do">
	...
	<div class="comBtnDiv">
		<c:if test="${is_select != '0'}">
			<a style="margin-right:10px;" class="graybtn1"
				href="${ctx}/shop/admin/sysRule!addRule.do"><span>添加</span></a>
		</c:if>
		<c:if test="${is_select == '0'}">
			<a style="margin-right:10px;" class="graybtn1"
		...
		<form method="POST" id="rule_list_form">
			<grid:grid from="webpage" ajax="yes">
				...
				<grid:body item="rule">
					...
					<grid:cell>
						<c:if test="${is_select != '0'}">
							<a href="sysRule!editRule.do?rule_id=${rule.rule_id}">修改 </a>
							<span class='tdsper'></span>
							<a href="javascript:void(0);" class="delRule"
								attr="${rule.rule_id}">删除 </a>
						</c:if>
					</grid:cell>
				</grid:body>
			</grid:grid>
		...
		$(".delRule")
				.live(
						...
						function() {
							...
							var rule_id = $(this).attr("attr");
							var url = ctx
									+ "/shop/admin/sysRule!delRule.do?ajax=yes&rule.rule_id="
									+ rule_id;
							Cmp.ajaxSubmit('del_rule_div', '', url, {},
									RuleList.jsonBack, 'json');
						});
