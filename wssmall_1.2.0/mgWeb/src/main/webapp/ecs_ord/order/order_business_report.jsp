...
<div class="grid" id="goodslist">
     <form method="POST"  id="OrderBusinessReport"  >
         ...
         <grid:grid from="webpage" formId="serchform" >
			<grid:header>
<grid:cell width="50px">写卡完成时间</grid:cell> <%-- card_write_time--%>
<grid:cell width="50px">订单日期</grid:cell><%--  tid_time--%>
...
<%--<grid:cell width="50px">新/老用户</grid:cell><%-- is_old--%>
...
<grid:cell>${order.card_write_time}</grid:cell>  <%--开户时间--%>		    
<grid:cell>${order.tid_time}</grid:cell>  <%--订单日期--%>
<grid:cell>${order.order_operator}</grid:cell>  <%--订单处理人--%>
<%-- <grid:cell>${order.order_type}</grid:cell>--%><%--操作类型--%>
<%-- <grid:cell>${order.order_plat_type}</grid:cell>  数据来源--%>
