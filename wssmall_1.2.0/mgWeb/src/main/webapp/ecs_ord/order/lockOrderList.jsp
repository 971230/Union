...
<form method="post" id="serchform" action='${ctx}/shop/admin/ordAuto!LockOrderList.do'>
 <div class="searchformDiv">
	 ...
	 <tr>
	  ...
	  <th>外呼状态：</th>
                <td>
                	<select name="params.out_call"  class="ipt_new" style="width:150px;">
                    	<option value="-1" ${params.out_call=='-1'?'selected':'' }>--请选择--</option>
                    	<option value="1" ${params.out_call=='1'?'selected':'' } >是</option>
                    	<option value="0" ${params.out_call=='0'?'selected':'' }>否</option>
					</select>
					&nbsp;&nbsp;&nbsp;&nbsp;
                </td>
	  <%--  <th>创建时间：</th>
...
<grid:grid from="webpage"  formId="serchform">
	...
	<grid:body item="lockOrder">
		...
		<grid:cell>${lockOrder.lock_user_name}<span style="font-weight:bold">/</span>${lockOrder.lock_user_realname}</grid:cell>
	</grid:body>
</grid:grid>
</form>
  ...
  $("#delSel").click(function(){
	  ...
	  if(window.confirm("确认解除选中的锁定订单吗?")){
		 ...
		 // Cmp.excute('',url,{},jsonBack,'json');
		  ...
		  var urllink = ctx+"/shop/admin/ordAuto!unLockOrder.do?ajax=yes&lockDealType=sel&lockOrderIdStr="+lockOrderIdStr;
				$.ajax({
   					type : "post",
   					async : true,
   					url : urllink,
   					...
   					success : function(data) {
   						...
   						var page=obj.text();						
   						var link=ctx+"/shop/admin/ordAuto!LockOrderList.do?"+"page="+page;
   						$("#serchform").attr('action',link);
   						$("#serchform").submit();//用于修改后刷新显示锁定人
   						var linkdefault=ctx+"/shop/admin/ordAuto!LockOrderList.do";
