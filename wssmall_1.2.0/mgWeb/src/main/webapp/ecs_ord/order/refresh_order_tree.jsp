		...
		<table class="tab_form">
			<tr>
				...
				<td style="width:180px;">
					<input type="text" id="rule_id" name="rule_id" value="" style="width:333px;" class="ipt_new">
				</td>
			</tr>
			<tr>
				<th>请输入订单编号   （","分开）：</th>
				<td style="width:180px;">
					<textarea rows="5" cols="50" id="order_ids_rule" name="order_ids_rule"></textarea>
				</td>
				<td>
					<a href="javascript:void(0);" id="execRuleBtn" class="dobtn" style="margin-left:5px;">执行规则</a>
					<a href="javascript:void(0);" id="resetOrderId_execRule" class="dobtn" style="margin-left:5px;">重置订单ID</a>
				</td>
			</tr>
			<tr>
				<th>失败订单号：</th>
				<td style="width:180px;">
					<textarea rows="5" cols="50" id="fail_order_ids" name="fail_order_ids"></textarea>
				</td>
			</tr>
		</table>
	</form>
...
<script>
	$(function() {
		...
		//订单规则执行
		$("#execRuleBtn").bind("click", function() {
			
			flag = confirm("确认执行规则吗？");
			if (!flag)return;
			//清空以前错误的订单号
			$('#fail_order_ids').val("");
			//获取订单号
			var order_ids = $("#order_ids_rule").val();
			//获取规则编号或动作编号
			...
			if (order_ids == "") {
				...
				$.ajax({
					type : "post",
					async : false,
					url : "orderRefreshTreeAttrAction!orderRuleExec.do?ajax=yes&order_ids="+order_ids+"&rule_id="+rule_id,
					data : {},
					dataType : "json",
					success : function(data) {
						$('#fail_order_ids').val(data.failOrderIds);
						alert(data.msg);
					}
				});
			}
		...
		//订单规则执行重置订单id
		$("#resetOrderId_execRule").bind("click", function() {
			$("#order_ids_rule").val("");
		});
