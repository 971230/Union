...
<div class="gridWarp">
	<div class="new_right">
        ...
        <jsp:include page="params2.jsp?exception_type=all"/>
        </form>
                        	...
                        	<ul>
                            	<li>
                            		<span style="width:60px;text-align:center;">
                            			${order.city_name}
										<%--<html:orderattr disabled="disabled"  attr_code="DC_MODE_REGION" order_id="${order.order_id}" field_name="order_city_code"  field_desc ="归属地市" field_type="select"></html:orderattr>--%>                            		
									</span>
                            	</li>
                            </ul>
                        </div>
		       ...
		       <c:choose>
		        <c:when test="${order.bss_time_type == '1' }">
		       		<div class="order_pri" id="order_info">
                        	...
                        	<p class="tit f_bold red" order_id="${order.order_id }" detail_url="${order.action_url }" flow_trace_id="${order.orderTree.orderExtBusiRequest.flow_trace_id }" >${order.flow_trace }</p>
                 ...
                 <c:otherwise>
                 		<div class="order_pri" id="order_info">
                        	...
                        	<p class="tit f_bold" order_id="${order.order_id }" detail_url="${order.action_url }" flow_trace_id="${order.orderTree.orderExtBusiRequest.flow_trace_id }">${order.flow_trace }</p>
	         	...
	         	<c:if test="${order.orderTree.orderExtBusiRequest.flow_trace_id == 'C'}">
		         	<c:forEach items="${order.orderTree.orderItemBusiRequests }" var="oi" varStatus="orderItem">
		        		<c:if test="${orderItem.index==0&&(oi.orderItemExtBusiRequest.goods_type=='20003'||oi.orderItemExtBusiRequest.goods_type=='20002')}"><!-- 商品 类型是合约机或者裸机-->
			           		...
			           		<input  type="text" value="" class="ipt_new" id="terminal_num_${order.order_id }" name="terminal_num" order_id="${order.order_id }" placeholder="终端串号"/> 
			           		<%-- <input type="button"  name="terminal_num_bt" id="terminal_num_bt_${order.order_id }" disabled="disabled" value="确认" order_id="${order.order_id }"/> --%>
		        		...
		        		<c:if test="${order.orderTree.orderExtBusiRequest.flow_trace_id == 'C'}">
				         	<c:forEach items="${order.orderTree.orderItemBusiRequests }" var="oi" varStatus="orderItem">
				        		<c:if test="${orderItem.index==0&&(oi.orderItemExtBusiRequest.goods_type=='20003'||oi.orderItemExtBusiRequest.goods_type=='20002')}"><!-- 商品 类型是合约机或者裸机-->
					           		...
					           		<p><a href="javascript:void(0);" class="dobtn" style="margin-left:5px;" name="terminal_num_bt" id="terminal_num_bt_${order.order_id }" value="确认" order_id="${order.order_id }">确认</a></p>
							      
		                        </c:if>
		                     </c:forEach>
						</c:if>
                        <p>
                        	<a href="javascript:void(0);" name="add_comments" innerId=${order.order_id } class="dobtn" style="margin-left:5px;margin-top:5px;">添加备注</a>
                        </p>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("a[name=inner_order_id]").bind("click",function(){
			...
			if(o_url==''||o_url==null||o_url=='undefined'){
				var resultUrl =  AutoFlow.getNewActionUrl(order_id);
				var resultObj = resultUrl.split("|");
				if(resultObj[0]==0){
					o_url = resultObj[1];	
				}else{
