	...
	<div class="new_right">
        ...
        <form action="" id="order_list_fm">
			...
			<grid:header>
				...
				<grid:cell style="text-align:center;">归属地市</grid:cell>
		    ...
		    <grid:body item="order" >
		  	    ...
		  	    <grid:cell style="width:60px;text-align:center;">
		  	    	<div class="list_t">
                        	<ul>
                            	<li>
                            		<span style="width:60px;text-align:center;">
                            			${order.city_name}
										<%--<html:orderattr disabled="disabled"  attr_code="DC_MODE_REGION" order_id="${order.orderTree.order_id}" field_name="order_city_code"  field_desc ="归属地市" field_type="select"></html:orderattr>--%>                            		
									</span>
                            	</li>
                            </ul>
                        </div>
		        ...
		        <c:choose>
		        		...
		        		<div class="order_pri">
                            ...
                            <p class="ps">操作类型：${order.optType=='01'?'退订':order.optType=='02'?'变更':'订购' }</p>
                        </div>
                        </c:when>
                        <c:otherwise>
                           	<div class="order_pri">
		       ...
		       <c:choose>
	       			...
	       			<c:if test="${order.orderTree.orderExtBusiRequest.abnormal_type == '4' }">
	       			自动关闭
	       			</c:if>
                   ...
                   <!-- add by cqq 20181128 自定义流程标识 -->
                   <c:if test="${order.orderTree.orderExtBusiRequest.is_work_custom == '1' }">
                		<p class="tit f_bold " >自定义流程</p>
                	</c:if>
		       		</c:when>
		       		<c:otherwise>
		       		<p class="ps">
                   ...
                   <c:if test="${order.orderTree.orderExtBusiRequest.is_work_custom == '1' }">
		       		...
		       		<c:forEach items="${order.orderTree.orderItemBusiRequests }" var="oi" varStatus="orderItem">
		        		...
		        		<c:if test="${order.orderTree.orderExtBusiRequest.order_from == '10078' || order.orderTree.orderExtBusiRequest.plat_type == 'AO'}">
                        	<c:choose>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='0' }">
		      				处理信息:待处理
		      			</c:when>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='1' }">
		      				处理信息:中台已领取
		      			</c:when>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='2' }">
		      				处理信息:中台已审单
		      			</c:when>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='99' }">
		      				处理信息:挂起
		      			</c:when>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='3' }">
		      				处理信息:已派工单
		      			</c:when>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='4' }">
		      				处理信息:受理成功
		      			</c:when>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='5' }">
		      				处理信息:装机成功
		      			</c:when>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='6' }">
		      				处理信息:受理失败
		      			</c:when>
		      			<c:when test="${order.orderTree.orderBusiRequest.order_state=='-1' }">
		      				处理信息:退单
				  ...
				  <grid:cell style="width:100px;">	
					<c:if test="${query_type =='order_view' && order.lock_user_name !=null && order.lock_user_name !=null && order.lock_user_realname != '' && order.lock_user_id != ''}">
                            	<div>${order.lock_user_name }/${order.lock_user_realname }</div>
                    </c:if>
		          </grid:cell>
...
<%-- 		        	<c:if test="${query_type =='order_view'}">
 --%>		        		
		        		<c:if test="${order.orderTree.orderExtBusiRequest.order_deal_method == '2'}">
		        			<p class="ps">处理方式：线下</p>
		        		</c:if>
		        		
		        		<c:if test="${order.orderTree.orderExtBusiRequest.order_deal_method == '1'}">
		        			<p class="ps">处理方式：线上</p>
		        		</c:if>
	       				
	       				<p class="ps">
			       			<c:if test="${order.work_state == '0' }">工单状态：未处理<br/></c:if>
			       			<c:if test="${order.work_state == '1' }">工单状态：处理成功<br/></c:if>
			       			<c:if test="${order.work_state == '2' }">工单状态：处理失败<br/></c:if>
			       			<c:if test="${order.work_state == '3' }">工单状态：已撤单<br/></c:if>
			       			<c:if test="${order.work_operator != '' }">工单处理人：${order.work_operator }</c:if>
	       				</p>
