...
<input type="hidden" name="oldCertNum" value="${oldCertNum}%>" />
<div class="gridWarp">
	<div class="new_right">
        <div class="right_warp">
            <!-- 顶部公共 -->
        	<jsp:include page="auto_flows_audit.jsp?order_id=${order_id }&btn=${yclbtn }"/>
        	...
        	<div class="grid_n_div">
            	 <h2><a href="javascript:void(0);" class="closeArrow"></a>订单基本信息<!--<a href="#" class="editBtn">编辑</a>--></h2> 
              	<div class="grid_n_cont">
					...
					<div id="order_base">
						<jsp:include page="include/order_base_audit.jsp?order_id=${order_id }&order_amount=${order_amount }"/>
              ...
              <div class="grid_n_div">
            	<h2><a href="#" class="openArrow"></a>货品信息<!-- <a href="#" class="editBtn">编辑</a>--></h2> 
              	<div class="grid_n_cont">
					...
					<c:if test="${goods_type =='20021' || goods_type == '170502112412000711' || goods_type == '180441456282001431'}">
                                	...
                                	<td>
                                		...
                                		<select name="city"  disabled="disabled" style="width:200px;">
                                       		<option value="">--请选择--</option>
                                 				<c:forEach var="citys" items="${inputTypeMaps.get('citys')}">
                                 				   <option <c:if test="${citys.value=city}">selected="selected"</c:if> value="${citys.value}">${citys.value_desc}</option>
                                 				</c:forEach>
                               			</select>
...
<script type="text/javascript">
  ...
  $(function(){
	  ...
	  CommonLoad.loadJSP('order_ext','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"order_ext_audit"},false,function(){AutoFlow.checkMsg();},true);
	  CommonLoad.loadJSP('prod_info','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"prod_info_audit"},false,function(){AutoFlow.checkMsg();},true);
	  ...
	  if(${!(orderTree.orderBusiRequest.order_type=='09'&&orderTree.orderExtBusiRequest.order_from=='10053')}){//非多商品
		  CommonLoad.loadJSP('goods_info','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"goods_info_audit"},false,function(){AutoFlow.checkMsg();},true);
		  if(${orderTree.orderExtBusiRequest.order_model=='10'}||${orderTree.orderExtBusiRequest.order_from=='10012'}){
			  CommonLoad.loadJSP('goods_open','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"view_goods_open_audit"},false,function(){AutoFlow.checkMsg();},true);
		  }else{
			  CommonLoad.loadJSP('goods_open','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"goods_open_audit"},false,function(){AutoFlow.checkMsg();},true);
		  }
	  	  
		  CommonLoad.loadJSP('sp_product','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"sp_product"},false,function(){AutoFlow.checkMsg();},true);
		  CommonLoad.loadJSP('goods_gift','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"goods_gift_audit"},false,function(){AutoFlow.checkMsg();},true);
	  }
	  if(${orderTree.orderBusiRequest.order_type=='09'&&orderTree.orderExtBusiRequest.order_from=='10053'}){//多商品
		  CommonLoad.loadJSP('goods_info_more','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"view_goods_info_more_audit"},false,function(){AutoFlow.checkMsg();},true);
	  }
	  CommonLoad.loadJSP('fee_info','<%=request.getContextPath() %>/shop/admin/orderFlowAction!includeNewPage.do',{order_id:"${order_id }",ajax:"yes",includePage:"fee_info"},false,function(){AutoFlow.checkMsg();},true);
	   //校验信息
	  AutoFlow.checkMsg();
