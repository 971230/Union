...
<div class="input">
   <form  action="<%=request.getContextPath() %>/shop/admin/orderWarningAction!edit.do"  class="validate" method="post" name="warningEditForm" id="warningEditForm" >
      <input type="hidden" name="orderWarning.warning_id"   value="${orderWarning.warning_id }">
      <table  class="form-table">
         <tr>
            <th><label class="text"><span class='red'>*</span>规则名称：</label></th>
            <td><input name="orderWarning.group_name" class="ipttxt" required="true" value="${orderWarning.group_name }"></td>
             ...
             <td>
               	<select name="orderWarning.flow_id" class="ipt_new" style="width:138px;" required="true" >
               		<c:forEach items="${flowTraceList }" var="ds">
               			<option value="${ds.pkey }"  ${ds.pkey==orderWarning.flow_id?'selected':'' }>${ds.pname }</option>
               		</c:forEach>
				</select>
              </td>
              ...
              <td>
               	 <select name="orderWarning.product_type" class="ipt_new" style="width:138px;" required="true" >
               		<c:forEach items="${goods_type_list }" var="ds">
               			<option value="${ds.pkey }" ${ds.pkey==orderWarning.product_type?'selected':'' } >${ds.pname }</option>
               		</c:forEach>
				</select>
              </td>
         ...
         <tr>
	          ...
	          <td colspan="6">
	               <textarea rows="5" cols="100" name="orderWarning.memo" >${orderWarning.memo }</textarea>
	          </td>
         </tr>
           <tr>
	         <th><label class="text"><span class='red'>*</span>黄色预警开始时间（分钟）：</label></th>
	         <td><input name="orderWarning.warning_time_1" id="warning_time_1" value="${orderWarning.warning_time_1 }" class="ipttxt" required="true"></td>
	         
	         <th><label class="text"><span class='red'>*</span>橙色预警开始时间（分钟）：</label></th>
	         <td><input name="orderWarning.warning_time_2" id="warning_time_2" value="${orderWarning.warning_time_2 }"  class="ipttxt" required="true"></td>
	         </tr>
	       <tr>
	         <th><label class="text"><span class='red'>*</span>红色预警开始时间（分钟）：</label></th>
	         <td><input name="orderWarning.warning_time_3" id="warning_time_3" value="${orderWarning.warning_time_3 }"  class="ipttxt" required="true"></td>
	         
	       
	         <th><label class="text"><span class='red'>*</span>预警结束时间（分钟）：</label></th>
	         <td><input name="orderWarning.warning_time_4"  id="warning_time_4" value="${orderWarning.warning_time_4 }"  class="ipttxt" required="true"></td>
          </tr>
      </table>
...
<script type="text/javascript">
  $(function(){
	  ...
	  $("#warningEditForm").validate();
	  $("#doSave").bind("click",function(){
		  if(checkAddForm()){
			  $("#warningEditForm").submit();
		 }
		});
  });
