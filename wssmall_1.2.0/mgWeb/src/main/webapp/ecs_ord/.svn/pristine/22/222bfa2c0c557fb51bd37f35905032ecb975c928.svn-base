	...
	<div class="grid_n_cont_sub">
		...
		<div class="netWarp">
			...
			<c:forEach var="gift" items="${giftInfoList}" varStatus="i_gift">
				<div class="goodTit">
					礼品${i_gift.index+1}<a href="#" class="icon_del"></a>
				</div>
				<div class="goodCon">
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							<th>赠品类型：</th>
							<td><html:selectdict name="gift.goods_type" curr_val="${gift.goods_type}"
						...
						<tr>
							...
							<th>赠品面值单位：</th>
							<td><html:selectdict name="gift.gift_unit" curr_val="${gift.gift_unit}"
									appen_options="<option value=''>---请选择---</option>"
									disabled="disabled" attr_code="DIC_GIFT_UNIT"
									style="border:1px solid #a4a4a4; height:18px; line-height:18px;width:200px; background:url(${context}/images/ipt.gif) repeat-x;"></html:selectdict>
							</td>
						...
						<tr>
							<th>赠品品牌：</th>
							<td><select name="gift.gift_brand" disabled="disabled"
								style="width: 200px;">
									<option value="">--请选择--</option>
									<c:forEach var="giftBrand" items="${giftBrandList}">
										<option
											<c:if test="${gift.gift_brand!=null && gift.gift_brand==giftBrand.brand_code}">selected="selected"</c:if>
											value="${giftBrand.brand_code}">${giftBrand.name}</option>
									</c:forEach>
							</select></td>
							<th>赠品型号：</th>
							<td><select name="gift.gift_model" disabled="disabled" style="width: 200px;">
						...
						<tr>
							<th>赠品机型：</th>
							<td><input name="gift.gift_sku" disabled="disabled" value="${gift.gift_sku }" class="ipt_new" style="width: 200px;" />
							</td>
							<th>是否需要加工：</th>
							<td><html:selectdict disabled="disabled" name="gift.is_process" curr_val="${gift.is_process}" attr_code="DIC_IS_PROCESS"
									style="border:1px solid #a4a4a4; height:18px; line-height:18px;width:200px; background:url(${context}/images/ipt.gif) repeat-x;"></html:selectdict>
							</td>
							<th>加工类型：</th>
							<td><html:selectdict disabled="disabled" appen_options="<option value=''>---请选择---</option>"
									name="gift.process_type" curr_val="${gift.process_type}" attr_code="DIC_PROCESS_TYPE"
									style="border:1px solid #a4a4a4; height:18px; line-height:18px;width:200px; background:url(${context}/images/ipt.gif) repeat-x;"></html:selectdict>
							</td>
						</tr>
						<tr>
							<th>加工内容：</th>
							<td><input name="gift.process_desc" disabled="disabled"
								type="text" class="ipt_new" value="${gift.process_desc}"
