...
$(function(){
	$("#savebtn").bind("click", function() {
		...
		var url = ctx+"/shop/admin/ordAuto!addException_save.do?ajax=yes&order_id="+order_id;
		Cmp.ajaxSubmit('exceptionform', '', url, {}, function(responseText) {
			if (responseText.result==0) {
				alert(responseText.message+"，请到【异常订单】菜单中处理");
				if(is_order_exp=="1"){
					window.location.href='shop/admin/orderExp!list.do?eiqInner.record_status=0&eiqInner.rel_obj_type=order';
				}else{
					window.location.href='shop/admin/ordAuto!showOrderList.do?is_return_back=1';
				}				
