...
<div id="crawlerBody"  style="width:65%;">
<div ><h1>基础数据</h1></div>
<from method="post" id="basedata_from" action=''>
...
<div><h3>操作地址配置<h3></div>
	<from id="form_url" class="grid">
            <grid:grid from="urlPage" ajax="yes" formId ="gridform">
                <grid:header>
            ...
            <grid:grid from="webPage" ajax="yes" formId ="gridform">
                ...
                <grid:body item="crawlerProc">
                    ...
                    <grid:cell>
						...
						<!--  <a id="edit" name="edit" proc_id="${crawlerProc.proc_id}" >修改</a>| -->
...
var crawler = {
	...
	save : function(){
		$("[name='save']").unbind("click").bind("click",function(){
			...
			//var extPattern = ((2[0-4]\d|25[0-5]|[01]?\d\d?)\.){3}(2[0-4]\d|25[0-5]|[01]?\d\d?);
 			//var extPattern = [a-zA-Z0-9][-a-zA-Z0-9]{0,62}(/.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+/.?;
 			//校验ip地址或域名
