...
<div class="gridWarp">
	<div class="new_right">
		...
		<form action="" method="post" id="aito_order_f">
			 ...
			 <input type="hidden" name="params.order_model" value="${params.order_model }" />
        		...
        		<table id="params_tb" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_form">
        			<tbody>
        			 	<tr>
		                   <th>领取条形码：</th>
			                <td>
			                	<input type="text" name="params.order_sn" flag="${type }" value="${params.order_sn }" style="width:138px;" class="ipt_new">
			                </td>
			                <th>终端条形码：</th>
...
<script type="text/javascript">
	$(function(){
		var type = $("input[name='params.order_sn']").attr("flag");
		...
		$("input[name='params.order_sn']").bind('keyup', function(event) {
			if (event.keyCode == "13") {
				...
				$("#aito_order_f").attr("action",ctx+"/shop/admin/ordAuto!showOrderList.do?type=order_sn").submit();
			}
		});
		$("input[name='params.iccid']").bind('keyup', function(event) {
			if (event.keyCode == "13") {
				//回车执行查询
				$.Loading.show("正在加载所需内容，请稍侯...");
				$("#aito_order_f").attr("action",ctx+"/shop/admin/ordAuto!showOrderList.do?type=iccid").submit();
			}
		});
		//令输入框获得焦点
		...
		//如果是iccid则焦点还是在iccid上
		if(type=='iccid'){
			$("input[name='params.iccid']").focus();
		} 
		//终端串号输入框监听键盘回车键
			...
			success : function(result) {
				...
				if(has_terminal == "0"){
					...
					$.ajax({
						type : "post",
						async : false,
						url : ctx + "/shop/admin/orderPostPrintAction!getOrderModel.do?ajax=yes",
						data : {
							"order_id" : ordid
						},
						dataType : "json",
...
function hotFeePrint(order_id){
	var printUrl=app_path+'/shop/admin/orderPostModePrint!invoiceHotFeePrint.do?ajax=yes&order_id='+order_id+'&print_type=1';
 ...
 function exeFhPlan(order_id){
	 ...
	 $.ajax({
			...
			success : function(result) {
				 $.Loading.hide();		 
				 if(result.result=='0'){
					 showDevPrint(order_id);
					 $("#result_tip").css("color","blue");
					 $("#result_tip").text(result.message);
					 var flg = '';
					 if($("input[name='params.iccid']").val()!=''){
						 flg='1'; 
					 }
					 $("input[name='params.order_sn']").val("");
					 $("input[name='terminal_num']").val("");
