...
<form method="post" id="serchform" action='<%=request.getContextPath() %>/shop/admin/ordAuto!showMonitorList.do'>
 <div class="searchformDiv">
		<!-- 选择框 -->
	  <jsp:include page="monitor_order_list_param.jsp"/>
</div>
</form>
<div class="grid" id="goodslist">
     <form method="POST"  id="monitor_order_list"  >
      
         <grid:grid from="webpage" formId="serchform" >
			<grid:header>
				<grid:cell width="110px">外部订单号</grid:cell>
				<grid:cell width="80px">创建时间</grid:cell>
				<grid:cell width="50px">订单来源</grid:cell>
				<grid:cell width="50px">地市</grid:cell>
				<grid:cell width="50px">商品类型</grid:cell>
				<grid:cell width="200px">商品名称</grid:cell>
				<grid:cell width="80px">套餐名称</grid:cell>
				<grid:cell width="50px">所选号码</grid:cell>
				<grid:cell width="60px">WMS退单状态</grid:cell>
				<grid:cell width="50px">签收状态</grid:cell>
				<grid:cell width="50px">回单状态</grid:cell>
				<grid:cell width="50px">是否预约单</grid:cell>
				<grid:cell width="50px">关联单号</grid:cell>
			</grid:header>
		    <grid:body item="order">
		  	    ...
		  	    <grid:cell>${order.wms_refund_status}</grid:cell>
		  	    <grid:cell>${order.sign_status }</grid:cell>
		  	    <grid:cell>${order.receipt_status }</grid:cell>
		  	    <grid:cell>${order.wm_isreservation_from }</grid:cell>
		  	    <grid:cell>${order.related_order_id }</grid:cell>
		  	</grid:body>
		</grid:grid>
	</form>
 </div>
...
$(function(){
	$("#searchBtn").click(function(){
		if (check()) {
			$("#serchform").attr("action",ctx+"/shop/admin/ordAuto!showMonitorList.do");
			$("#serchform").submit();
		}
	});
	$("#excelOrd").click(function(){
		if (check()) {
			var url = ctx+"/shop/admin/ordAuto!showMonitorList.do?ajax=yes&excel=check";
			Cmp.ajaxSubmit('serchform','',url,{},function(data){
				if(data.result=='0'){
					alert("数据量超过10000，请按时间分次导出。");
				}else{
					$("#serchform").attr("action",ctx+"/shop/admin/ordAuto!showMonitorList.do?ajax=yes&excel=yes");
					$("#serchform").submit();
					$("#serchform").attr("action",ctx+"/shop/admin/ordAuto!showMonitorList.do");
				}
			},'json');
