		    ...
		    <grid:body item="order">
		       ...
		       <grid:cell style="width:100px;">
		       		<div class="order_pri">
                            ...
                            <p class="ps">外呼原因：${order.outcall_type_c }</p>
		       ...
		       <grid:cell style="width:200px;">
		        ...
		        <grid:cell style="width:200px;">
		       		<div >
                            <button type="button" class="call_out" lock_clazz="${order.lock_clazz }" outcall_type_c="${order.outcall_type_c }" order_id="${order.orderTree.order_id }">外呼处理</button>
                        </div>
		       </grid:cell> 
		  </grid:body>
      	</grid:grid>
	...
	$(function(){
		...
		$("a[name=inner_order_id]").bind("click",function(){	
			...
			$.ajax({
				type : "post",
				async : false,
				url : "ordAuto!order_lock_showCallOut.do?ajax=yes&order_id="+order_id,
				data : {},
				dataType : "json",
				success : function(data) {
					if (data.result == "0") {
			...
			if(falgs){
				...
				var outcall_type_c = $(this).attr("outcall_type_c");
				if(o_url && o_url.indexOf("?")!=-1){
					o_url += "&order_id="+order_id+"&query_type=${query_type}"+"&outcall_type_c="+outcall_type_c;
				}else{
					o_url += "?order_id="+order_id+"&query_type=${query_type}"+"&outcall_type_c="+outcall_type_c;
				}
