(function() {
  ...
  function code128(code, barcodeType) {
    ...
    var codestr = code;
    codestr = addEmpty(codestr);
    //return bar2html(a.join(''))+"<br><span>"+codestr+"</span>" ; 
    return "<table><tr><td align='center'>"+bar2html(a.join(''))+"</td></tr><tr><td align='center'><span>"+codestr+"</span></td></tr></table>" ; 
  }
  
  
  function code128com(code, barcodeType) {
	    var codeorg = code;
	    if (arguments.length<2)
	    	 barcodeType = code128Detect(code);
	    if (barcodeType=='C' && code.length%2==1) 
  ...
  function code128new(code, barcodeType) {//仅返回二维码
	    if (arguments.length<2)
	    	 barcodeType = code128Detect(code);
	    if (barcodeType=='C' && code.length%2==1) 
	    	code = '0'+code; 
	    ...
	    codestr = addEmpty(codestr);
	    //return bar2html(a.join(''))+"<br><span>"+codestr+"</span>" ; 
	    return "<table><tr><td align='center'>"+bar2html(a.join(''))+"</td></tr></table>" ; 
  }
  ...
  exports.code128new = code128new;
  exports.code128com = code128com;
})();
...
function createBarcodenew(showDiv,textValue,barcodeType){//仅返回二维码
	var divElement = document.getElementById(showDiv);
	divElement.innerHTML = code128new(textValue,barcodeType);
}

function createBarcodeCom(showDiv,textValue,barcodeType){//仅返回二维码
	var divElement = document.getElementById(showDiv);
	divElement.innerHTML = code128com(textValue,barcodeType);
}
function addEmpty(str){
	var len = str.length;
	var newStr = "";
	for(var i=0;i<len/3;i++){
		newStr = newStr + str.substring(i*3,(i+1)*3)+"&nbsp;&nbsp;&nbsp;";
	}
	return "&nbsp;&nbsp;&nbsp;"+newStr;
}
