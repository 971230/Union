...
var UserAdmin_agent={
	...
	updatejsonBack : function(responseText) { // json回调函数
		...
		if (responseText.result == 1) {
		   ...
		   window.location=ctx + "userAdmin!editLLkjAgent.do?id=${userAdmin.userid}";
		}
		if (responseText.result == 0) {
			alert(responseText.message);	
		  
		    window.location="userAdmin!llkjUserList.do?state=2&usertype=1";
		}
	},
};
...
$(function(){
	...
	UserAdmin_staff.init();
	    ...
	    $("#editbtn").click(function(){
	        ...
	        if($("[name='updatePwd']").attr("checked")){
	        	if(checkpwdandCrm()){
