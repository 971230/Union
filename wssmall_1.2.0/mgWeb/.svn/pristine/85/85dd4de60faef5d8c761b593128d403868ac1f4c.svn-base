...
var Widgeter = {
	...
	createWidget : function(_panel, widgetHandle) {
		...
		if(!this.created){//修复当拖进可用区域后再拖到不可用区域时的问题
			...
			$('#wssapp_setting').dialog('option','title','创建挂件');
			//绑定创建挂件的事件
			$("#wssapp_setting").dialog('option', 'buttons', { "确定": function() {
				that.doCreateWidget();
			} })
			//绑定关闭事件,如果不成功除鬼
	...
	doCreateWidget : function() {
		...
		$('#wssapp_form').ajaxSubmit(options);
	},
	...
	editWidget : function(widgetEl) {
		...
		$('#wssapp_setting').dialog('option','title','修改挂件');
		$('#wssapp_setting').dialog('option', 'buttons', { "确定": function() {
