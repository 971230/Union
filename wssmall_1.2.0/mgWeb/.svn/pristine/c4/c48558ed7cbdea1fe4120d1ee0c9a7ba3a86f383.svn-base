		...
		<tr>
			...
			<table cellspacing="0" cellpadding="0" border="0" width="100%"
				...
				<tbody>
					...
					<tr>
						<td><html:dateformat pattern="yyyy-MM-dd HH:mm:ss" d_time="${payment.create_time}" /></td>
						...
						<td>
							...
							<c:if test="${payment.pay_type == 'free' }">免支付</c:if>
						</td>
						<td>
							<c:if test="${payment.status == 1 }">成功</c:if>
							<c:if test="${payment.status == 2 }">未支付</c:if>
						</td>
					</tr>
					</c:forEach>
				</tbody>
