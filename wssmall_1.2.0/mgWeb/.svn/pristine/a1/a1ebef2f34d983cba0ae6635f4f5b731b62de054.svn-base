...
<div >
<form  action="goods!productList.do"  id="searchProductsListForm" method="post">
		<div class="searchformDiv">
    	      ...
    	      <tr>
					...
					<td>
						<input type="text" class="ipttxt" style="width: 150px" id="sn" name="sn" value="${params.sn }"/>
					</td>
    	...
    	<div class="comBtnDiv">
			...
			<!--  <a href="goods!handleSpService.do" style="margin-right:10px;" onclick="javascript:void(0);" class="graybtn1"><span>导出货品</span></a>-->
	 ...
	 <input type="button" title="说明：选择页面上货品批量发布" style="margin-right:10px;" onclick="goodsBatchPublishByChoice();" value="可选批量发布" class="comBtn">
			</a>
			</span>
		</div>
	</form>
	<form id="gridform" class="grid">
		<grid:grid from="webpage" formId="searchProductsListForm" action="${listFormActionVal}" >
			...
			<grid:body item="goods">
				...
				<grid:cell>&nbsp;
						<span >
							<a href="goods!productEdit.do?goods_id=${goods.goods_id}&selfProdType=${selfProdType}&store=${goods.store}&have_stock=${goods.have_stock}"  id="goods_name_${goods.product_id}">${goods.name }</a> </span>
				</grid:cell>
				<grid:cell>
					${goods.type_name}
				</grid:cell>
			...
			<tr id="iframe_tr" style="display:none">
              <td colspan="9">
                <iframe style="width:100%; height:65px;" src="goods/params_input_h.html"></iframe>
              </td>
            </tr>
...
function goodsBatchPublishByChoice(){
	var $checkedGoods = $('input[name="eckBox"]:checked');
	...
	var url ="goodsOrg!productPubtree.do?goodsIds="+ids+"&type=products";;
	abrirCajaVentana("goodsSelectShopDialog",url);
}
