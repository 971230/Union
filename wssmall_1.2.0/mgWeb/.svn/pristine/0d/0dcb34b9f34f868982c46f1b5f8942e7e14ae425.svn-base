        ...
        <form action="" id="orderexp_spec_list_fm" class="grid_w" method="post">
        	<grid:grid from="webpage" formId="order_exp_count_f">
				...
				<grid:body item="expSpecCount">
			  	    <grid:cell><div  style="width:450px;word-break:keep-all;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                            			<xmp>${expSpecCount.match_content}</xmp>
									</div></grid:cell>		
			  	    <grid:cell><a name="showExpInst" text="异常单管理" key_id="${expSpecCount.key_id}" target="iframe" href="<%=request.getContextPath() %>/shop/admin/orderExp!list.do?start_time=${start_time }&end_time=${end_time }&eiqInner.key_id=${expSpecCount.key_id}" app_menu_id="2015021501" iframeall=yes>${expSpecCount.catalog_count }</a></grid:cell>
...
$(function(){
	...
	$("a[name=showExpInst]").bind("click",function(){
		...
		parent.parent.postMessage(key_id,'*');
		return false;
	});
	/**
	var cut = $(this);
