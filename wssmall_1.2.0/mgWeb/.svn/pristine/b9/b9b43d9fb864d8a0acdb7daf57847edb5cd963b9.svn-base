var Regions = {
		init:function(){
			...
			$.ajax({
				type : "post",
				url : "region!listChildren.do?parentid=0&ajax=yes",
				//data : {"parent_id":id,"level":level},
				dataType : 'text',
				success : function(result) {
					//alert('result'+result);
					$("#regionMain").after($(result));
					self.bindEvent();
				}
			});
		...
		bindEvent:function(){
			...
			$(".delete").bind("click", function(){
				...
				var self = this;
				var parentid = $("tr[id="+$(self).attr("region_id")+"]").attr("parentid");
				var region_path = ","+ parentid+"," +$(self).attr("region_id") ;
				$.ajax({
					url:"region!delete.do?region_id="+ region_path+ "&ajax=yes",
					dataType:"json",
					success:function(){
