...
var Supplier=$.extend({},Eop.Grid,{
	...
	doAuditSupplier:function(supplier_id){
		...
		Cmp.asExcute('',ctx + '/shop/admin/supplierFJ!isSupplierRegeditAudit.do?ajax=yes&supplier_id=' + supplier_id,{},function(response){
			if(response.result == 1){
				isAudit = true;
			}
		},'json');
		if(!isAudit){
			var url=app_path+'/shop/admin/supplierFJ!showregeditaudit.do?ajax=yes&supplier_id='+supplier_id;
	    	self.toUrlOpen(url);
