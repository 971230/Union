 ...
 *@auther:Reason.Yea 
 *分页控件
 *var pager = new Pager(div_id,action_id,action,param,function(reply){});
 *@param div_id:页面放置table的div_id
 *@param action_id:java或者sql服务的action_id
 *@param action:是否sql服务,true表示sql，false表示java服务
...
(function(scope) {
	var Pager = Base.extend({
		constructor : function(div_id,func_id,action,param,call_back,exp_excel,queryTable,hide_pager,class_name) {
			this.version = "1.0";
			this.config={};
		...
		doService:function(){
			...
			Service.excuteNoLoading(me.config.func_id, me.config.action, me.config, function(reply){
