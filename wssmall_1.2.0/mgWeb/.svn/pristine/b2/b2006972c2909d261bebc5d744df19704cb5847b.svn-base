			...
			<grid:grid  from="webpage" ajax='yes' formId="tcForm">
			    ...
			    <grid:body item="obj">
				    ...
				    <grid:cell>${obj.begin_date}</grid:cell>
				   	...
				   	<grid:cell>
				   		<a onclick="javascript:cancelTask('${obj.task_id}','${obj.task_code}','${obj.task_name}');return false;">删除</a>
				   	</grid:cell>
			  </grid:body>  
			</grid:grid>
		</form>
...
function cancelTask(task_id, task_code, task_name){
	if(confirm("编号为"+ task_code + "，名称为"+ task_name+"的任务即将被删除，您确认本次操作吗？")){
		$.ajax({
			url: ctx + "/shop/admin/task!cancelTask.do?ajax=yes",
			type:"post",
