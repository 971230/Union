...
<div class="grid">
	...
	<div class="toolbar">
		...
		<a href="order!trash_list.do"><input class="comBtn" type="button" name="" id="" value="回收站" style="margin-right:10px;outline-style:none"/></a>
...
<grid:grid  from="webpage">
  ...
  <grid:body item="order">
        ...
        <grid:cell><c:if test="${order.status<3}"><B></c:if><html:dateformat pattern="yyyy-MM-dd" d_time="${order.create_time}"></html:dateformat> <c:if test="${order.status<3}"></B></c:if></grid:cell> 
