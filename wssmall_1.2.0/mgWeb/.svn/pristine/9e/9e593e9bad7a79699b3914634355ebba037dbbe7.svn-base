    ...
    <table>
        <c:choose>
	       <c:when test="${!empty orgList}">
	         <tr>
	            <td colspan="3" style="text-align:center;color:red">选中的商品发布到了如下组织，确定要停用吗？</td>
	         </tr>
	         <tr>
		         <td>
			       <c:forEach var = "org" items='${orgList}' varStatus="status">
						<span style="text-align:left;color:blue">${org.name}</span> </br></br>
						<span>${org.sale_store}</span></br></br>
			       </c:forEach>
			     </td>
			 </tr>  
	       </c:when>
