...
var GoodsSelector={
	...
	search:function(onConfirm){
		var self = this;
		var options = {
				url :ctx+"/shop/admin/selector!goods.do?ajax=yes",
				type : "GET",
				dataType : 'html',
	...
	getGoodsList:function(callback){
		var options = {
				url :ctx+"/shop/admin/selector!listGoods.do?ajax=yes",
				type : "GET",
				dataType : 'json',
				success : function(result) {				
					if(callback){
...
var ProductSelector={
	...
	search:function(onConfirm){
		...
		$("#serchformProd").ajaxSubmit(options);		
	},
	getGoodsList:function(callback){
		var options = {
...
var ActivitySelector={
		...
		init:function(conid,onConfirm)	{
			...
			$("#"+conid+" .submitBtn").click(function(){
				self.getActivityList(onConfirm);
				Eop.Dialog.close(conid);
			});
		...
		search:function(onConfirm){
			var self = this;
			var options = {
					url :basePath+"selector!activity.do?ajax=yes",
					type : "GET",
					dataType : 'html',
					success : function(result) {				
						$("#"+self.conid).empty().append(result);
			...
			$("#serchActform").ajaxSubmit(options);		
		},
		getActivityList:function(callback){
			var options = {
					url :basePath+"selector!listActivity.do?ajax=yes",
					type : "GET",
					dataType : 'json',
					success : function(result) {				
						if(callback){
		...
		open:function(conid,onConfirm){
			var self= this;
			$("#"+conid).load(basePath+'selector!activity.do?ajax=yes',function(){
				self.init(conid,onConfirm);
			});
			Eop.Dialog.open(conid);		
		}
