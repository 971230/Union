...
var BackendUi={
	...
	disSysMenu:function(){
		...
		$.each(menu.sys,function(k,v){
			...
			if(v.text == '应用中心'){
				...
				link = $("<a  target='"+v.target+"' href='"+ v.url +"' text='"+v.text +"'  >" + v.text + "</a>");
		...
		$.each(menu.app,function(k,v){
			...
			var children = v.children;
				...
				if(k==0){ 
					...
					link.attr("href",app_path+"/llkj/admin/userDaily!getUserInfo.do");
	...
	disAppChildren:function(children){
		...
		//add by wui 计算高度,设置左边菜单的高度 cms全屏展示，不需要计算高度
