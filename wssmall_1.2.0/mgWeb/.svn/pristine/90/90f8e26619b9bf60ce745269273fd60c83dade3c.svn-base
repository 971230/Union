	...
	<tr>
		<td width="220px" style="vertical-align: top;">
			...
			<ul>
				...
				<li>${target }</li>
				<li>发表时间：[<html:dateformat pattern="yyyy-MM-dd HH:mm:ss" d_time="${commentDTO.comments.time }"></html:dateformat>]</li>
			</ul>
			</div>
		</td>
		<td class="tableform">
			...
			<table>
				<tr>
					<th style="text-align: left; padding-left: 0;">
						...
						<h5>回复用户评论：</h5>
						<textarea rows="7" cols="70" name="comments.commt" style="border: solid #069 1px;" id="commentscontent"></textarea>
					</th>
				</tr>
 ...
 <tr><td >
  ...
  <a href="javascript:;" id="comments_commit">回复</a>
			...
			<div class="tableform">
				...
				<h5>${reply.author } 于[<html:dateformat pattern="yyyy-MM-dd HH:mm:ss" d_time="${reply.time }"></html:dateformat>]回复：[<c:if test="${reply.display == 'true' }"><a href="javascript:hideComments('${reply.comment_id}')">隐藏</a></c:if><c:if test="${reply.display != 'true' }"><a href="javascript:showComments('${reply.comment_id}')">显示</a></c:if>] [<a href="comments!deletealone.do?comment_id=${reply.comment_id }">删除</a>]</h5><!--
				<h5>${reply.author } 于[<html:dateformat pattern="yyyy-MM-dd HH:mm:ss" d_time="${reply.time }"></html:dateformat>]回复：[<c:if test="${reply.display == 'true' }"><a href="comments!hide.do?comment_id=${reply.comment_id }">隐藏</a></c:if><c:if test="${reply.display != 'true' }"><a href="comments!show.do?comment_id=${reply.comment_id }">显示</a></c:if>] [<a href="comments!deletealone.do?comment_id=${reply.comment_id }">删除</a>]</h5>
...
<!--
	...
	function hideComments(comment_id){
		var options = {
