...
<div >
  <input type="hidden" id="currentPartnerId" value="${currentPartnerId }">
	<form action="partner!list.do" method="post" id="parListForm">
		<div class="searchformDiv">
	    	  <table width="100%" cellspacing="0" cellpadding="0" border="0">
	    	    ...
	    	    <th>分销商名称：</th>
	    	      <td><input type="text" class="ipttxt"  style="width: 90px" name="part_serch.partner_name" value="${part_serch.partner_name }" class="searchipt" /></td>
	    	      <th>分销商编码：</th>
	    	      <td><input type="text" class="ipttxt"  style="width: 90px" name="part_serch.partner_code" value="${part_serch.partner_code }" class="searchipt" /></td>
	    	      <th>联系人：</th>
    	...
    	<div class="comBtnDiv">
    	  <c:if test="${stateType == '1' or stateType=='all'}">
	    	    <a href="partner!add.do?flag=add&is_edit=false" style="margin-right:10px;" class="graybtn1" ><span>添加</span></a>
			...
			<c:if test="${currFounder!=2}">
			    <c:if test="${stateType== '1' or stateType=='all'}">
			     ...
			     <c:if test="${stateType== '1' or stateType=='all'}">
			         ...
			         <a href="javascript:;" id="blockBtn" style="margin-right:10px;" class="graybtn1" ><span>冻结分销商</span></a>
			     </c:if>
			   <c:if test="${stateType== '1' or stateType == '2' or stateType=='all'}">
			       
			       <a href="javascript:;" id="renewBtn" style="margin-right:10px;" class="graybtn1" ><span>解冻分销商</span></a>
   ...
   <grid:cell>
       <c:if test="${obj.state == 0 or obj.state==1}">
          <c:if test="${obj.sequ==0}">
	  	  <a title="编辑" href="partner!detail.do?flag=edit&state=${obj.state }&sequ=${obj.sequ }&partner_id=${obj.partner_id }"> 修改	</a>
	  	  </c:if>
	   </c:if>
