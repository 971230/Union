...
var batchReturn = {
	...
	upLoadExcel : function(currA) {
		...
		if(hasCommit == false) {
			iframeDocument.find("form").attr("action",ctx + 
				"/servlet/batchAcceptExcelServlet?url=/shop/admin/batch/UpLoadFile.jsp&service=phoneNo").submit();
			$(currA).attr("disabled", true);
			hasCommit = true;
		}
		return true;
	...
	exportExcel : function(){	
		...
		iframeDocument.find("form").attr("action",ctx + 
				"/servlet/batchAcceptExcelServlet?url=/shop/admin/batch/UpLoadFile.jsp&lx=mb&service=phoneNo").submit();
	},
	loadReturnList : function(batchId){
		AccNbrList.returnBatch(batchId);
	}
}
$(function() {
	batchReturn.init();
