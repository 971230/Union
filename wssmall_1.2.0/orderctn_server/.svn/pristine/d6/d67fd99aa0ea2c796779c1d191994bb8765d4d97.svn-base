...
public class InternetOrderServlet extends HttpServlet{

	private static Logger logger = Logger.getLogger(InternetOrderServlet.class);
...
//			inJson = new String(inJson.getBytes("GBK"), "ISO8859_1");
			logger.info("[InternetOrderServlet] 完整请求报文:" + inJson);
			//解析接口名称
		...
		//返回接口调用结果
		logger.info("[InternetOrderServlet] 返回报文:"+outJson);
	...
	private String ordersyn(String inJsonBody){
		...
		try {
			...
			logger.info("[InternetOrderServlet]-[periphery.ordersyn] 请求报文体:" + inJson);
			if (null != inJson && !"".equals(inJson)) {
			    ...
			    if (null != def && def.equals(key)) {
					...
					//
					req.setOutServiceCode(OrderCtnConsts.OUT_SERVICE_CODE_INTERNETORDERSTANDARD);
					...
					logger.info("[InternetOrderServlet] 执行标准化总时间为:[" + (end-begin) +"] 返回对象: "+ IJSONUtil.beanToJson(resp));
					if (null != resp) {
