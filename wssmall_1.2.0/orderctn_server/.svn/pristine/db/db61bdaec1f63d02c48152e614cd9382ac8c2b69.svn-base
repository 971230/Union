...
public class InternetGroupOrderCtnServlet extends HttpServlet{

	private static Logger logger = Logger.getLogger(InternetGroupOrderCtnServlet.class);
	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException{
		...
		try {
			...
			logger.info("[InternetGroupOrderCtnServlet] 完整请求报文:" + inJson);
			//解析接口名称
			net.sf.json.JSONObject temInJsonObj = net.sf.json.JSONObject.fromObject(inJson);
			inJsonBody = temInJsonObj.getJSONObject("bss_order_pre_submit_req")==null?"":temInJsonObj.getJSONObject("bss_order_pre_submit_req").toString();
			JSONObject json = JSONObject.fromObject(inJsonBody);
			orderId = json.getString("mall_order_id");
		} catch (Exception e) {
			outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"1\",\"resp_msg\":\"订单接受失败\",\"order_id\":\""+orderId+"\"}}";
		...
		//返回接口调用结果
		logger.info("[InternetGroupOrderCtnServlet] 返回报文:"+outJson);
	...
	private String ordersyn(String inJsonBody){
		...
		try {
			...
			logger.info("[InternetGroupOrderCtnServlet]-[periphery.ordersyn] 请求报文体:" + inJson);
			if (null != inJson && !"".equals(inJson)) {
				...
				if(StringUtils.isEmpty(activeNo) || StringUtils.isEmpty(orderId) || StringUtils.isEmpty(orderSource)){
					outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"1\",\"resp_msg\":\"订单[" + orderId + "]同步失败,缺少订单关键参数.\",\"order_id\":\""+orderId+"\"}}";
				}
			    ...
			    if (null != def && def.equals(key)) {
					outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"1\",\"resp_msg\":\"订单[" + orderId + "]同步失败,订单已接收.\",\"order_id\":\""+orderId+"\"}}";
			    } else {
			    	// 写缓存
					...
					req.setOutServiceCode(OrderCtnConsts.OUT_SERVICE_CODE_INTERGROUPORDERSTANDARD);
					...
					logger.info("[InternetGroupOrderCtnServlet] 执行标准化总时间为:[" + (end-begin) +"] 返回对象: "+ IJSONUtil.beanToJson(resp));
					if (null != resp) {
						...
						if (OrderCtnConsts.RPC_TYPE_DUBBO.equals(rpc_type)) {// dubbo 调用返回
							...
							if (ConstsCore.ERROR_SUCC.equals(error_code)) {// 同步成功
								outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"0\",\"resp_msg\":\"订单[" + orderId + "]同步成功\",\"order_id\":\""+orderId+"\"}}";
							} else {
								outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"1\",\"resp_msg\":\"订单[" + orderId + "]同步失败,"+(resp.getError_msg() == null? "":resp.getError_msg())+"\",\"order_id\":\""+orderId+"\"}}";
							}
							...
							if (ConstsCore.ERROR_SUCC.equals(error_code)) {// 同步成功
								...
								outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"1\",\"resp_msg\":\"订单[" + orderId + "]同步失败\",\"order_id\":\""+orderId+"\"}}";
							}
						} else {
							outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"1\",\"resp_msg\":\"订单[" + orderId + "]同步失败\",\"order_id\":\""+orderId+"\"}}";
						}
						...
						if("1".equals(esearch_flag)) {
							...
							if (StringUtils.isNotEmpty(resp.getBase_co_id())) {
								...
								thread.setName("BroadBandCtnWriteEsearchThread");
								thread.start();
							}
						}
				...
				outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"1\",\"resp_msg\":\"同步失败,接口报文为空\",\"order_id\":\"\"}}";
			}
		} catch (Exception e) {
			outJson = "{\"bss_order_pre_submit_rsq\":{\"resp_code\":\"1\",\"resp_msg\":\"同步失败,"+e.getMessage()+"\",\"order_id\":\"\"}}";
			logger.info(e.getMessage(), e);
		}
		return outJson;
	}
