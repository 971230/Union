...
public class FixBusiOrderServlet extends HttpServlet{

	private static Logger logger = Logger.getLogger(FixBusiOrderServlet.class);
	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException{
		...
		try {
			...
			logger.info("[FixBusiOrderServlet] 完整请求报文:" + inJson);
			//解析接口名称
		...
		//返回接口调用结果
		logger.info("[FixBusiOrderServlet] 返回报文:"+outJson);
	...
	private String ordersyn(String inJsonBody){
		...
		try {
			...
			logger.info("[FixBusiOrderServlet]-[periphery.ordersyn] 请求报文体:" + inJson);
			if (null != inJson && !"".equals(inJson)) {
				...
				if(org.apache.commons.lang.StringUtils.isBlank(orderSource)){
					JSONObject jsonObj = JSONObject.fromObject(inJson);
					orderSource = String.valueOf(jsonObj.get("source_system"));
				}
				...
				String intent_order_id = MallUtils.searchValue(inJson, "rel_order_id");
				...
				if (!StringUtils.isEmpty(intent_order_id)) {
					...
					String sql = "select (select distinct order_id from es_order_intent where intent_order_id='" + intent_order_id	+ "' and source_from = '" + ManagerUtils.getSourceFrom() + "') as intent, (select distinct order_id from es_order_extvtl where intent_order_id='" + intent_order_id	+ "' and source_from = '" + ManagerUtils.getSourceFrom() + "') as order_id from dual";
					...
					if (StringUtils.isEmpty(map.get("intent"))) {
						outJson = "{\"mall_resp\":{\"resp_code\":\"1\",\"resp_msg\":\"订单[" + orderId
								+ "]同步失败." + intent_order_id + "异常}}";
						return outJson;
					}
					if (!StringUtils.isEmpty(map.get("order_id"))) {
						outJson = "{\"mall_resp\":{\"resp_code\":\"1\",\"resp_msg\":\"订单[" + orderId
								+ "]同步失败,订单已接收." + intent_order_id + "重复}}";
						return outJson;
					}
				}
			    ...
			    if (null != def && def.equals(key)) {
					...
					req.setOutServiceCode(OrderCtnConsts.OUT_SERVICE_CODE_FIXBUSIORDERSTANDARD);
					...
					logger.info("[FixBusiOrderServlet] 执行标准化总时间为:[" + (end-begin) +"] 返回对象: "+ IJSONUtil.beanToJson(resp));
					if (null != resp) {
						...
						if (OrderCtnConsts.RPC_TYPE_DUBBO.equals(rpc_type)) {// dubbo 调用返回
							...
							if (ConstsCore.ERROR_SUCC.equals(error_code)) {// 同步成功
								outJson = "{\"mall_resp\":{\"resp_code\":\"0\",\"resp_msg\":\"订单[" + orderId + "]同步成功\",\"order_id\":\""+orderId+"\"}}";
							} else {
					    ...
					    if("1".equals(esearch_flag)) {
							...
							if (StringUtils.isNotEmpty(resp.getBase_co_id())) {
								...
								thread.setName("FixBusiOrderWriteEsearchThread");
								thread.start();
							}
					    }
					} else {
