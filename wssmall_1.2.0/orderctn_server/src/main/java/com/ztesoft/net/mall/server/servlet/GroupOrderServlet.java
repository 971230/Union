...
public class GroupOrderServlet extends HttpServlet{
	...
	private static Logger logger = Logger.getLogger(GroupOrderServlet.class);
	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException{
		...
		try {
			...
			logger.info("[GroupOrderServlet] 完整请求报文:" + inJson);
			//解析接口名称
		...
		//返回接口调用结果
		logger.info("[GroupOrderServlet] 返回报文:"+outJson);
	...
	private String ordersyn(String inJsonBody){
		...
		try {
			...
			logger.info("[GroupOrderServlet]-[periphery.ordersyn] 请求报文体:" + inJson);
			if (null != inJson && !"".equals(inJson)) {
			    ...
			    if (null != def && def.equals(key)) {
					...
					req.setOutServiceCode(OrderCtnConsts.OUT_SERVICE_CODE_GROUPORDERSTANDARD);
					...
					logger.info("[GroupOrderServlet] 执行标准化总时间为:[" + (end-begin) +"] 返回对象: "+ IJSONUtil.beanToJson(resp));
					if (null != resp) {
						...
						if("1".equals(esearch_flag)) {
							...
							if (StringUtils.isNotEmpty(resp.getBase_co_id())) {
								...
								thread.setName("GroupOrderWriteEsearchThread");
								thread.start();
							}
						}
					} else {
