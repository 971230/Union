...
public class BroadBandCtnServlet extends HttpServlet{

	private static Logger logger = Logger.getLogger(BroadBandCtnServlet.class);
	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException{
		...
		try {
			...
			logger.info("[BroadBandCtnServlet] 完整请求报文:" + inJson);
			//解析接口名称
		...
		//返回接口调用结果
		logger.info("[BroadBandCtnServlet] 返回报文:"+outJson);
	...
	private String ordersyn(String inJsonBody){
		...
		try {
			...
			logger.info("[BroadBandCtnServlet]-[periphery.ordersyn] 请求报文体:" + inJson);
			if (null != inJson && !"".equals(inJson)) {
			    ...
			    if (null != def && def.equals(key)) {
					...
					req.setOutServiceCode(OrderCtnConsts.OUT_SERVICE_CODE_BROADBANDSTANDARD);
					...
					logger.info("[BroadBandCtnServlet] 执行标准化总时间为:[" + (end-begin) +"] 返回对象: "+ IJSONUtil.beanToJson(resp));
					if (null != resp) {
