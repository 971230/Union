 ...
 * 
 * 
 * @Description: 网手微宽带订单归集
 * @author MCC
 * @date 2016-11-09 17:07
 */
public class NetworkHandMicroCtnServlet extends HttpServlet {

	private static Logger logger = Logger.getLogger(NetworkHandMicroCtnServlet.class);
	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			logger.info("[NetworkHandMicroCtnServlet] 完整请求报文:" + inJson);
			if (null != inJson && !"".equals(inJson)) {
				//外部订单号
				JSONObject json = JSONObject.fromObject(inJson);//MallUtils.searchValue 获取不到属性，改为转JSONObject取值
				orderId = json.getString("SHCID");//MallUtils.searchValue(inJson, "outOrderId");
				//订单来源
				String sourceStr = json.getString("SYS_TAG");
				orderSource = "ECS";
			    ...
			    if (null != def && def.equals(key)) {
					...
					req.setOutServiceCode(OrderCtnConsts.OUT_SERVICE_CODE_NETWORK_HAND_MICRO_STANDARD);
					...
					logger.info("[NetworkHandMicroCtnServlet] 执行标准化总时间为:[" + (end-begin) +"] 返回对象: "+ IJSONUtil.beanToJson(resp));
					if (null != resp) {
						...
						if("1".equals(esearch_flag)) {
							...
							if (StringUtils.isNotEmpty(resp.getBase_co_id())) {
								...
								thread.setName("NetworkHandMicroCtnWriteEsearchThread");
								thread.start();
							}
						}
	...
	private String getRequestJson(HttpServletRequest request, HttpServletResponse response)
		...
		return java.net.URLDecoder.decode(jsonBuffer.toString(), "UTF-8");
	}
}
