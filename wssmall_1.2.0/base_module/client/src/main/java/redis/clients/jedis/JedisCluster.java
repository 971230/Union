...
public class JedisCluster implements JedisCommands, BasicCommands, Closeable {
  ...
  public JedisCluster(Set<HostAndPort> jedisClusterNode, int connectionTimeout, int soTimeout,
      int maxRedirections, final String passwd, final String cliName, final GenericObjectPoolConfig poolConfig) {
    this.connectionHandler = new JedisSlotBasedConnectionHandler(jedisClusterNode, poolConfig,
        connectionTimeout, soTimeout, passwd, cliName);
    this.maxRedirections = maxRedirections;
  }
  ...
  // added by yu.guangjie at 2015-09-08
  public Long pttl(final String key) {
	...
	//added by yu.guangjie at 2015-09-14
	...
	//added by yu.guangjie at 2015-09-09
	public JedisPool getConnPoolBySlot(int slot) {
	  return connectionHandler.cache.getSlotPool(slot);
	}
	// added by yu.guangjie at 2015-09-10
	public Set<JedisPool> getMasterPools() {
	  return connectionHandler.cache.getMasterPool();
	} 
	// added by yu.guangjie at 2015-12-15
	public JedisPool getJedisPool(String nodeKey) {
	  return connectionHandler.cache.getNode(nodeKey);
	} 
	
	//added by yu.guangjie at 2015-09-10
	public ScanResult<Entry<String, String>> hscan(final String key, final String cursor, final ScanParams params) {
	  return new JedisClusterCommand<ScanResult<Entry<String, String>>>(connectionHandler, 1) {
	    @Override
	    public ScanResult<Entry<String, String>> execute(Jedis connection) {
	...
	public ScanResult<String> sscan(final String key, final String cursor, final ScanParams params) {
	 return new JedisClusterCommand<ScanResult<String>>(connectionHandler, 1) {
	   @Override
	   public ScanResult<String> execute(Jedis connection) {
	...
	public ScanResult<Tuple> zscan(final String key, final String cursor, final ScanParams params) {
	 return new JedisClusterCommand<ScanResult<Tuple>>(connectionHandler, 1) {
	   @Override
	   public ScanResult<Tuple> execute(Jedis connection) {
	...
	//added by yu.guangjie at 2015-09-17
	public Long subscribe(final String key, final JedisPubSub jPubSub, final String... channels) {
		 return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
		   @Override
		   public Long execute(Jedis connection) {
		     connection.subscribe(jPubSub, channels);
		     return new Long(0);
		   }
		 }.run(key);
	}
	public Long psubscribe(final String key, final JedisPubSub jPubSub, final String... patterns) {
		 return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
		   @Override
		   public Long execute(Jedis connection) {
		     connection.psubscribe(jPubSub, patterns);
		     return new Long(0);
		   }
		 }.run(key);
	}
	public Long publish(final String key, final String channel, final String message) {
		return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
			   @Override
			   public Long execute(Jedis connection) {
