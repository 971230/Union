...
public class DateUtil {
	...
	public static String[] getLastMonth() {
		 ...
		 //  logger.info(lastMonth[0] + "||" + lastMonth[1]);
		   return lastMonth;
		}
	...
	/**
	 * 根据间隔分钟数把每天从0点开始 分隔成若干个间隔 返回当前时间属于哪个间隔 index从0开始。
	 * @param intervalMins 间隔分钟数
	 * @return
	 */
	public static int determineIntervalIndex(int intervalMins) {
		if (intervalMins > Consts.MINTUTE_OF_DAY) {
			intervalMins = Consts.MINTUTE_OF_DAY;
		}
		Calendar calendar = Calendar.getInstance();
		int currentHour = calendar.get(Calendar.HOUR_OF_DAY);
		int currentMin = calendar.get(Calendar.MINUTE);
		int minOfDay = currentHour * 60 + currentMin;
		return minOfDay / intervalMins;
	}
