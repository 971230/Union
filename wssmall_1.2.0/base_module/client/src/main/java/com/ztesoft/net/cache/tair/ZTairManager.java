...
public class ZTairManager implements TairManager {

	private static final Log log = LogFactory.getLog(ZTairManager.class);
	...
	public ZTairManager() {
		this("ZTairManager", true);
	}

	public ZTairManager(String name, boolean sharedFactory) {
		this.name = name;
		if (sharedFactory)
	...
	public void initTair() {
		...
		configServer = new ConfigServer(clientFactory, groupName,configServerList, packetStreamer);
	...
	private TairClient getClient(Object key, boolean isRead) {
		...
		if (host != null) {
			try {
				...
				log.info("getClient failed " + host, e);
			}
		}
		return null;
	}
	 ...
	 * @return
	 */
	protected BasePacket sendRequest(Object key, BasePacket packet,
			boolean isRead) {
		...
		try {
			...
			log.info("send request to " + client + " failed ", e);
		}
	...
	private Result<List<Object>> doMLock(int namespace,
		...
		if (sucRespSize == uniqueKeys.size()) {
			...
			if (log.isDebugEnabled()) {
				log.info(descStr + "partly success: request key size: "
		...
		if ((returnPacket != null)
			...
			if (rc.isSuccess() && entryList.size() > 0) {
				...
				try {
					...
					log.info("ITEM SERIALIZEERROR", e1);
	...
	public Map<String, String> getStat(int qtype, String groupName,
			long serverId) {
		Map<String, String> temp = configServer.retrieveStat(qtype, groupName,
				serverId);
		return temp;
	}
