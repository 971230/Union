	...
	public List<Goods> getGoodsInfo() {
		...
		for(OrderItemProdBusiRequest product : products){
			...
			if((EcsOrderConsts.GOODS_TYPE_NUM_CARD.equals(type_id) && EcsOrderConsts.PRODUCT_TYPE_CODE_OFFER.equals(product.getProd_spec_type_code())) 
				...
				if ( good.getGoodsType()!=null&&(good.getGoodsType().equals(EcsOrderConsts.ZB_PRODUCT_TYPE_SJL)
						|| good.getGoodsType().equals(EcsOrderConsts.ZB_PRODUCT_TYPE_PJL)
						|| good.getGoodsType().equals(EcsOrderConsts.ZB_PRODUCT_TYPE_SWK))) {
					String terminal_num = prodExt.getTerminal_num();
					if(StringUtils.isEmpty(terminal_num)){
						terminal_num = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.TERMINAL_NUM);
					}
