...
public class SpecUtils {
	...
	public static Map getProductSpecMapByGoodsId(String goods_id, String type_id){
		Goods product = getProductByGoodsId(goods_id,type_id);
		if(product == null)
			return new HashMap();
		Map specMap = getProductSpecMap(product);
		return specMap;
	 ...
	 * @return
	 */
	public static Map<String,String> getProductSpecsHis(String order_id, String type_id){
	 ...
	 * @return
	 */
	public static Map<String,String> getProductSpecs(String order_id, String type_id){
		Map<String,String> specs = new HashMap<String,String>();
	...
	private static Goods getProductByGoodsId(String goods_id, String type_id){
		String prod_goods_id = null;
		Goods product = null;
		List<Goods> products = getAllOrderProductsByGoodsId(goods_id);
		if(products!=null && products.size()>0){
			for(Goods prod :  products){
				if(type_id.equals(prod.getType_id())){
					product = prod;
...
//				item.getOrderItemProdBusiRequests();
	 ...
	 * @return
	 */
	public static List<Goods> getAllOrderProductsByGoodsId(String goods_id){
		List<Goods> products = new ArrayList<Goods>();
		if(!StringUtils.isEmpty(goods_id)){
			ProductsListReq req = new ProductsListReq();
			req.setGoods_id(goods_id);
		...
		if(uosFlowResp!=null){
			...
			if(null != nodes && !nodes.isEmpty()){
				for(int i = 0; i < nodes.size(); i++){
					...
					String zbTacheCode = CommonDataFactory.getInstance().getDictCodeValue(StypeConsts.ZB_FLOW_NODE, node.getTacheCode());
					if("D".equals(zbTacheCode) && isExec) //总部D发货环节、且规则匹配需要跳过，不组装发货流程
						continue;
					if(null != zbTacheCode && !EcsOrderConsts.EMPTY_STR_VALUE.equals(zbTacheCode)){
						if(orderFlow.equals(EcsOrderConsts.EMPTY_STR_VALUE)){
							orderFlow += zbTacheCode;
						}else{
							orderFlow += "-" + zbTacheCode;
						}
					}
	...
	public static String getNumberSku(String order_id,String goods_type,String specification_code,String sim_type,String white_card_type){
		...
		else if(EcsOrderConsts.GOODS_TYPE_WIFI_CARD.equals(goods_type) && EcsOrderConsts.SIM_TYPE_BK.equals(sim_type)){
			String card_name = AttrUtils.getInstance().getOtherDictCodeValueDesc(StypeConsts.BAI_CARD_SKU, white_card_type);
			Map<String, String> product = getProductByName(card_name);
			sku = product==null?EcsOrderConsts.EMPTY_STR_VALUE : product.get("skua");
		}
		else if(EcsOrderConsts.GOODS_TYPE_WIFI_CARD.equals(goods_type) && EcsOrderConsts.SIM_TYPE_CK.equals(sim_type)){
	...
	/**
	 * 生成15+位随机数，不支持15位以下随机码
	 * @param n
	 * @return
	 */
	public static String getOrderIdRandom(int n){
		String sessionId = "";
		...
		int minute = c.get(Calendar.MINUTE); 
		int second = c.get(Calendar.SECOND);
		sessionId = "51" + year  + "" + month + "" + date+ "" + hour + "" + minute + ""+ second+""+Math.round(Math.random()*(99-10));//CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.ACTIVE_NO);
		if(sessionId.length()<n){
			for(int i=0; i<sessionId.length()-n+1; i++){
				sessionId = sessionId+"0";
			}
		}else if(sessionId.length()>n){
			sessionId = sessionId.substring(sessionId.length()-n);
		}
		return sessionId;
	}
	public static String getGoodsTypeNameById(String type_id){
		String type_name = "";
		if("10000".equals(type_id)){
			type_name = "终端";
		}else if("10001".equals(type_id)){
			type_name = "合约计划";
		}else if("10002".equals(type_id)){
			type_name = "套餐";
		}else if("10003".equals(type_id)){
			type_name = "上网卡硬件";
		}else if("10004".equals(type_id)){
			type_name = "成品卡";
		}else if("10005".equals(type_id)){
			type_name = "白卡";
		}else if("10006".equals(type_id)){
			type_name = "配件";
		}else if("10007".equals(type_id)){
			type_name = "礼品";
		}else if("10008".equals(type_id)){
			type_name = "增值业务";
		}else if("10009".equals(type_id)){
			type_name = "业务包";
		}else if("10010".equals(type_id)){
			type_name = "转兑包";
		}else if("10011".equals(type_id)){
			type_name = "号码";
		}else if("10015".equals(type_id)){
			type_name = "业务受理";
		}
		return type_name;
	}
