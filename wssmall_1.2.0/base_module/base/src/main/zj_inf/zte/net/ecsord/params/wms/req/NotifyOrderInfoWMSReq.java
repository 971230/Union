...
public class NotifyOrderInfoWMSReq extends ZteRequest<NotifyOrderInfoWMSResp>{
	...
	public List<NotifyOrderInfoVo> getOrderInfo() {
		...
		String channel_id = CommonDataFactory.getInstance().getAttrFieldValue(orderId, AttrConsts.RESERVE3);//渠道信息
		String product_comp_code = CommonDataFactory.getInstance().getComp_code(channel_id);//由渠道映射货主
		...
		NotifyOrderInfoVo.setOrderModel(CommonDataFactory.getInstance().getAttrFieldValue(orderId, AttrConsts.ORDER_MODEL));//订单生产模式
		NotifyOrderInfoVo.setHsOrderType("");
		//华盛订单类型  03：退货采购单 21：调拨出库单 
		String platType = CommonDataFactory.getInstance().getOrderTree(orderId).getOrderExtBusiRequest().getPlat_type();
		if(StringUtils.equals(EcsOrderConsts.PLAT_TYPE_10061, platType)){
			List<HuashengOrderBusiRequest> huashengOrderBusiRequest = CommonDataFactory.getInstance().getOrderTree(orderId).getHuashengOrderBusiRequest();
			if(null != huashengOrderBusiRequest && huashengOrderBusiRequest.size() > 0){
				NotifyOrderInfoVo.setHsOrderType(huashengOrderBusiRequest.get(0).getZjklb());
			}
		}
		...
		if(gifts!=null && gifts.size()>0){
			for(AttrGiftInfoBusiRequest gift : gifts){
				if(EcsOrderConsts.NO_DEFAULT_VALUE.equals(gift.getIs_virtual())){//实物赠品，非实物赠品不用传给WMS
					...
					vo.setGiftComp(product_comp_code);
		...
		//B2B订单或华盛来源订单取OrderItemExtvtlBusiRequest数据同步WMS
		if(EcsOrderConsts.ORDER_TYPE_09.equals(order_type) 
				|| EcsOrderConsts.PLAT_TYPE_10061.equals(platType)){
			List<OrderItemExtvtlBusiRequest> orderItemExtvtls = CommonDataFactory.getInstance().getOrderTree(orderId).getOrderItemExtvtlBusiRequests();
			if(orderItemExtvtls!=null&&orderItemExtvtls.size()>0){
				for(OrderItemExtvtlBusiRequest orderItemExtvtl : orderItemExtvtls){
					if(EcsOrderConsts.PRODUCT_TYPE_TERMINAL.equals(orderItemExtvtl.getGoods_type())||EcsOrderConsts.PRODUCT_TYPE_ADJUNCT.equals(orderItemExtvtl.getGoods_type())){
						NotifyGoodInfoVo notifyGoodInfoVo=new NotifyGoodInfoVo();
						notifyGoodInfoVo.setOrderId(orderId);
						String goods_type_id = CommonDataFactory.getInstance().getGoodSpec(null, orderItemExtvtl.getGoods_id(), SpecConsts.TYPE_ID);
						...
						notifyGoodInfoVo.setDetailId(orderItemExtvtl.getItems_id());
						notifyGoodInfoVo.setDetailType("1");
						notifyGoodInfoVo.setPackageId(orderItemExtvtl.getGoods_id());
						notifyGoodInfoVo.setProductType("");
						notifyGoodInfoVo.setQty(1);
						notifyGoodInfoVo.setProductCode(orderItemExtvtl.getSku());
						notifyGoodInfoVo.setProductName(orderItemExtvtl.getGoods_name());
						notifyGoodInfoVo.setProductComp(product_comp_code);//货主
						notifyGoodInfoVo.setCardType(EcsOrderConsts.EMPTY_STR_VALUE);
						notifyGoodInfoVo.setNumber(EcsOrderConsts.EMPTY_STR_VALUE);
						notifyGoodInfoVo.setProductLgort("1000");
						//屏蔽无用代码 xiao.ruidan 20180518
						//华盛商品库位 ,联通商品默认1000
//						if(StringUtils.equals(EcsOrderConsts.PLAT_TYPE_10061, platType)){
//							List<HuashengOrderItemBusiRequest> huashengOrderItems = CommonDataFactory.getInstance().getOrderTree(orderId).getHuashengOrderItemBusiRequest();
//							for(HuashengOrderItemBusiRequest orderItem : huashengOrderItems){
//								if(StringUtils.equals(orderItem.getSku(), orderItemExtvtl.getSku())){
//									notifyGoodInfoVo.setProductLgort(orderItem.getLgort());
//								}
//							}
//						}
			...
			//如果是b2b业务，获取所有商品信息添加到notifyGoodInfoVos，如果不是 按之前逻辑执行
				...
				for(OrderItemProdBusiRequest prod : prods){
					...
					if(EcsOrderConsts.PRODUCT_TYPE_CODE_OFFER.equals(prod.getProd_spec_type_code())
						...
						if(EcsOrderConsts.PRODUCT_TYPE_CODE_OFFER.equals(prod.getProd_spec_type_code())){//sim卡
							if (EcsOrderConsts.IS_DEFAULT_VALUE.equals(is_old)) {
								continue;
							}
							//合约机新用户、号卡白卡需要写卡
