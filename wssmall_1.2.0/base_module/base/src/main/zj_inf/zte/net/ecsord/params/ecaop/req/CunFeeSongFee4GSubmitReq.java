 ...
 * @see 4G存费送费正式提交
 */
public class CunFeeSongFee4GSubmitReq extends ZteRequest {
	...
	private List<PayInfoVo> payInfo = new ArrayList<PayInfoVo>();
	...
	public String getCity() {
		...
		if (StringUtil.isEmpty(this.city)) {
			this.city = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.CITY);
		}
		return this.city;
	}
...
//		bean.setPayType(EcsOrderConsts.BILL_TYPE_10);//支付类型
		...
		bean.setPayFee(String.valueOf(CommonDataFactory.getInstance().getOrderTree(notNeedReqStrOrderId).getOrderBusiRequest().getOrder_amount()));
		ls.add(bean);
		return ls;
	}

	public void setPayInfo(List<PayInfoVo> payInfo) {
	...
	public String getApiMethodName() {
		return "ecaop.trades.sell.mob.oldu.fee.chg.sub";
	}
