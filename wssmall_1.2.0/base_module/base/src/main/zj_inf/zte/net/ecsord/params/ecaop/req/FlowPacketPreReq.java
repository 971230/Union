...
/**
 * songqi sp类产品订购申请 视频包：预提交
 */
public class FlowPacketPreReq extends ZteRequest {
	...
	public String getApiMethodName() {
		return "ecaop.trades.sell.mob.sp.order.app";
	}
	...
	public String getSerialNumber() {
		this.serialNumber = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.PHONE_NUM);
		// this.serialNumber = "13021050538";
		return serialNumber;
	}
	...
	public List<FlowProductInfo> getProductInfo() {// 产品
		/**
		 * 货品 附件产品 参数中 CBSS产品编码 cbss_product_code 包编码 package_code 元素编码
		 * element_code 10050 getProductSpec
		 */
		// String productId =
		...
		for (Goods p : products) {
			...
			if(StringUtils.equals(package_type, "sp") || StringUtils.isEmpty(package_type)) {
				if (specs != null && !specs.isEmpty()) {
					productId = specs.get("cbss_product_code") == null ? EcsOrderConsts.EMPTY_STR_VALUE : specs.get("cbss_product_code").toString();
				...
				element.setElementType("X");// Y 元素类型 D资费,S服务,A活动，X S服务
				packageElement.add(element);
			}
		}
		product.setProductId(productId);
		product.setOptType("00");// N 00：订购；01：退订
	...
	//	this.recomPersonId = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.DEVELOPMENT_CODE);
	      this.recomPersonId = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, "develop_code_new");
	...
	public String getRecomPersonName() {
		//this.recomPersonName = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.DEVELOPMENT_NAME);
	      this.recomPersonName = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, "develop_name_new");
