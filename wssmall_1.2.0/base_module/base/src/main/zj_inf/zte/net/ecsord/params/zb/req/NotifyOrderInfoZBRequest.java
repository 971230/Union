...
public class NotifyOrderInfoZBRequest extends ZteRequest {
	...
	public String getUpdateScope() {
		...
		if(EcsOrderConsts.IS_OLD_1.equals(user_type) && EcsOrderConsts.CERTI_TYPE_WXZ.equals(certi_type)){
			...
			UpdateScope = EcsOrderConsts.UPDATESCOPE_111111110101;
		}
		else{
			//UpdateScope = EcsOrderConsts.UPDATESCOPE_111111111111;
			UpdateScope = EcsOrderConsts.UPDATESCOPE_111111111101;
		}
		return UpdateScope;
	}
	...
	public Post getPostInfo() {
		...
		List<OrderDeliveryBusiRequest> deliveryList = CommonDataFactory.getInstance().getOrderTree(notNeedReqStrOrderId).getOrderDeliveryBusiRequests();
		if(deliveryList.size()>0){
			delivery = deliveryList.get(0);
		}
		Long province_id = delivery.getProvince_id();
		if(null==province_id){
			PostInfo.setConsiGoodsProv("");
		}else{
			PostInfo.setConsiGoodsProv(String.valueOf(province_id));
		}
		Long city_id = delivery.getCity_id();
		if(null==city_id){
			PostInfo.setConsiGoodsCity("");
		}else{
			PostInfo.setConsiGoodsCity(String.valueOf(city_id));
		}
		Long region_id = delivery.getRegion_id();
		if(null==region_id){
			PostInfo.setConsiGoodsDist("");
		}else{
			PostInfo.setConsiGoodsDist(String.valueOf(region_id));
		}
		PostInfo.setConsiName(CommonDataFactory.getInstance().getAttrFieldValue(
				notNeedReqStrOrderId, AttrConsts.SHIP_NAME));
	...
	public Pay getPayInfo() {
		...
		String pay_fin_time=CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.PAY_FIN_TIME);
		if(StringUtil.isEmpty(pay_fin_time)) //为空转换处理
			pay_fin_time = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.PAY_TIME);
...
//		PayInfo.setPayFinTime(CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.PAY_FIN_TIME));
	...
	public List<Discount> getDiscountInfo() {
		...
		for (AttrDiscountInfoBusiRequest AttrDiscountInfo : AttrDiscountInfoBusiRequests) {
			...
			if(0.0 ==AttrDiscountInfo.getDiscount_num())
				continue;
	...
	public UserInfomation getUserInfo() {
		...
		if(EcsOrderConsts.IS_OLD_1.equals(user_type) && EcsOrderConsts.CERTI_TYPE_WXZ.equals(certi_type)){
			...
			UserInfo.setSex(CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.CERTI_SEX));
			String is_examine_card = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.IS_EXAMINE_CARD);
			String checktype = "03";
			...
			UserInfo.setCheckType(checktype);
		}
		return UserInfo;
	}
...
//		String operatorID = CommonDataFactory.getInstance().getOrderTree(notNeedReqStrOrderId).getOrderExtBusiRequest().getLock_user_id();
		...
		String operatorID=orderLockBusiRequest!=null?orderLockBusiRequest.getLock_user_id():null;
...
//			String operatorName = CommonDataFactory.getInstance().getOrderTree(notNeedReqStrOrderId).getOrderExtBusiRequest().getLock_user_name();
	...
	public Flow getFlowInfo() {
		FlowInfo=new Flow();
		FlowInfo.setOperMode(EcsOrderConsts.ORDER_MODE_MAN);
