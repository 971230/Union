 ...
 *
 */
public class OldUserRenActivityReq extends ZteRequest<OldUserRenActivityResp> {
	...
	public String getApiMethodName() {
		return "ecaop.trades.sell.mob.oldu.renActivity.app";
	}
	...
	public List<ProductInfoReqVo> getProductInfo() {
		...
		vo.setProductId(new_product); // Y 产品标识
		...
		if(!new_product.equals(old_product)){
			vo.setOptType(EcsOrderConsts.AOP_OPER_TYPE_ORDER);	//变更按订购传00
		}else{
			vo.setOptType(EcsOrderConsts.AOP_OPER_TYPE_NO_CHANGE);	//不变更传03
		}
...
//		}
	...
	public List<ActivityInfoReqVo> getActivityInfo() {
		...
		List<ProdPackElementReqVo> activityEles = new ArrayList<ProdPackElementReqVo>();
		for(AttrPackageActivityBusiRequest attrPackageActivity : attrPackageActivityBusiRequest){
			ProdPackElementReqVo activityElement = new ProdPackElementReqVo();
	...
	public List<ParaVo> getPara() {
		List<ParaVo> plist = new ArrayList<ParaVo>();		
		//4G的加参数
		String net_type = CommonDataFactory.getInstance().getProductSpec(notNeedReqStrOrderId, SpecConsts.TYPE_ID_10002, null, SpecConsts.NET_TYPE);
