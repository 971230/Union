...
public class NotifyDeliveryZBRequest extends ZteRequest {
	...
	public String getExpComName() {
		...
		return AttrUtils.getInstance().getOtherDictCodeValueDesc(StypeConsts.ZB_LOGISTICS_COMPANY_CODE, logi_company_code);	    
	}
	...
	public String getExpNo() {
		String expNo = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId, AttrConsts.LOGI_NO);
		if(StringUtils.isEmpty(expNo)){
			OrderDeliveryBusiRequest request = CommonDataFactory.getInstance().getDeliveryBusiRequest(notNeedReqStrOrderId, EcsOrderConsts.LOGIS_NORMAL);
			expNo = request.getLogi_no();
		}
		return expNo;
	}
