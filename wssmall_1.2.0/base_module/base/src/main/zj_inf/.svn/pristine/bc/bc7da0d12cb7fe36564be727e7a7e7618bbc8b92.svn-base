...
public class SynchronousOrderZBRequest extends ZteRequest {
	...
	public String getConsiGoodsProv() {
		Long province_id = getOrderDeliveryBusiRequests().getProvince_id();
		if(null==province_id){
			return "";
		}else{
			return String.valueOf(province_id);
		}
	}
	
	private OrderDeliveryBusiRequest getOrderDeliveryBusiRequests(){
		...
		if(deliveryList.size()>0){
			...
			return new OrderDeliveryBusiRequest();
		}
	}
	...
	public String getConsiGoodsCity() {
		Long city_id = getOrderDeliveryBusiRequests().getCity_id();
		if(null==city_id){
			return "";
		}else{
			return String.valueOf(city_id);
		}
	}
	...
	public String getConsiGoodsDist() {
		Long region_id = getOrderDeliveryBusiRequests().getRegion_id();
		if(null==region_id){
			return "";
		}else{
			return String.valueOf(region_id);
		}
	}
	...
	public String getOrderProvince() {
		String orderProvince = CommonDataFactory.getInstance().getAttrFieldValue(notNeedReqStrOrderId,
				AttrConsts.ORDER_PROVINCE_CODE);
		if(StringUtils.isEmpty(orderProvince)){
			ICacheUtil cacheUtil = SpringContextHolder.getBean("cacheUtil");
			orderProvince = cacheUtil.getConfigInfo("PROVINCE_REGION_CODE");
		}
...
//		}
	...
	public Pay getPayInfo() {
		...
		PayInfo.setPayFinTime(pay_fin_time);
	...
	public Operator getOperatorInfo() {
		...
		for (OrderLockBusiRequest orderLockRequest : orderLockRequestList) {
			if(trace_id.equals(orderLockRequest.getTache_code()) && "1".equals(orderLockRequest.getLock_status())) {
		...
		if(StringUtils.isEmpty(operatorID)){
			...
			List<OrderLockBusiRequest> orderLockBusiRequestsList = CommonDataFactory.getInstance().getOrderTree(notNeedReqStrOrderId).getOrderLockBusiRequests();
			for(int i=0;i<orderLockBusiRequestsList.size();i++) {
				...
				if(trace_id.equals(orderLockBusiRequest.getTache_code())) {
					operatorName = orderLockBusiRequest.getLock_user_name();
				}
			}
