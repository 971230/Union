...
public class Order implements java.io.Serializable {
	...
	private List<OrderItemAddAccept> orderItemAcceptList;
	...
	@NotDbField
	public List<OrderItemAddAccept> getOrderItemAcceptList() {
		return orderItemAcceptList;
	}
	public void setOrderItemAcceptList(List<OrderItemAddAccept> orderItemAcceptList) {
		this.orderItemAcceptList = orderItemAcceptList;
	}
	@NotDbField
	...
	public String getShipStatus() {
		...
		if(ship_status==6)shipStatus="已换货";
		if(ship_status==5)shipStatus="部分换货";
...
//	public static final int ORDER_ACCEPT_FAIL = -11; // 受理失败
	...
	public String getOrderStatus() {
		...
		if(status==-2)orderStatus="已退货";
		if(status==-1)orderStatus="已退款";
		...
		if(status==7)orderStatus="已完成";
		if(status==8)orderStatus="已作废";
		if(status==99)orderStatus="异常单";
		if(status==OrderStatus.ORDER_CONFIRM_CANCEL)orderStatus="已取消";
		if(status==OrderStatus.ORDER_WAI_CONFIRM)orderStatus="待确认";
		if(status==-7)orderStatus="已换货";
		if(status==OrderStatus.RETURNED_APPLY)orderStatus="退货申请";
		if(status==OrderStatus.RETURNED_APPLY_PASS)orderStatus="待退货";//退货审核通过
		if(status==OrderStatus.EXCHANGE_APPLY)orderStatus="换货申请";
		if(status==OrderStatus.EXCHANGE_APPLY_PASS)orderStatus="待换货";//退货审核通过
