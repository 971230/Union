...
public abstract class BaseCallMarshaller implements Marshaller
{
	...
	public Calls marshallInbound(HttpServletRequest request, HttpServletResponse response) throws IOException, ServerException
    {
        ...
        for (int callNum = 0; callNum < calls.getCallCount(); callNum++)
        {
            ...
            //REASON.YEA add HashMap marsh
            reMarshal(params,inctx);
    ...
    /**
     * REASON.YEA add HashMap and ArrayList marsh
     * 网厅专用转换
     * @param params
     * @param inctx
     */
    private void reMarshal(Object[] params, InboundContext inctx) {
		if(params!=null && params.length>0){
			for (int i = 0; i < params.length; i++) {
				if(params[i].getClass().getName().equals("java.util.HashMap")){
					HashMap map = (HashMap) params[i];
					//System.out.println("map:"+map);
					Iterator it  = map.keySet().iterator();
					while(it.hasNext()){
						String key = (String) it.next();
						String val = (String) map.get(key);
						Object rel =getRealObjVal(val,inctx);
						map.put(key, rel);
					}
				}
			}
    ...
    /*{
        card={CARD_NAME:reference:c0-e3, CARD_FEE:reference:c0-e4, MOBILE_NUM:reference:c0-e5,MOBILE_NUM_CONFIRM:reference:c0-e6}, 
    	xx=[reference:c0-e11,reference:c0-e12,reference:c0-e13,reference:c0-e14], 
    	serv_no=card_info
     }
	*/
	private static Object getRealObjVal(String val, InboundContext inctx) {
		try{
			if (val == null) return val;
			if(val.indexOf("{")>-1){//HashMap conventor
				HashMap map = new HashMap();
				String values[] = val.substring(1, val.length()-1).split(",");
				for (int i = 0; i < values.length; i++) {
					String sub[] = values[i].trim().split(":reference:");
					String sub_key = sub[0];
					String sub_val = sub[1];
					InboundVariable variable = inctx.getInboundVariable(sub_val);
					String sub_obj = variable.getValue();
					map.put(sub_key, getRealObjVal(sub_obj,inctx));
				}
				return map;
			}else if(val.indexOf("[")>-1){//ArrayList conventor
				...
				for (int i = 0; i < values.length; i++) {
					...
					InboundVariable variable = inctx.getInboundVariable(sub_val);
					String sub_obj = variable.getValue();
					ls.add(getRealObjVal(sub_obj,inctx));
				}
				return ls;
			}else{//中文字符集转换
			...
			log.error("val decode exception："+val);
		}
		return val;
	}
	/**
