...
import com.ztesoft.net.app.base.core.model.UpdateLog;
...
import com.ztesoft.net.eop.processor.core.RemoteRequest;
...
import com.ztesoft.net.framework.util.XMLUtil;
...
public class UpdateManager2 implements IUpdateManager {
	...
	private void exeUpdate(String appid,String version) {
		...
		if (new File(updateSql).exists()) {
			...
			String content = FileBaseUtil.read(updateSql, "UTF-8");
		...
		if (new File(cmsUpdateSql).exists()) {
			...
			String content = FileBaseUtil.read(cmsUpdateSql, "UTF-8");
		...
		FileBaseUtil.copyFolder(patchPath + "/web", EopSetting.EOP_PATH);
		if(logger.isDebugEnabled()){
	...
	private void updateVersion(String version){
		...
		try {
			...
			if(null == config){
				path = StringUtil.getRootPath("eop.properties");
			}else{
				path = config + "eop.properties";
			}
			File file  = new File(path);		
		 
			//更新eop.properties
			EopSetting.VERSION=version;
			InputStream in  = new FileInputStream(file) ;//FileBaseUtil.getResourceAsStream("eop.properties");
			Properties props = new Properties();			
			props.load(in);
			props.setProperty("version", version);
			FileOutputStream os=new FileOutputStream(file);
			props.store(os, "eop.properties");
			if(logger.isDebugEnabled()){
			...
			logger.info("更新eop.properties出错",e.fillInStackTrace());
			e.printStackTrace();
		}
	...
	private void download(String appid,String version) {
		...
		try{
			...
			this.logger.info("下载升级包["+remoteZip+"]出错");
			e.printStackTrace();
	...
	private void unZip(String appid,String version){
		...
		FileBaseUtil.unZip(zipPath, EopSetting.EOP_PATH+"/patch/"+appid+"/"+version, true);
		if(this.logger.isDebugEnabled()){
