 ...
 * @param <T>
 */
public class JdbcDaoSupportWork<T> implements IDaoSupport<T> {

	private JdbcTemplate jdbcTemplateWork;
	private SimpleJdbcTemplate simpleJdbcTemplateWork;
	...
	public Connection getConnection(){
		Connection conn = DataSourceUtils.getConnection(this.jdbcTemplateWork.getDataSource());
		return conn;
	}
	@Override
	public void closeConnection(Connection conn){
		try {
			DataSourceUtils.doReleaseConnection(conn, this.jdbcTemplateWork.getDataSource());
	...
	public void execute(String sql, Object... args) {
		try {
			...
			this.simpleJdbcTemplateWork.update(sql, args);
		} catch (Exception e) {
	...
	public void batchExecute(String sql, List<Object[]> batchArgs){
		try {
			if(null != batchArgs && !batchArgs.isEmpty()){
				...
				this.simpleJdbcTemplateWork.batchUpdate(sql, batchArgs);
			}else {
	...
	public void batchExecuteByListMap(String sql, List<Map> batchArgs){
		try {
			...
			this.simpleJdbcTemplateWork.batchUpdate(sql, batch);
		} catch (Exception e) {
	...
	public String getSeqByTableName(String tableName) {
		...
		try {
			seq_name = this.simpleJdbcTemplateWork.queryForObject(sql,
	...
	public String getLastId(String table) {
		if (EopSetting.DBTYPE.equals("1")) {
			return this.jdbcTemplateWork.queryForInt("SELECT last_insert_id() as id") + "";
		} else if (EopSetting.DBTYPE.equals("2")) {
			int result = 0;
			result = this.jdbcTemplateWork.queryForInt("SELECT s_" + table
					+ ".currval as id from DUAL");
			return result + "";
		} else if (EopSetting.DBTYPE.equals("3")) {
			int result = 0;
			result = this.jdbcTemplateWork.queryForInt("select @@identity");
			return result + "";
		}
	...
	public void insert(String table, Map<String, Object> fields, Object... args) { //
		...
		//oracle数据插入
				...
				try {
						...
						jdbcTemplateWork.update(sql, values);
				} catch (Exception e) {
						e.printStackTrace();
						throw new DBRuntimeException(e, sql);
				}
		...
		TaskThreadPool taskThreadPool = new TaskThreadPool(new Task(inst) {
			...
			public ZteResponse execute(ZteRequest inst) {
				try {
					...
					//旧数据归档
					jdbcTemplateWork.execute(updateSql);
					jdbcTemplateWork.execute(deleteSql);
					//插入新数据
	...
	private String setPKValue(String table, Map<String, Object> fields) {
		...
		//if ("2".equals(EopSetting.DBTYPE)) {
			...
			try {
				...
				List keyList = this.simpleJdbcTemplateWork.queryForList(sql, table.toUpperCase());
...
//					this.getLastId(table);
	...
	public int queryForInt(String sql, Object... args) {
		try {
			...
			return this.simpleJdbcTemplateWork.queryForInt(sql, args);
	...
	public String queryForString(String sql) {
		...
		try {
			sql = this.addSourceFrom(sql);
			s  = this.jdbcTemplateWork.queryForObject(sql, String.class);
		} catch (RuntimeException e) {
//			e.printStackTrace();
		}
		return s;
	...
	public String queryForString(String sql,Object...args) {
        ...
        try {
        	sql = this.addSourceFrom(sql);
            s  = this.jdbcTemplateWork.queryForObject(sql, String.class,args);
        } catch (RuntimeException e) {
			e.printStackTrace();
        }
        return s;
	...
	public List queryForList(String sql, Object... args) {
		sql = this.addSourceFrom(sql);
		List<Map<String, Object>> list = this.jdbcTemplateWork.queryForList(sql, args);
    ...
    public List queryForListNoSourceFrom(String sql, Object... args) {
        List<Map<String, Object>> list = this.simpleJdbcTemplateWork.queryForList(sql, args);
        List<Map> mysqlMap = new ArrayList<Map>();
        for(Map m:list){
            Iterator it = m.keySet().iterator();
            Map newMap = new HashMap();
	...
	public List queryForLists(String sql) {
		sql = this.addSourceFrom(sql);
		return jdbcTemplateWork.queryForList(sql);
	}
	
	@Override
	public List queryForLists(String sql,T t) {
		sql = this.addSourceFrom(sql);
		return jdbcTemplateWork.queryForList(sql,t.getClass());
	}
	...
	public List queryForListByMap(String sql, Map args) {
		sql = this.addSourceFrom(sql);
		return this.simpleJdbcTemplateWork.queryForList(sql, args);
	}
	...
	public List<T> queryForList(String sql, RowMapper mapper, Object... args) {
		try {
			sql = this.addSourceFrom(sql);
			return this.jdbcTemplateWork.query(sql, args, mapper);
		} catch (Exception ex) {
			ex.printStackTrace();
...
//		this.jdbcTemplateGProd.queryForList(sql, elementType, args)
		sql = this.addSourceFrom(sql);
		return this.simpleJdbcTemplateWork.query(sql,
	...
	public long queryForLong(String sql, Object... args) {
		sql = this.addSourceFrom(sql);
		return this.jdbcTemplateWork.queryForLong(sql, args);
	}
	...
	public Map queryForMap(String sql, Object... args) {
		...
		try {
			Map map = this.jdbcTemplateWork.queryForMap(sql, args);
	...
	public T queryForObject(String sql, Class clazz, Object... args) {
		try {
			sql = this.addSourceFrom(sql);
			return (T) simpleJdbcTemplateWork
	...
	public int update(String table, Map fields, String where,Object ...args) {
		...
		try {
			...
			sql = "UPDATE " + table + " SET " + StringUtil.implode(", ", cols)
					+ " WHERE " + where;
			result = simpleJdbcTemplateWork.update(sql, values);
		} catch (Exception e) {
			e.printStackTrace();
			throw new DBRuntimeException(e, sql);
		}
	...
	public void setJdbcTemplateWork(JdbcTemplate jdbcTemplateWork) {
		this.jdbcTemplateWork = jdbcTemplateWork;
	}

	public void setSimpleJdbcTemplateWork(
			SimpleJdbcTemplate simpleJdbcTemplateWork) {
		this.simpleJdbcTemplateWork = simpleJdbcTemplateWork;
	}
	...
	public T queryForObject(String sql, ParameterizedRowMapper mapper,
		...
		try {
			sql = this.addSourceFrom(sql);
			T t = (T) this.simpleJdbcTemplateWork.queryForObject(sql, mapper, args);
			return t;
		} catch (RuntimeException e) {
			return null;
		}
	...
	public String getSequenceLen(String strSeqName,String strSeqType,int intSeqLen){
		...
		if(EopSetting.DBTYPE.equals(DBTUtil.DB_TYPE_MYSQL)){ //add by wui mysql转换
			...
			String ret = this.simpleJdbcTemplateWork.queryForObject(sql, String.class);
	...
	public int nextVal(String seq_name) {
		...
		return this.simpleJdbcTemplateWork.queryForInt(sql, new HashMap());
	}
	...
	public String queryForSingleResult(String sql, String key, Object... args) {
		...
		sql = this.addSourceFrom(sql);
		try {
			 map = jdbcTemplateWork.queryForMap(sql,args);
		} catch (Exception e) {
			return "";
		}
		String value ="";
	...
	public String executeProc(CallableStatementCreator callableStatementCreator,
		...
		// TODO Auto-generated method stub
		return jdbcTemplateWork.execute(callableStatementCreator, callableStatementCallback);
	}
	@Override
	...
	public void save(String table,String operate,Object pojo,String[] conditionCol) throws Exception{
		...
		this.jdbcTemplateWork.update(builder.toString(), args);
	}
	...
	public List queryForMapList(String tableOrSql, Object pojo,
		...
		List<Map<String, Object>> list = this.jdbcTemplateWork.queryForList(baseSql);
	...
	public List<T> queryForListWithoutSourceFrom(String sql, Class clazz, Object... args) {
		return this.jdbcTemplateWork.query(sql,
				ParameterizedBeanPropertyRowMapper.newInstance(clazz), args);
	}
	...
	public int queryForIntWithoutSourceFrom(String sql, Object... args) {
		try {
			return this.jdbcTemplateWork.queryForInt(sql, args);
		} catch (RuntimeException e) {
			this.logger.info(e.getMessage(), e);
			throw e;
		}
