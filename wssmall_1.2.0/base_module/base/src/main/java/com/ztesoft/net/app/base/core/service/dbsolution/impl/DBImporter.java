package com.ztesoft.net.app.base.core.service.dbsolution.impl;
...
public class DBImporter extends DBPorter {
	private static Logger logger = Logger.getLogger(DBImporter.class);
	...
	private boolean doInsert(Element action) {
		...
		try {
			if (solution.beforeInsert(table, fields, values)) {
				...
				logger.info("beforeInsert返回false，insert被阻止！");
				return false;
			}
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("出错语句：" + sql);
			logger.info("出错值：" + values);
			return false;
		}
		return true;
	}
	...
	public boolean doImport(String xml) {
		...
		try {
			if (xml.startsWith("file:")) {
				xml = FileBaseUtil.readFile(xml.replaceAll("file:", ""));
