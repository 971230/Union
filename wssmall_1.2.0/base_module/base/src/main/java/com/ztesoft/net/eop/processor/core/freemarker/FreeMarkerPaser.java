package com.ztesoft.net.eop.processor.core.freemarker;
...
//		log.info(fmp);
  ...
  public String proessShopPageContent(){
		try {
			String content = "";
			...
			if(false)
			{
				...
				String filePath = "/com/ztesoft/net/cms/base/widget/web/"+name + pageExt;
				InputStream stream = null;
				try {
					ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
					if (classLoader != null) 
						stream = classLoader.getResourceAsStream(filePath);
				        content = FileBaseUtil.readStreamToString(stream);
				        content = content.replaceAll("\\$\\{staticserver\\}",EopSetting.IMG_SERVER_DOMAIN);
				} catch (Exception e) {
					e.printStackTrace();
				}finally{
					if(null != stream)
...
//				log.info(content);
	...
	private Configuration getCfg(){
			...
			if(pageFolder==null) {//默认使用挂件所在文件夹
				...
				//log.info(" folder not null use "+ pageFolder);
