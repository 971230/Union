...
import com.ztesoft.net.eop.resource.*;
import com.ztesoft.net.eop.resource.model.*;
...
public class SiteManagerImpl implements ISiteManager {
	...
	public String addDomain(EopSiteDomain eopSiteDomain) {
	...
	@Override
	public String add(EopUser user, EopSite site, String domain) {
		...
		site.setCreatetime(DBTUtil.current());
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public String add(EopSite site, String domain) {
	...
	public void delete(String id) {
		...
		String userStyleFile = EopSetting.IMG_SERVER_PATH + "/user/" + userid
				+ "/" + siteid;
		FileBaseUtil.delete(userTplFile);
		FileBaseUtil.delete(userStyleFile);
	}
	...
	private static class DnsMapper implements ParameterizedRowMapper<Dns> {
		...
		public Dns mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			site.setAdminthemeid(rs.getString("adminthemeid"));
			site.setThemeid(rs.getString("themeid"));
			site.setSitename(rs.getString("sitename"));
			site.setUserid(rs.getString("userid"));
			...
			site.setCreatetime(rs.getString("createtime"));
	...
	@Override
	public EopSite getById(String id) {
	...
	@Override
	public EopSite getByDomain(String domain) {

		String sql = "select distinct a.* from eop_site a join eop_sitedomain b on b.siteid = a.id and b.domain=? and a.source_from is not null ";
        ...
        for(EopSite obj: sitelist){
           if(obj.getSource_from().equals(EopSetting.SOURCE_FROM)){
               site=obj;
               break;
           }
        }
	...
	public Page list(int pageNo, int pageSize, String order, String search) {
		...
		List<Map> listsite = (page.getResult());
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public void updatePoint(String id, int point,int sitestate) {
	 ...
	 */
	@Override
	public int getPoint(String id, int point) {
		EopSite site = EopContext.getContext().getCurrentSite();
	...
	public void initData() {
		...
		if (file.exists()) {
			String content = FileBaseUtil.read(sqlPath, "UTF-8");
	...
	public List<Map> list(String userid) {
		...
		for (Map site : list) {
			List<EopSiteDomain> domainList = this.domainManager
					.listSiteDomain(site.get("id").toString());
			site.put("eopSiteDomainList", domainList);
		}
		return list;
	}
