...
public class ConnectionPool {
	...
	/**
     * 初始化函数 使用给定的名称
     * @param dsName String dsName 数据源名称
     * @throws DAOSysException 如果不能得到数据源就抛出该异常
     */
    private void initialize(String dsName) {
        try {
        	dsName="dataSource";
            datasource = DataSourceHelper.singleton().getDataSource(dsName);
        }
	...
	public ConnectionPool(){
		initialize(JNDINames.DEFAULT_DATASOURCE);
		this.setTestTable("dual");
	}
	...
	private void createConnections(int numConnections) throws SQLException {
		...
		for (int x = 0; x < numConnections; x++) {
			...
			if (this.maxConnections > 0
					&& ConnectionPool.connections.size() >= this.maxConnections) {
...
//				dbPassword);
	 ...
	 * @throws SQLException
	 */
	public synchronized Connection getTransConnection() throws SQLException {
		...
		ConnectionContext.getContext().setConnection(conn);
		return conn;// 返回获得的可用的连接
	 ...
	 * @throws SQLException
	 */
	public synchronized void batchCommitConnections() throws Exception{
		// 确保连接池己创新存在

		if (connections == null) {

			logger.info(" 连接池不存在，批量提交 !");
		...
		while (enumerate.hasMoreElements()) {
			...
			// 关闭此连接，用一个新的连接代替它。
			commitConnection(pConn.getConnection());
	...
	public synchronized void closeAllConnection()throws Exception{
		if (connections == null) {
...
//			commitConnection(pConn.getConnection());
	 ...
	 * @throws SQLException
	 */
	public synchronized void commitConnection(Connection conn) throws SQLException{
		conn.commit();
	}
	 ...
	 * @throws SQLException
	 */
	public synchronized void batchRollBack() throws Exception{
		if (connections == null) {

			logger.info(" 连接池不存在，提交 !");
...
//			connections.removeElement(pConn);
	 ...
	 * @throws SQLException
	 */
	public synchronized void rollBack(Connection conn) throws SQLException{
		conn.rollback();
	}
