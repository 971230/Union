...
import com.ztesoft.net.app.base.core.service.solution.ISolutionExporter;
	...
	public void export(String name, boolean exportData, boolean exportTheme,
		...
		try {
			...
			if(exportData){
				...
				xmlFile.append("</dbsolution>");
				if("1".equals(EopSetting.DBTYPE)){
					FileBaseUtil.write(temppath +"/example_data_mysql.sql", sqlContent.toString());
				}
				setupCreator.save(setup, temppath + "/setup.xml");
				FileBaseUtil.write(temppath + "/example_data.xml", xmlFile.toString());
			}
			...
			if(exportTheme){
				...
				if("1".equals( EopSetting.RESOURCEMODE )){ //静态资源分离
					FileBaseUtil.copyFolder(EopSetting.EOP_PATH+EopContext.getContext().getContextPath() +"/themes", temppath+"/themes");
					FileBaseUtil.copyFolder(EopSetting.IMG_SERVER_PATH+EopContext.getContext().getContextPath() +"/themes", temppath+"/themes");
				}
			...
			if(exportAttr){
				//拷贝附件
				FileBaseUtil.copyFolder(EopSetting.IMG_SERVER_PATH+EopContext.getContext().getContextPath()+"/attachment", temppath+"/attachment");				
			}
			...
			delete.execute();
			 ...
			 e.printStackTrace();
			 this.logger.info("导出解决方案出错",e.fillInStackTrace());
