...
public class DataCatManager extends BaseSupport<DataCat> implements
		com.ztesoft.net.cms.core.service.IDataCatManager {
...
//	      判断文章分类不能同名
	...
	public DataCat get(String catid,String source_from) {
		...
		return this.baseDaoSupport.queryForObject(
				"select * from data_cat where cat_id=? and source_from=?",
				DataCat.class, catid, source_from);
	}
	...
	public List<DataCat> listAllChildren(String parentid) {
		...
		String sql = "select * from  data_cat  order by source_from,parent_id,cat_order" ;
		...
		for(DataCat cat :allCatList){
			if(cat.getParent_id().equals(parentid)){
				if(this.logger.isDebugEnabled()){
	...
	private List<DataCat> getChildren(List<DataCat> catList ,String parentid){
		if(this.logger.isDebugEnabled()){
			this.logger.debug("查找["+parentid+"]的子");
		}
	...
	public List<DataCat> getParents(String catid,String source_from) {
		DataCat cat  = this.get(catid,source_from);
		String path = cat.getCat_path();
		path  = path.replaceAll("\\|", ",")+"-1";
		return this.baseDaoSupport.queryForList("select * from data_cat where cat_id in("+ path+") and source_from='"+source_from+"'" , DataCat.class);
	}
