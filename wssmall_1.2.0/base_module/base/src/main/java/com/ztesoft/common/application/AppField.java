...
private static Logger logger = Logger.getLogger(AppField.class);
...
public void loadFromHashMap(HashMap vo){
	if(vo!=null && !vo.isEmpty()){
		hsFieldsInfo.put(this.CLASS_NAME, vo.get("clssName"));
		hsFieldsInfo.put(this.FIELD_NAME, vo.get("fieldName"));
		hsFieldsInfo.put(this.FIELD_TYPE, vo.get("fieldType"));
		hsFieldsInfo.put(this.FIELD_PRIVILEGE, vo.get("fieldPrivilege"));
		hsFieldsInfo.put(this.FIELD_LENGTH, vo.get("fieldLength"));
		if(((String)vo.get("defaultValue")).equals("null"))
			hsFieldsInfo.put(this.DEFAULT_VALUE, null);
		else
			hsFieldsInfo.put(this.DEFAULT_VALUE, vo.get("defaultValue"));
	}
}
