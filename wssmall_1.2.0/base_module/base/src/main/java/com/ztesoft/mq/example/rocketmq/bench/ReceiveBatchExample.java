...
public class ReceiveBatchExample implements EventBatchListenHandler {
	private static Logger logger = Logger.getLogger(ReceiveBatchExample.class);
	final static StatsBenchmarkConsumerBatch statsBenchmarkConsumer = new StatsBenchmarkConsumerBatch();
	
	@Override
	public void executeEvent(List<EventMessageExt> es)
			throws EventHandleException {
		// TODO Auto-generated method stub
		EventMessageExt e = es.get(0);
        ...
        // 2
        MessageExt message = (MessageExt)e.getProperty("messageExt");
        long born2ConsumerRT = now - message.getBornTimestamp();
        statsBenchmarkConsumer.getBorn2ConsumerTotalRT().addAndGet(born2ConsumerRT);

        // 3
        long store2ConsumerRT = now - message.getStoreTimestamp();
	...
	public static void main(String[] args) {		
        ...
        BenchExample example = new BenchExample();
		example.initReceiveBatch();
...
class StatsBenchmarkConsumerBatch {
    // 1
