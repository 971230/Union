...
public class AnnotationTest {
	 ...
	 * @param hm
	 */
	public void combinationMethodData(String className, String methodname, HashMap hm){
		try {
			Class clazz = Class.forName(className);
					...
					if(mzca != null && mzca.isOpen()){
						...
						if(methodname.equals(method.getName())){//遍历入参 出参
							...
							for(Class<?> c : ps){
								  ...
								  //增加getApiMethodName
								  try {
									  Method m = c.getMethod("getApiMethodName", null);
									  if(m!=null && c.getName().indexOf("zte.params")>=0){
										  Object obj = c.newInstance();
										  String returnValue = (String) m.invoke(obj, null);
										  HashMap methodHM = new HashMap();
										  methodHM.put("name", "API方法标识");
										  methodHM.put("fieldname", returnValue);
										  methodHM.put("type","String");
										  methodHM.put("isnecessary", "");
										  methodHM.put("desc", "ApiMethodName，在注册中心中唯一");
										  methodPList.add(methodHM);	
									  }
								  } catch (Exception e) {
									  e.printStackTrace();
	 ...
	 * 类所有方法遍历
	 */
	public void methodTraversal(String classname){
		try {
			Class clazz = Class.forName(classname);
			...
			if(methods.length>0){
				for(Method method : methods){
					...
					logger.info("00000:"+method.getName());
					...
					if(((ArrayList)hm.get("methodlist")).size()>0){//有注解方法时生成明细
						...
						jh.setOutFileName(AnnotationTest.path+"//apidoc//"+clazz.getName()+"."+method.getName()+".html");
						jh.setDataMap(hm);
						jh.createHtml();					
					}
     ...
     */  
    private void getClassNameByFile(String filePath, List<HashMap<String,String>> className, String packageName, boolean childPackage){  
    ...
    public void memberVarList(String classname){
		try {
			Class clazz = Class.forName(classname);
			...
			jh.setOutFileName(AnnotationTest.path+"//apidoc//"+clazz.getName()+".html");
			jh.setDataMap(hm);
			jh.createHtml();	
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
     ...
     */
    public void f1(String classname){
//		String classname = "com.ztesoft.net.annotation.Unit";
		...
		if(((ArrayList)hm.get("methodlist")).size()>0){//有注解方法时生成明细
			jh.setTemplateFileName("api_cat_detail.html");
			jh.setOutFileName(AnnotationTest.path+"//apidoc//adfadfadsfsadfadsf.html");
			jh.setDataMap(hm);
			jh.createHtml();					
		} 	
    }
    ...
    public void f2(String path){
    	...
    	while(ite.hasNext()){
        	...
        	String filePath = EopSetting.EOP_PATH.substring(0, EopSetting.EOP_PATH.indexOf("mgWeb")-1)+"//sdk//src//main//"+packageName+"//";//包路径前缀
    		...
    		if(i==0){//为了点击“返回”，将第一项固化
    			jh.setOutFileName(AnnotationTest.path+"//apidoc//index.html");
    		}else{
    			jh.setOutFileName(AnnotationTest.path+"//apidoc//"+packageName+"_"+"api_list.html");
    		}
    		i++;
    		jh.setDataMap(hm);
