...
public class IncrAttrTableTag extends TagSupport {
	  /**
	   * Logger for this class
	   */
	  private static final Logger logger = Logger.getLogger(IncrAttrTableTag.class);
	    ...
	    if (ds != null && ds instanceof DatasetVo) {
	      ...
	      if (datasetVo != null) {
	        ...
	        if (fieldVos != null) {
	          ...
	          sbuffer.append("<table id='" + this.getId() + "' dataset='" + this.getDataset() +
	                         "' class='datatable' initialized='true' highlightSelection='" +
	                         this.getHighlightSelection() + "' ");
	          sbuffer.append(
	            "  style='width:100%;' cellspacing='0' cellpadding='2' border='0' rules='all' ");
	          ...
	          if ("true".equalsIgnoreCase(this.getShowHeader())) {
	            ...
	            for (int i = 0; i < fieldVos.size(); i++) {
	              ...
	              sbuffer.append("<td id='" + this.getDataset() + "_head_" + fieldVo.getField() +
	                             "' class='columnheader' extra='columnheader' name='" +
	                             fieldVo.getField() + "' field='" + fieldVo.getField() + "' sortable='" +
	                             fieldVo.getSortable() + "' titlt='" + fieldVo.getToolTip() + "'");
	              sbuffer.append(" nowrap align='middle'");
	              if (isFirst) {
	                sbuffer.append(" style='border-left:0px;'");
	                isFirst = false;
	              }
	              if(fieldVo.getField().equals("attr_name"))
		            	sbuffer.append(" width='20%' ");
		            else
		            	sbuffer.append(" width='80%' ");
	              sbuffer.append(">");
	          ...
	          if ("true".equalsIgnoreCase(this.getShowFooter())) {
	            ...
	            for (int i = 0; i < fieldVos.size(); i++) {
	              ...
	              sbuffer.append("<td id='" + this.getDataset() + "_footer_" + fieldVo.getField() +
	                             "' class='columnfooter' extra='columnfooter' name='" +
	                             fieldVo.getField() + "' field='" + fieldVo.getField() + "'");
	          ...
	          if (_showIndicator) {
	            sbuffer.append(
	              "<td class='indicate' align='middle' width='9' isIndicate='true'>&nbsp;</td>");
	          }
	          boolean isFirst = true;
	          for (int i = 0; i < fieldVos.size(); i++) {
	            FieldVo fieldVo = ( (FieldVo) fieldVos.get(i));
	            ...
	            sbuffer.append("<td  id='" + this.getId() + "_" + fieldVo.getField() +
	                           "' class='tablecell' extra='tablecell' name='" + fieldVo.getField() +
	                           "' field='" + fieldVo.getField() + "' editable='" + fieldVo.getEditable() +
	                           "' readOnly='" + fieldVo.getReadOnly() + "' dropDown='" +
	                           fieldVo.getDropDown() + "' dataType='string'");
	            ...
	            if(fieldVo.getField().equals("attr_name"))
	            	sbuffer.append(" width='20%' ");
/*	            else if(fieldVo.getField().equals("invalidMsg"))
	            	sbuffer.append(" width='30%' ");*/
	            else
	            	sbuffer.append(" width='80%' ");
	            sbuffer.append(">");
	            ...
	            if ("select".equalsIgnoreCase(fieldVo.getField())) {
	              sbuffer.append(
	                "<input style='height:16px' onclick='return TableCheckbox.onclick();' type='checkbox'>");
	            }
