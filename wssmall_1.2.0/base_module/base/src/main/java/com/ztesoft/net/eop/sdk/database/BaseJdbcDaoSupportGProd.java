...
import com.ztesoft.net.framework.database.impl.JdbcDaoSupportGProd;
 ...
 * @param <T>
 */
public class BaseJdbcDaoSupportGProd<T> extends JdbcDaoSupportGProd<T> {
	private IDBRouter dbRouterGProd;

	public void setDbRouterGProd(IDBRouter dbRouterGProd) {
		this.dbRouterGProd = dbRouterGProd;
	}
	...
	public void insert(String table, Object po) {
		...
		table = this.dbRouterGProd.getTableName(table);
...
//					+ " $3");
	...
	public String rpJoinTbName(String sql) {
		...
		if (m.find()) {
			...
			m.appendReplacement(sb, "join " + this.dbRouterGProd.getTableName(tname)
					+ " ");
		}
		m.appendTail(sb);
	...
	public String rpFromTbName(String sql) {
		...
		if (m.find()) {
			...
			m.appendReplacement(sb, " from " + this.dbRouterGProd.getTableName(tname)
					+ " ");
		}
		m.appendTail(sb);
		return sb.toString();
