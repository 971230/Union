...
public class AccessExporter implements   Runnable  {
	private static Logger logger = Logger.getLogger(AccessExporter.class);
	...
	public void run() { 
		...
		InputStream in =FileBaseUtil.getResourceAsStream("com/enation/eop/resource/access.xls");
		...
		if(list!=null && !list.isEmpty()){
			...
			for(Access access :list){
				...
				excelUtil.writeStringToCell(i, 4, access.getAccess_time()); //访问日期
	...
	public static void main(String[] args){
		...
		logger.info( DateUtil.toDate(date, "yyyy-MM-dd") .getTime() /1000);
	}
