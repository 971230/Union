...
public class XSSSecurityManager {
    ...
    public static void init(FilterConfig config){
    	...
    	//初始化过滤配置文件
        String xssPath = "/com/ztesoft/ibss/security/xss/xss_security_config.xml";
        ...
        try {
			...
			e.printStackTrace() ;
			logger.info("安全过滤配置文件xss_security_config.xml加载异常",e);
		}
	...
	public static boolean initConfig(String path) throws DocumentException {
		...
		if(regexEle != null){
	        ...
	        if(tempStr.charAt(tempStr.length()-1)=='|'){
	        	...
	        	logger.info("安全过滤配置文件加载失败:正则表达式异常 "+tempStr.toString());
	        	return false;
	        }
		}else{
			logger.info("安全过滤配置文件中没有 "+XSSSecurityCon.REGEX_LIST+" 属性");
			return false;
		}
