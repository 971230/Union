...
import com.ztesoft.net.eop.resource.model.Dns;
...
public class SiteCacheProxy extends AbstractCacheProxy<EopSite> implements ISiteManager {
	...
	public String addDomain(EopSiteDomain eopSiteDomain) {
		EopSite site  =siteManager.getById(eopSiteDomain.getSiteid());
	...
	@Override
	public String add(EopSite newSite,String domain) {
	...
	public void editDomain(String domainOld, String domainNew) {
		EopSite site  =siteManager.getByDomain(domainOld);
	...
	public EopSite getByDomain(String domain) {
		...
		if(site  == null){
			...
			site  =  this.siteManager.getByDomain(domain);
		...
		if(logger.isDebugEnabled()){
			logger.debug("finded and siteid is : "+ site.getId()+","+site.getSitename());
		}
...
//				site.put("logofile", logofile);
	...
	public void edit(EopSite eopSite) {
		...
		EopSite site = siteManager.getById(eopSite.getId());
		refreshCache(site);
