...
public class SmartUpload
{
    ...
    //考虑不同的class文件存放的目录不一样而增加的方法
    //获得指定顺序文件的扩展名
    public String getFileExt(int fileIndex){
      return m_files.getFile(fileIndex).getFileExt();
      }
    //获得文件内容的起始
   public int getFileStart(int fileIndex){
    return m_files.getFile(fileIndex).getStartData();
    }
    public int getFileSize (int fileIndex){
      return m_files.getFile(fileIndex).getSize();
    }
    public String getFileName(int fileIndex)
    {
      return m_files.getFile(fileIndex).getFileName();
    }
        ...
        throws SmartUploadException, IOException, ServletException
    {
        ...
        //下面为测试代码
//        FileInputStream fs = new FileInputStream("c:\\test.in");
//        byte readArray[] = new byte[10240];
//        ByteArrayOutputStream os = new ByteArrayOutputStream();
//        int iReadSize =0;
//        m_totalBytes =0;
//        iReadSize =fs.read(readArray);
//       
//        while(iReadSize!=-1)
//        {
//            os.write(readArray,m_totalBytes,iReadSize);
//            m_totalBytes += iReadSize;
//            iReadSize =fs.read(readArray);
//        }
//        m_binArray = os.toByteArray();
//        os.close();
...
//        FileOutputStream fs = new FileOutputStream("c:\\test.in");
//        try{
//            fs.write(m_binArray);
//            fs.flush();
//        }finally{
