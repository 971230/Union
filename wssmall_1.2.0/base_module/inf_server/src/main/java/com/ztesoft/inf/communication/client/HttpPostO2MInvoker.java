...
public class HttpPostO2MInvoker extends Invoker{
...
//		String url = "http://10.123.98.57:7001/integrationweb/integration/ordersStatus.sync" + "?result=" + StringUtil.md5(md5Str);
		...
		try {
			...
			byte[] bytes = mapToXml((Map)param.get("MSGPKG")).getBytes("utf-8");
			RequestEntity requestEntity = new ByteArrayRequestEntity(bytes, "utf-8");
			postMethod.setRequestEntity(requestEntity);
            //postMethod.addParameter("xml", mapToXml((Map)param.get("MSGPKG")));
            
    		context.setRequestString(mapToXml((Map)param.get("MSGPKG")));
            ...
            if(resContent.contains("<RES_CODE>00000</RES_CODE>")){
    		...
    		Map respMap = XmlUtils.xmlToMap(resultStr);
    		String cdataContent = null;
    		
    	    context.setResultString(resultStr);
			context.setResponeString(resultStr);
			
			Map new_respMap = new HashMap();
			new_respMap.put("MSGPKG", respMap);
    		try {
    			Class<?> clazz = Class.forName(rspPath);
    			String jsonString = JSON.toJSONString(new_respMap);
//    			String jsonString = Xml2JsonUtil.xml2JSON(resultStr);
    			resp = (ZteResponse) JsonUtil.fromJson(jsonString, clazz);
    			
    			String error_code =Const.getStrValue((Map)respMap.get("PKG_HEAD"), "RES_CODE");
    			String error_msg = Const.getStrValue((Map)respMap.get("PKG_HEAD"), "RES_MSG");
    			if(com.ztesoft.form.util.StringUtil.isEmpty(error_code) && resContent.contains("<res_code>30029</res_code>")){
    					error_code = respMap.get("res_code").toString();
        				error_msg = respMap.get("res_message").toString();
    			}
	...
	public Object invokeTest(InvokeContext context) throws Exception {
		...
		try {
    		...
    		if(resultStr.indexOf("<Body>")>=0){
        		...
        		try {
        			document= DocumentHelper.parseText(cdataContent);
	...
	public static String mapToXml(Map map) {
		...
		Element root = null;
		root = document.addElement("MSGPKG");
