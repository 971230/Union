...
public class OperationBO extends BaseSupport implements IOperationBO{
	...
	public Operation getOperationByCode(String operationCode)
		...
		Map map = this.baseDaoSupport.queryForMap(OPERATION, operationCode);
		if (map == null || map.isEmpty())
			return null;
		Operation operation = new Operation(map);
		String[] params = new String[] { operationCode, "IN" };
		List maps = this.baseDaoSupport.queryForList(OP_ARGS, params);
		...
		map = this.baseDaoSupport.queryForMap(OP_ARGS, params);
		if (map == null || map.isEmpty()) {
