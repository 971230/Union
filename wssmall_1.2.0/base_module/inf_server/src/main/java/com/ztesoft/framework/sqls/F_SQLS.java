...
//				Sqls f_sql_local = (Sqls) f_sql_local_class.newInstance();
				...
				Sqls f_sql_local = PlatService.getPlatClass(sqlsManager.getClass());
	...
	public static final	String INSERT_CLIENT_CALLLOG_NO_CLOB = "insert into inf_comm_client_calllog( req_time,"
			+ " rsp_time, op_code, ep_address,state, state_msg,param_desc, result_desc,"
			+ "col1,col2,col3,col4,col5,log_id) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static final String SELECT_REMOTE_SERVICE = "select service_id serviceId,service_code servicecode,op_code opCode,api_name apiName,api_param apiParam from es_remote_service t where t.status = 1";
	
	public static final String UPDATE_INF_CALL_LOG = " update inf_comm_client_calllog a set a.result_desc = ?, a.rsp_xml = ? , a.rsp_time = ? where a.log_id = ? ";
	public static final String UPDATE_INF_CALL_LOG_NO_CLOB = " update inf_comm_client_calllog a set a.result_desc = ?,  a.rsp_time = ? where a.log_id = ? ";
}
