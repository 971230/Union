...
public class HttpPostYXTInvoker extends Invoker{
	protected Logger logger = Logger.getLogger(HttpPostYXTInvoker.class);
...
//            	resContent = "{\"code\":\"9999\",\"detail\":\"未知(aop@!@#$%^&~:{}|?><<*()[]/.,';未返回错误描述)\"}";
            ...
            Map resMap = JSONObject.fromObject(resContent);
            String resstr = (String)resMap.get("result");
            Map result = new HashMap();
            String[] strs = resstr.split("&");
            for (int i = 0; i < strs.length; i++) {
            	result.put(strs[i].split("=")[0], strs[i].split("=")[1]);
			}
            ...
            if (result != null) {
                ...
                resContent = resContent.replace("\"Body\":", "\"BodyJson\":");
                resContent = resContent.replace("\"result\":", "\"resultcode\":");
                /** add by yehaibo 2017-12-06 start
                 * bb!=-1 && aa!=-1 and判断有误，且replace方法不需要判断是否存在*/
//                int aa = resContent.indexOf("BodyJson\":\"\"");
//                int bb = resContent.indexOf("respJson\":\"\"");
//                if(bb!=-1 && aa!=-1){//对respJson结果为空的置为null
//                }
	            resContent = resContent.replace("respJson\":\"\"", "respJson\": null");
	            resContent = resContent.replace("BodyJson\":\"\"", "BodyJson\": null");
	            /** add by yehaibo 2017-12-06 end*/
