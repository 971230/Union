...
public class HttpPostAipBSSNewZJInvoker extends Invoker{
	protected Logger logger = Logger.getLogger(HttpPostAipBSSNewZJInvoker.class);
...
//            	resContent = "{\"code\":\"9999\",\"detail\":\"未知(aop@!@#$%^&~:{}|?><<*()[]/.,';未返回错误描述)\"}";
            ...
            if (result != null) {
    			...
    			if (StringUtil.equals(code, "9999")) {
    				if(result.containsKey("detail")){
    					throw new Exception((String) result.get("detail"));
    				}
