 ...
 * 4G省内闲时流量包附加产品 [附加产品]
 */
public class FourGIdlesseTrafficPackCbssInvoker extends AbsCbssInvoker<MobileNetworkServiceHandleReq, MobileNetworkServiceHandleResp> {

	private final static Logger logger = Logger.getLogger(FourGIdlesseTrafficPackCbssInvoker.class);
	...
	public MobileNetworkServiceHandleResp businessProcess(MobileNetworkServiceHandleReq zteRequest, HttpLoginClient client, CustomInfo customInfo) throws Exception {
		...
		// HttpLoginClient client = (HttpLoginClient)
		// zteRequest.getHttpLoginclient();
	...
	public void mobileProductPageClickSubmit3_2Request(HttpLoginClient client) {
		...
		String tradeMain = "[{\"TRADE_ID\": \"" + client.getParam().getTradeId() + "\", \"TRADE_TYPE\": \"移网产品/服务变更\", \"SERIAL_NUMBER\": \""
				+ client.getParam().getBill().getSerialNumber() + "\", \"TRADE_FEE\": \"0.00\", \"CUST_NAME\": \"" + client.getParam().getCustomInfo().getCustName()
				+ "\", \"CUST_ID\": \"" + client.getParam().getCustomInfo().getCustId() + "\", \"USER_ID\": \"" + client.getParam().getUserId() + "\", \"ACCT_ID\": \""
				+ client.getParam().getUserId() + "\", \"NET_TYPE_CODE\": \"50\", \"TRADE_TYPE_CODE\": \"120\"}]";
	...
	public void getFullBase(HttpLoginClient client) throws Exception {
		...
		String Form0 = "ORDER_MGR,RElA_TRADE_ID,ORDER_TYPE,SUPPORT_TAG,COMM_SHARE_NBR_STRING,AC_INFOS,FORGIFT_USER_ID,QUERY_ACCOUNT_ID,_rightCode,inModeCode,NET_TYPE_CODE,SERIAL_NUMBER,subQueryTrade";
	...
	public void getBase(HttpLoginClient client) throws Exception {
		...
		getParams.add(new BasicNameValuePair("subSysCode", client.getParam().getProductTypeCode()));
...
//			bill.setSerialNumber("18578324321");//5 a
			...
			FourGIdlesseTrafficPackCbssInvoker fourGIdlesseTrafficPackCbssInvoker = new FourGIdlesseTrafficPackCbssInvoker();
			MobileNetworkServiceHandleReq zteRequest = new MobileNetworkServiceHandleReq();
//			zteRequest.setMobileNo("18578324321"); //5 a
			zteRequest.setMobileNo("18566413906");  //10 b
			zteRequest.setPackFlag("b");
			CustomInfo customInfo = new CustomInfo();
			client.getParam().setOrder_create_time("2015-02-26 08:08:08");
			fourGIdlesseTrafficPackCbssInvoker.businessProcess(zteRequest, client, customInfo);
