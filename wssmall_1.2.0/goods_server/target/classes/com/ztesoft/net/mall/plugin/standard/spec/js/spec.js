...
var Product= {
 ...
 //alert(2)
	...
	createPros:function(specs,names){
		...
		var self = this;
		$.ajax({
			url:ctx+'/shop/admin/memberPrice!disLvInput.do?ajax=yes',
			dataType:'html',
		...
		for(i in products){
			...
			for(j in specar){
				...
				if( parseInt(spec.spectype) == 0){
					html+="<div class='select-spec-unselect spec_selected' style='padding-left:50px;'><span>" + spec.specvalue +"</span></div>";
				}else{
			...
			html+='<td><input type="text" class="price" name="prices" size="8" autocomplete="off" value="'+ price +'"><input type="button" gid="" pid="" hasConfirm="0"  class="mempriceBtn" value="会员价" /><div class="member_price_box" index="'+i+'">'+priceInputHtml+'</div></td>';
