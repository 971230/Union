...
public class OrderNAction extends WWAction {
   ...
   // private PaymentCfgInf paymentCfgServ;
    private Order ord;
    private Bank bank;
   // private List<TestRule> testRuleList;
    ...
    private List<OrderMessage> messageList;
    ...
    public String listn() {
        ...
        ordParam.setUserName(username);
        //this.webpage = this.orderManager.listc(this.getPage(), this
        //		.getPageSize(), 0, ordParam, order);
        this.webpage = this.orderManager.listGroupOrder(this.getPage(), this.getPageSize(), 0, ordParam, order);
    ...
    public String listOrderException(){
    	
    	if(this.orderException == null){
    		this.orderException = new OrderException();
    	}
    	
    	this.webpage = this.orderManager.listOrderException(this.orderException,this.getPage(),this.getPageSize());
    	
    	return "order_exception_list";
    }
    
    public String addOrderException(){
    	
    	return "order_exception_add";
    }
    
    public String editOrderException(){
    	this.orderException = this.orderManager.qryExceptionById(this.exception_id);
    	
    	return "order_exception_add";
    }
    
    //预约
    public String listYuyue(){
    	try {
			//testRuleList = this.orderManager.qryYuyueList();
		} catch (Exception e) {
			e.printStackTrace();
		}
    	return "listYuyue";
    ...
    public String delOrderException(){
    	try {
			this.orderManager.delOrderException(exception_id);
    ...
    public String saveOrderException(){
    	
    	try {
			Integer count = this.orderManager.qryExceptionCount(this.orderException);
			if(count > 0){
				this.json = "{result:0,message:'此编码已存在，请重新输入您的异常编码'}";
			}else{
				this.orderManager.saveOrderException(orderException);
				this.json = "{result:1,message:'操作成功'}";
			}
		} catch (Exception e) {
			this.json = "{result:0,message:'操作失败'}";
    ...
    public String detail() {
        ...
        //orderManager.setOrderSecurity(ord);
        this.ord = this.orderManager.get(orderId);
		orderManager.setOrderSecurity(ord);
		returned = orderApplycMamager.countAmountByServiceType(OrderStatus.ORDER_TYPE_4,orderId)>0;
		refund = orderApplycMamager.countAmountByServiceType(OrderStatus.ORDER_TYPE_2,orderId)>0;
		exchange = orderApplycMamager.countAmountByServiceType(OrderStatus.ORDER_TYPE_3,orderId)>0;
...
//		
    ...
    public String remark() {
        ...
        messageList = orderCommentManager.listOrderMessage(orderId);
        return "remark";
    }
...
/*
    ...
    public void setMessageList(List<OrderMessage> messageList) {
	...
	/*public List<TestRule> getTestRuleList() {
		return testRuleList;
	}

	public void setTestRuleList(List<TestRule> testRuleList) {
		this.testRuleList = testRuleList;
	}*/
