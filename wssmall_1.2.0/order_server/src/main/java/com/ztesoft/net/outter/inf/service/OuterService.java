...
public class OuterService extends BaseSupport implements IOuterService {

	@Resource
	protected IOutterTmplManager outterTmplManager;
	@Resource
	protected ITplInstManager tplInstManager;
	@Resource
	protected IDcPublicInfoManager dcPublicInfoManager;
	private static Logger logger = Logger.getLogger(OuterService.class);
	...
	public void fieldDefaultVal(Outer outer){
		try{
	        ...
	        List<Map> extDefaultList = dcPublicCache.getList(EXT_PUBLICS_KEY);
	        Class ocs = outer.getClass();
	...
	public boolean outerIsExists(String outterid,String order_from){
		...
		return this.baseDaoSupport.queryForInt(sql, outterid,order_from)>0;
	}
			...
			synchronized (key) {
				for(Outer oo:outerList){
					...
					while (it.hasNext()) {
						...
						AttrSyLoadReq req = new AttrSyLoadReq();
						req.setOuter(oo);
						req.setKey(key);
						req.setValue(value);
						AttrDef attrDef = CacheUtils.getCacheAttrDef(key+EcsOrderConsts.ATTR_SPEC_ID_999);
						if(attrDef !=null){
							req.setHander_class(attrDef.getHandler_class());
							AttrSyLoadResp resp = client.execute(req, AttrSyLoadResp.class);
							if(ConstsCore.ERROR_FAIL.equals(resp.getError_code())) //抛出异常
								throw new RuntimeException(resp.getError_msg());
						}
					}
...
//			ex.printStackTrace();
	...
	public TempInst getAcceptTmpl(String goods_id){
		return tplInstManager.getTempInstByGoodsIdAndSource(goods_id, ManagerUtils.getSourceFrom());
	}
...
//	}
	...
	public OrderOuter packageOrderOuter(Outer oo,String batch_id){
		...
		if(!StringUtils.isEmpty(oo.getPay_method())){
			//支付方式ID
			PublicConst payment = outterTmplManager.queryPublicConst(oo.getOrder_from(), OuterSysConst.CONST_OBJECT_TYPE_PAYMENT, oo.getPay_method());
			if(payment!=null){
		...
		if(!StringUtils.isEmpty(oo.getSending_type())){
			//配送方式
			PublicConst shipping = outterTmplManager.queryPublicConst(oo.getOrder_from(), OuterSysConst.CONST_OBJECT_TYPE_SHIPPING, oo.getSending_type());
			if(shipping!=null){
		...
		if(!StringUtils.isEmpty(oa.getO_order_status())){
			//订单状态
			PublicConst pc = outterTmplManager.queryPublicConst(oo.getOrder_from(), OuterSysConst.CONST_OBJECT_TYPE_ORDER_STATUS, oa.getO_order_status());
			if(pc!=null){
		...
		if(!StringUtils.isEmpty(oa.getO_pay_status())){
			//支付状态
			PublicConst pc = outterTmplManager.queryPublicConst(oo.getOrder_from(), OuterSysConst.CONST_OBJECT_TYPE_PAY_STATUS, oa.getO_pay_status());
			if(pc!=null){
