package com.ztesoft.net.mall.core.plugin.member;
...
public class MemberPluginBundle extends AutoRegisterPluginsBundle {
	...
	public void onLogout(Member member){
		try{
			...
			AutoRegisterPluginsBundle.loger.info("调用会员插件注销事件错误", e);
			throw e;
		}
	}
	...
	public void onLogin(Member member){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用会员插件登录事件错误", e);
			throw e;
		}
	}
	...
	public void onRegister(Member member){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用会员插件注册事件错误", e);
			throw e;
		}
	}
	...
	public void onEmailCheck(Member member){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用会员插件邮件验证事件错误", e);
			throw e;
		}
	}
