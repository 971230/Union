...
public class OrderStateUpdateServlet extends HttpServlet {
	...
	private void execute(HttpServletRequest request, HttpServletResponse response)
		...
		OrderInfoUpdateNewResp resp = null;
		try {
			// 获取请求的json数据
			...
			OrderInfoUpdateNewReq requ = (OrderInfoUpdateNewReq) JSONObject.toBean(orderInfoJson, OrderInfoUpdateNewReq.class);
			// 根据请求参数更新订单信息
			ZteClient client = ClientFactory.getZteDubboClient(ManagerUtils.getSourceFrom());
			resp = client.execute(requ, OrderInfoUpdateNewResp.class);
		} catch (Exception e) {
			resp = new OrderInfoUpdateNewResp();
			resp.setResp_code("1");
