 ...
 *
 */
public class BrandNumberHandler extends BaseSupport implements IAttrHandler{
	@Override
	...
	public AttrInstLoadResp handler(AttrSwitchParams params) {
		...
		if(!CommonDataFactory.getInstance().isZbOrder(plat_type) && StringUtils.isEmpty(value)){
			if(!StringUtils.isEmpty(pro_goods_id)){
				value = CommonDataFactory.getInstance().getProductSpec(order_id, null, pro_goods_id, SpecConsts.BRAND_CODE);
			}
			else if(!StringUtils.isEmpty(goods_id)){
				value = CommonDataFactory.getInstance().getGoodSpec(null, goods_id, SpecConsts.BRAND_CODE);
			}
		}
