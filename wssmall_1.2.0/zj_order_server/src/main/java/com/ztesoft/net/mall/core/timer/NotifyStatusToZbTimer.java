		...
		return ;
 		...
 		synchronized (_KEY) {
 			List<CoQueue> coQueueLst = coQueueManager.getForABGrayJob(
 					new String[]{"CO_NOTIFYSTATUS_ZB","CO_DELIVERYNOTIFY_ZB","CO_ROUTE_NOTIFY_ZB","CO_ORDERINFONOTIFY_ZB","CO_ABNORMALTOZB"}, NotifyStatusToZbTimer.maxNum,local_env_status,local_env_code,true); //修改脚本，生产环境查询es_abgray_ord_env_rel存在的记录，或者不存在的记录，否则非生产环境，则完全匹配。
