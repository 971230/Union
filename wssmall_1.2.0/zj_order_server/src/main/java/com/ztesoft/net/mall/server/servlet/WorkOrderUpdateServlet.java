 ...
 * @date 2017年12月1日
 */
public class WorkOrderUpdateServlet extends HttpServlet {

	private static final long serialVersionUID = 3411911039617039211L;

	private static Logger logger = Logger.getLogger(WorkOrderUpdateServlet.class);
	...
	private void execute(HttpServletRequest request, HttpServletResponse response)
		...
		WorkOrderUpdateReq requ = null;
		WorkOrderUpdateResp resp = null;
		try {
			...
			requ = (WorkOrderUpdateReq) JSONObject.toBean(orderInfoJson, WorkOrderUpdateReq.class);
			ZteClient client = ClientFactory.getZteDubboClient(ManagerUtils.getSourceFrom());
			resp = client.execute(requ, WorkOrderUpdateResp.class);
		} catch (Exception e) {
			resp = new WorkOrderUpdateResp();
			resp.setResp_code("1");
