 ...
 *
 */
public class BrandNameHandler  extends BaseSupport implements IAttrHandler{
	...
	public AttrInstLoadResp handler(AttrSwitchParams params) {
		...
		if(!CommonDataFactory.getInstance().isZbOrder(plat_type) && StringUtils.isEmpty(value)) {
			if(!StringUtils.isEmpty(pro_goods_id)){
				value = CommonDataFactory.getInstance().getProductSpec(order_id, null, pro_goods_id, SpecConsts.BRAND_NAME);
			}
			else if(!StringUtils.isEmpty(goods_id)){
				value = CommonDataFactory.getInstance().getGoodSpec(null, goods_id, SpecConsts.BRAND_NAME);
			}
		}
