 ...
 *
 */
public class ContactAddrHandler extends BaseSupport implements IAttrHandler {
	...
	public AttrInstLoadResp handler(AttrSwitchParams params) {
		...
		String contactAddr = value;
		boolean isTbOrder = CommonDataFactory.getInstance().isTbOrder(order_from);//是否淘宝订单,默认否
		if(isTbOrder){
			contactAddr = Const.getStrValue(orderAttrValues, AttrConsts.SHIP_ADDR);
		}
		
		AttrInstLoadResp resp = new AttrInstLoadResp();
		resp.setField_value(contactAddr);
		return resp;
	}
