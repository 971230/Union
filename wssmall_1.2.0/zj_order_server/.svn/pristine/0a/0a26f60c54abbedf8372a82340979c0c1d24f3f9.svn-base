 ...
 *
 */
public class AuditOrderCancelServlet extends HttpServlet {

	private static final long serialVersionUID = 7900290024351083847L;
	
	private static Logger logger = Logger.getLogger(AuditOrderCancelServlet.class);
	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException{
		...
		AuditOrderCancelResp resp = null;
		try {
			//获取请求的json数据
			requJsonStr = getRequestJson(request, response);
			logger.info("[AuditOrderCancelServlet] 请求报文-request:" + requJsonStr);
			//解析请求参数
			JSONObject requJson = JSONObject.fromObject(requJsonStr);
			//请求入参
			JSONObject orderInfoJson = requJson.getJSONObject("order_cancle_req");
			AuditOrderCancelReq requ = (AuditOrderCancelReq) JSONObject.toBean(orderInfoJson, AuditOrderCancelReq.class);
			//根据请求参数更新订单信息
			ZteClient client = ClientFactory.getZteDubboClient(ManagerUtils.getSourceFrom());
			resp = client.execute(requ, AuditOrderCancelResp.class);
		} catch (Exception e) {
			resp = new AuditOrderCancelResp(); 
			resp.setResp_code("1");
		...
		//返回接口调用结果
		logger.info("[AuditOrderCancelServlet] 响应报文-response:"+respJsonStr);
