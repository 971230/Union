...
public final class ZhuanDuiBaoUtil
{
	...
	private static final String SQL_INSERT="insert into es_gd_zhuanduibao(order_id,out_tid,pmt_code,goods_id) values(?,?,?,?)";
	
	/**
	 * this is the only function which used to save the information of 轉兌包
	 * @param order_id: the order id of the inner system
	 * @param out_tid: the order id of the outer system
	 * @param pmt_codes: the activities about this order. Delimited by comma 
	 */
	public static void save(String order_id, String out_tid, String pmt_code_s)
	{
		...
		{			
			...
			for(String pmt_code : pmt_codes)
			{
				...
				for(Map item : result)
				{
					dbUtil.execute(SQL_INSERT, order_id,out_tid,pmt_code,item.get("goods_id"));
				}
			}							
		}
		catch (Exception e)
