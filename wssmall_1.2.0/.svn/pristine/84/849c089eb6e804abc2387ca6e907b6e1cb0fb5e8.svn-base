...
/**
 * @Description 消费者-消费主题：crm_topic
 * @author  zhangJun
 * @date    2015-3-12
 * @version 1.0
 */
public class MqReceiveCRM implements EventSingleListenHandler {
	
	private static Logger logger = Logger.getLogger(MqReceiveCRM.class);
	@Resource
	...
	public void executeEvent(EventMessageExt e) throws EventHandleException {
		...
		//logger.info("crm_topic消费者接收一条消息" );
		//接收对象，请求相应的api
			...
			try{
				...
				if(!ConstsCore.CONSTS_YES.equals(pressure_test_flag)){//压力测试开关，开：就不做标准化处理
					if(msg.getDeSerializeData()!=null){
						...
						logger.info("[MQ消费者] 订单: ["+request.getBase_order_id()+"] [MqReceiveCRM] 开始消费!!");
						...
						logger.info("[MQ消费者] 订单: ["+request.getBase_order_id()+"] [MqReceiveCRM] 消息队列消费消息耗时 :"+(end3-end2));
					}
				}
			}catch (Exception e) {
