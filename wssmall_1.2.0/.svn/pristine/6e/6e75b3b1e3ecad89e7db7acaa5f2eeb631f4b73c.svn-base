 ...
 * add by wui动态memcache，放置动态数据
 */
public class DyXmCacheContext {
	
	private static MemcachedClient client = null ;
	
	private DyXmCacheContext(){
	...
	private static void init(){
		MemcachedClientBuilder builder = new XMemcachedClientBuilder(AddrUtil
				.getAddresses(CacheValues.DY_MEMCACHED_SERVERS));
        ...
        builder.getConfiguration().setSessionIdleTimeout(10000);// add by wui延长超时时间
		try {
