...
import com.ztesoft.rop.event.AfterDoServiceEvent;
import com.ztesoft.rop.event.AfterStartedRopEvent;
import com.ztesoft.rop.event.PreCloseRopEvent;
import com.ztesoft.rop.event.PreDoServiceEvent;
import com.ztesoft.rop.event.RopEventListener;
import com.ztesoft.rop.event.RopEventMulticaster;
import com.ztesoft.rop.event.SimpleRopEventMulticaster;
import com.ztesoft.rop.marshaller.JacksonJsonRopMarshaller;
import com.ztesoft.rop.marshaller.JaxbXmlRopMarshaller;
import com.ztesoft.rop.request.RopRequestMessageConverter;
...
import com.ztesoft.rop.response.RejectedServiceResponse;
import com.ztesoft.rop.response.ServiceUnavailableErrorResponse;
import com.ztesoft.rop.response.SubErrors;
import com.ztesoft.rop.response.TimeoutErrorResponse;
import com.ztesoft.rop.route.ServiceRouter;
import com.ztesoft.rop.security.DefaultInvokeTimesController;
import com.ztesoft.rop.security.DefaultSecurityManager;
import com.ztesoft.rop.security.InvokeTimesController;
import com.ztesoft.rop.session.DefaultSessionManager;
import com.ztesoft.rop.session.SessionManager;
import commons.CommonTools;

/**
 * 
 * modified by easonwu 2013-10-18 增加日志拦截信息处理
 *
 */
public class AnnotationServletServiceRouter implements ServiceRouter {
    ...
    private com.ztesoft.rop.security.SecurityManager securityManager;
    ...
    public void service(Object request, Object response) {
        ...
        //logger.info("----sysy=="+ System.getProperty("java.endorsed.dirs"));
        // 获取服务方法最大过期时间
        ...
        try {
            ...
            RopRequestContext ropRequestContext = buildRequestContextWhenException(
                    servletRequest, beginTime);
            RejectedServiceResponse ropResponse = new RejectedServiceResponse(
            ...
            RopRequestContext ropRequestContext = buildRequestContextWhenException(
                    servletRequest, beginTime);
            TimeoutErrorResponse ropResponse = new TimeoutErrorResponse(
                    ropRequestContext.getMethod(),
                    ropRequestContext.getLocale(), serviceMethodTimeout);
            //writeResponse(ropResponse, servletResponse,ServletRequestContextBuilder.getResponseFormat(servletRequest));
            fireAfterDoServiceEvent(ropRequestContext);
        } catch (Throwable throwable) {// 产生未知的错误
            ServiceUnavailableErrorResponse ropResponse = new ServiceUnavailableErrorResponse(
                    method,ServletRequestContextBuilder.getLocale(servletRequest),throwable);
            //writeResponse(ropResponse, servletResponse,ServletRequestContextBuilder.getResponseFormat(servletRequest));
            RopRequestContext ropRequestContext = buildRequestContextWhenException(servletRequest, beginTime);
            fireAfterDoServiceEvent(ropRequestContext);
        }finally{
        	//mod by wui移动线程变量对象位置
        }
    }
    ...
    public void startup() {
        ...
        this.requestContextBuilder = new ServletRequestContextBuilder(
                this.formattingConversionService, this.sessionManager);
    ...
    @Override
    public void setThreadFerryClass(
            Class<? extends ThreadFerry> threadFerryClass) {
        this.threadFerryClass = threadFerryClass;
    }

    @Override
    public void setInvokeTimesController(
            InvokeTimesController invokeTimesController) {
    ...
    @Override
    public void setSecurityManager(com.ztesoft.rop.security.SecurityManager securityManager) {
        this.securityManager = securityManager;
    }

    @Override
    public void setFormattingConversionService(
            FormattingConversionService formatConversionService) {
    ...
    public int getServiceTimeoutSeconds() {
        return serviceTimeoutSeconds >= 0 ? serviceTimeoutSeconds
                : Integer.MAX_VALUE;
    }
    ...
    private class ServiceRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                //初始化当前线程变量 modified by easonwu 2013-10-17 //mod by wui 将线程变量对象放入线程池对象中，避免线程池引起的问题
                if(StringUtils.isNotEmpty(ropRequestContext.getMethod()))
                	securityManager.initCurrentThreadVal(ropRequestContext.getAppKey(),ropRequestContext.getMethod() ,ropRequestContext.getVersion()) ;
                ...
                // 输出响应
                writeResponse(ropRequestContext,ropRequestContext.getRopResponse(),servletResponse, ropRequestContext.getMessageFormat());
            } catch (Throwable e) {
            	e.printStackTrace();
                ...
                // 输出响应前拦截
                //invokeBeforceResponseOfInterceptors(ropRequestContext);
                writeResponse(ropRequestContext,ropResponse, servletResponse,ropRequestContext.getMessageFormat());
            } finally {
                if (ropRequestContext != null) {
                    ...
                    // 完成一次服务请求，计算次数
                    invokeTimesController.caculateInvokeTimes(ropRequestContext,ropRequestContext.getAppKey(),ropRequestContext.getSession(ropRequestContext.getRopRequest().getSessionId()));
                    fireAfterDoServiceEvent(ropRequestContext);
                    
                    CurrentThreadVar.remove(); //add by wui清空当前线程变量对象
     ...
     * @return
     */
    private RopRequestContext buildRequestContextWhenException(
            HttpServletRequest request, long beginTime) {
        RopRequestContext ropRequestContext = requestContextBuilder
                .buildBySysParams(ropContext, request);
    ...
    private RopContext buildRopContext() {
        DefaultRopContext defaultRopContext = new DefaultRopContext(
    ...
    private void fireAfterStartedRopEvent() {
        AfterStartedRopEvent ropEvent = new AfterStartedRopEvent(this,
                this.ropContext);
    ...
    private void fireAfterDoServiceEvent(RopRequestContext ropRequestContext) {
        this.ropEventMulticaster.multicastEvent(new AfterDoServiceEvent(this,
                ropRequestContext));
    }
    ...
    private void invokeBeforceServiceOfInterceptors(RopRequestContext ropRequestContext) {
        ...
        try {
            ...
            ropRequestContext.setRopResponse(new ServiceUnavailableErrorResponse(
                    ropRequestContext.getMethod(), ropRequestContext.getLocale(), e));
            logger.info("在执行拦截器[" + tempInterceptor.getClass().getName() + "]时发生异常.", e);
        }
    }
     ...
     * @param ropRequestContext
     */
    private void invokeBeforceResponseOfInterceptors(
        ...
        try {
            ...
            ropRequestContext
                    .setRopResponse(new ServiceUnavailableErrorResponse(
                            ropRequestContext.getMethod(), ropRequestContext
                            .getLocale(), e));
            logger.info("在执行拦截器[" + tempInterceptor.getClass().getName() + "]时发生异常.", e);
        }
    }

    private void writeResponse(RopRequestContext ropRequestContext,Object ropResponse,
                               HttpServletResponse httpServletResponse, String messageFormat) {
    	
    	RopResp resp =null;
        try {
            httpServletResponse.setCharacterEncoding(Constants.UTF8);
            if (messageFormat.equalsIgnoreCase("xml")) {
                httpServletResponse.setContentType(APPLICATION_XML);
                xmlMarshallerRop.marshaller(ropResponse,
                        httpServletResponse.getOutputStream());
            } else if (messageFormat.equalsIgnoreCase(MessageFormat.fjson.name())) {
            	
            	if(ropResponse instanceof ErrorResponse && MessageFormat.fjson.name().equals(ropRequestContext.getFormat())) { //add by wui异常对象转换
            		String error_code =((ErrorResponse)ropResponse).getErrorflag();
            		String error_msg = ((ErrorResponse)ropResponse).getMessage();
            		resp = BeanUtils.instantiateClass(ropRequestContext.getServiceMethodHandler().getRespType());
            		resp.setError_code(error_code);
            		resp.setError_msg(error_msg);
				}
            	
            	String json =  "";
            	if(resp !=null)
            		 json = CommonTools.beanToJson(resp);
            	else
            		 json = CommonTools.beanToJson(ropResponse);
                //add by wui支持fastjson格式
            ...
            } else if (messageFormat.equalsIgnoreCase(MessageFormat.httpjson.name())) {
            	if(ropResponse instanceof ErrorResponse && MessageFormat.httpjson.name().equals(ropRequestContext.getFormat())) { //add by wui异常对象转换
            		String error_code =((ErrorResponse)ropResponse).getErrorflag();
            		String error_msg = ((ErrorResponse)ropResponse).getMessage();
            		resp = BeanUtils.instantiateClass(ropRequestContext.getServiceMethodHandler().getRespType());
            		resp.setError_code(error_code);
    			...
    			String json ="";
            	if(resp !=null){
            		 com.ztesoft.common.util.BeanUtils.beanToMap(httpRespMap, resp);
            		 json =  CommonTools.beanToJson(httpRespMap);
            	}else{
            		 com.ztesoft.common.util.BeanUtils.beanToMap(httpRespMap, ropResponse);
            		 json =  CommonTools.beanToJson(httpRespMap);
            	}
            ...
            }else if(messageFormat.equalsIgnoreCase(MessageFormat.httpxml.name())){
            	if(ropResponse instanceof ErrorResponse && MessageFormat.httpxml.name().equals(ropRequestContext.getFormat())) {
            		String error_code =((ErrorResponse)ropResponse).getErrorflag();
            		String error_msg = ((ErrorResponse)ropResponse).getMessage();
            		resp = BeanUtils.instantiateClass(ropRequestContext.getServiceMethodHandler().getRespType());
            		resp.setError_code(error_code);
...
//                jsonMarshallerRop.marshaller(ropResponse,
//                        httpServletResponse.getOutputStream()); httpServletResponse.setContentType(APPLICATION_JSON);
//              jsonMarshallerRop.marshaller(ropResponse,
//              httpServletResponse.getOutputStream());
//      
//      
    ...
    private Object doService(RopRequestContext ropRequestContext) {
        ...
        if (ropRequestContext.getMethod() == null) {
            ...
            CurrentThreadVar.getVar().setExceptionStat("方法为空") ;
            ...
            CurrentThreadVar.getVar().setExceptionStat("无效方法") ;
        } else {
            try {
                ropResponse = serviceMethodAdapter.invokeServiceMethod(ropRequestContext);
                
                CurrentThreadVar.getVar().setSuccessStat() ;
                ...
                //add by wui fastjson转换处理
                if(MessageFormat.fjson.name().equals(ropRequestContext.getFormat())){
                	ropResponse = BeanUtils.instantiate(ropRequestContext.getServiceMethodHandler().getRespType());
                    ((RopResp)ropResponse).setError_code("-1");
                    ((RopResp)ropResponse).setError_msg(e.getMessage());
                }
                
                CurrentThreadVar.getVar().setExceptionStat(e.getMessage()) ;
            }
        }
        return ropResponse;
    }
    ...
    private void initMessageSource() {
        if (logger.isInfoEnabled()) {
            logger.info("加载错误码国际化资源：" + I18N_ROP_ERROR + ","
                    + this.extErrorBasename);
        }
    ...
    public com.ztesoft.rop.security.SecurityManager getSecurityManager() {
