...
import com.ztesoft.rop.response.*;
import com.ztesoft.rop.route.impl.DefaultServiceAccessController;
import com.ztesoft.rop.route.impl.SimpleRopRequestContext;
 ...
 * @author 
 * @version 1.0
 *  modified by easonwu 2013-10-18 增加线程变量初始
 */
public class DefaultSecurityManager implements SecurityManager {
    ...
    protected AppSecretManager appSecretManager = new ReadDbBaseAppSecretManager();
   ...
   // protected FileUploadController fileUploadController;
    ...
    public MainError validateSystemParameters(RopRequestContext rrc) {
        ...
        if (!appSecretManager.isValidAppKey(rrc.getAppKey()) && EopSetting.ROP_VALI.equals("yes")) {//rop验证开关
...
//        }
        ...
        if (rrc.getMethod() == null) {
            ...
            if (!ropContext.isValidMethod(rrc.getMethod()) && EopSetting.ROP_VALI.equals("yes")) {
        ...
        if (rrc.getVersion() == null) {
            ...
            if (!ropContext.isValidMethodVersion(rrc.getMethod(), rrc.getVersion()) && EopSetting.ROP_VALI.equals("yes")) {
    ...
    public MainError validateOther(RopRequestContext rrctx) {
        ...
        if (mainError != null && EopSetting.ROP_VALI.equals("yes")) {
            return mainError;
        }

        //2.判断应用/会话/用户访问服务的次数或频度是否超限
        if(EopSetting.ROP_VALI.equals("yes"))
   ...
   /* @Override
    ...
    private MainError checkInvokeTimesLimit(RopRequestContext rrctx) {
    	//
        if (invokeTimesController.isAppSessionInvokeFrequencyExceed(rrctx,rrctx.getAppKey(),rrctx.getSession(rrctx.getRopRequest().getSessionId()))) {
            return MainErrors.getError(MainErrorType.EXCEED_APP_INVOKE_FREQUENCY_LIMITED, rrctx.getLocale());
        } else if (invokeTimesController.isAppMethodInvokeLimitExceed(rrctx,rrctx.getAppKey())) {
            return MainErrors.getError(MainErrorType.EXCEED_USER_INVOKE_LIMITED, rrctx.getLocale(),rrctx.getMethod());
        } else if (invokeTimesController.isAppInvokeLimitExceed(rrctx,rrctx.getAppKey())) {
    ...
    private MainError checkServiceAccessAllow(RopRequestContext smc) {
        if (EopSetting.ROP_VALI.equals("yes") && !getServiceAccessController().isAppGranted(smc,smc.getAppKey(), smc.getMethod(), smc.getVersion()) ) {
           ...
           /* if (!getServiceAccessController().isUserGranted(smc,smc.getSession(), smc.getMethod(), smc.getVersion())) {
                ...
                if (ctx.getSign() == null) {
                    ...
                    if (StringUtils.isEmpty(signSecret)) {
                       // throw new RopException("无法获取" + ctx.getAppKey() + "对应的密钥");
                    	logger.info(ctx.getAppKey() + "对应的密钥");
                        return MainErrors.getError(MainErrorType.NULL_SIGNATURE, ctx.getLocale(),ctx.getAppKey());
                    }

                    String signValue = RopUtils.sign(needSignParams, signSecret);
                    if (!signValue.equals(ctx.getSign())) {
                        if (logger.isErrorEnabled()) {
                            logger.info(ctx.getAppKey() + "的签名不合法，请检查");
                        }
    ...
    private MainError checkSession(RopRequestContext smc) {
        ...
        if (smc.getServiceMethodHandler() != null &&
            ...
            if (smc.getRopRequest().getSessionId() == null || smc.getRopRequest().getSessionId().equals("")) {
    ...
    private boolean isValidSession(RopRequestContext smc) {
        if (sessionManager.getSession(smc.getRopRequest().getSessionId()) == null) {
            if (logger.isDebugEnabled()) {
                logger.debug(smc.getRopRequest().getSessionId() + "会话不存在，请检查。");
            }
            return false;
        } else {
            return true;
	...
	/**
	 * 
	 * 初始化当前线程变量 by easonwu 
	 * 
	 */
	public void initCurrentThreadVal(String appKey,String method,String version) {
		Map data = RopDaoImpl.getInstance().qryAppInfoBy(appKey,method,version);
		
		CurrentThreadVar.getVar().setByMap(data); 
	}
