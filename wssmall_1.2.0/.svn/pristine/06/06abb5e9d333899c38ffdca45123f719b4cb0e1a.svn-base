...
public class ZteLogger {
	...
	public static void setNeedEnableLogger(boolean needEnableLogger) {
		ZteLogger.needEnableLogger = needEnableLogger;
	}
	 ...
	 * 发生特别错误时记录完整错误现场
	 */
	public static void logErrorScene(Map<String, Object> rt, ZteResponse tRsp, String appSecret) {
		if (!needEnableLogger) {
			return;
		}
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
...
//		sb.append(tRsp.getSubCode());
		...
		appendLog((ZteHashMap) rt.get("protocalMustParams"), sb);
		sb.append("^_^");
		sb.append("ProtocalOptParams:");
		appendLog((ZteHashMap) rt.get("protocalOptParams"), sb);
		sb.append("^_^");
		sb.append("ApplicationParams:");
		appendLog((ZteHashMap) rt.get("textParams"), sb);
		sb.append("^_^");
		sb.append("Body:");
	...
	private static void appendLog(ZteHashMap map, StringBuilder sb) {
		boolean first = true;
