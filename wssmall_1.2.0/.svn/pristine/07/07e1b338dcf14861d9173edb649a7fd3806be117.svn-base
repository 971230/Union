package com.ztesoft.net.cache.redis.jedis.tests.utils;
...
public class JedisSentinelTestUtil {
	private static Logger logger = Logger.getLogger(JedisSentinelTestUtil.class);
	...
	while (true) {
		...
		logger.info("Master not recognized by Sentinel "
		...
		logger.info("Slaves not recognized by Sentinel "
			+ sentinel.getHost() + ":" + sentinel.getPort()
			+ ", sleep...");
		continue;
	    }
