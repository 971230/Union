...
//		判断文章分类的父类id不能同自己的id一样
        if(cat.getParent_id() == null)
        	...
        	if(cat.getCat_id() != null && cat.getParent_id().equals(cat.getCat_id()))
        		throw new RuntimeException();//文章分类的父类id不能同自己的id一样
...
//      判断文章分类不能同名
        if(cat.getName() != null){
		    ...
		    if(count > 0)
		    	throw new RuntimeException();//文章分类不能同名
        }
...
//	      判断文章分类不能同名
	...
	public List listChildById(String cat_id) {
		...
		for(ArticleCat cat :allCatList){
			if(cat.getParent_id().equals(cat_id)){
	...
	private List<ArticleCat> getChildren(List<ArticleCat> catList ,String parentid){
		if(this.logger.isDebugEnabled()){
