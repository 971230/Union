...
public class CityMallOrderServlet extends HttpServlet {
	
	private static final String LOCAL_CHARSET = "utf-8";
	protected static IGoodsService goodServices;
	protected static IOrderServiceLocal localGoodServices;
	private static Logger logger = Logger.getLogger(CityMallOrderServlet.class);
...
//			    out_order.setReserve0(jsonObject_order_req.get("invoice_type").toString());
		    ...
		    if(jsonObject_order_req.get("baidu_begin_time")!= null && !jsonObject_order_req.get("baidu_begin_time").toString().equals("")){
		    ...
		    if(jsonObject_order_req.get("baidu_end_time")!= null && !jsonObject_order_req.get("baidu_end_time").toString().equals("")){
		    	String timea = jsonObject_order_req.get("baidu_end_time").toString();
			    SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");  
		        SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");  
		        String timesb = null;  
		        timesb = sdf2.format(sdf.parse(timea));
		    	out_order.setReserve9(timesb.toString());//百度冻结结束时间-----API(入参DTO为 ：Reserve9)
		    }else{
		    ...
		    if(jsonObject_order_req.get("source_from_system") != null && jsonObject_order_req.get("source_from_system") !=""){
		    	...
		    	else if(jsonObject_order_req.get("source_from").toString().equals("10014") 
						||jsonObject_order_req.get("source_from").toString().equals("10040")){
					out_order.setPlat_type("深圳联通商城");
				}
				else{
		    ...
		    if(jsonObject_order_req.get("ship_city") != null && jsonObject_order_req.get("ship_city") !=""){
		    	...
		    	String cityname = localGoodServices.getCityname(jsonObject_order_req.get("ship_city").toString());
		    	out_order.setCity(cityname);//收货人所在市city-地址地市ship_city
		    	 logger.info("*******************************"+cityname);
		    	String proId = localGoodServices.getProviceId(jsonObject_order_req.get("ship_city").toString());//根据地市编码查询省份编码
		    	out_order.setProvinc_code(proId);
		    	String proname = localGoodServices.getProvicename(jsonObject_order_req.get("ship_city").toString());//根据地市编码查询省份名称
		    ...
		    if(jsonObject_order_req.get("ship_country")!= null && jsonObject_order_req.get("ship_country") !=""){
		    	...
		    	String counrtyname = localGoodServices.getCityname(jsonObject_order_req.get("ship_country").toString());//地市编码转名称
		    ...
		    if(jsonObject_order_req.get("create_time") != null && !jsonObject_order_req.get("create_time").toString().equals("")){
		    ...
		    if(jsonObject_order_req.get("time")!= null && !jsonObject_order_req.get("time").toString().equals("")){
...
//		    	out_order.setOrder_channel(jsonObject_order_req.get("source_from").toString());
		    	...
		    	out_order.setOrder_channel("淘宝");
					...
					out_order.setOrder_channel("百度担保");
						...
						||jsonObject_order_req.get("source_from").toString().equals("10040")){
					out_order.setOrder_from("沃财富");
					out_order.setOrder_channel("沃财富");
				}
				else{
		 ...
		 JSONArray jsonArray = jsonObject_order_req.getJSONArray("order_list");
		  for (int i = 0; i < jsonArray.size(); i++) {
			  OuterItem OuterItemgz = new OuterItem();
			                ...
			                if(jsonStr.get("activitycode")!= null &&jsonStr.get("activitycode") !=""
			        	    		|| jsonStr.get("resourcestypeid")!= null &&jsonStr.get("resourcestypeid") !=""){
			        	    	String outer_sku_id = getValue(jsonStr.get("resourcestypeid"));
			        	    	String pagkage_id = getValue(jsonStr.get("activitycode"));
			        	    	Map<String, String> param = new HashMap<String, String>();
			        	    	if (!StringUtils.isEmpty(outer_sku_id) || !StringUtils.isEmpty(pagkage_id)) {
...
//									}
		  ...
		  if(LackOfField == "" ){
			  ...
			  outJson = "{\"order_resp\": {\"resp_code\": \"1\",\"resp_msg\": \""+jsonObject_order_req.get("order_id").toString()+"同步失败，原因为："+LackOfField+" \"}}"; 
		  }
...
//		  logger.info("1.9订单同步信息===List裡面字段測試==Pay_time="+OuterItemgz.getProduct_id().toString()); 
		 // 下面部分是输出部分的处理  
			...
			logger.info("-----------调用API返回-------------------------------------------------API返回resp="+resp.getError_msg());
			if("0".equals(resp.getError_code())){
				  outJson = "{\"order_resp\": {\"resp_code\": \"0\",\"resp_msg\": \""+jsonObject_order_req.get("order_id").toString()+"同步成功\"}}"; 
			}else {
	     ...
	     // respose.getWriter().print(outJson);
		  return outJson;
	    	...
	    	logger.info("1.9订单同步信息=======orderid:"+out_order.getOut_tid()+",操作接口失败=========================================");
	        return outJson;
	    } 
	}
	...
	private static void packageGoodsParam(OuterItem o, Map<String, String> param) {
