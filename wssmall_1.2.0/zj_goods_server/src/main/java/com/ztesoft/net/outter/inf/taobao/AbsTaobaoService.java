...
public abstract class AbsTaobaoService extends AbsTaobao{
	 ...
	 * @throws ApiException 
	 */
	public Outer packageTaobaoOrder(Trade t,String end_time,String appkey,String secret,String sessionKey,String url,String order_from,TradeGetResponse tradeInfo,boolean isJXT,String package_id,String sku) throws ApiException{
		try{
			...
			if(isJXT || (tradeInfo==null && !isJXT) || (t.getIsWt()!=null && t.getIsWt())){
				try{
					TradeWtverticalGetResponse wtresp = getTradeWtvertical(t.getTid()+"",appkey,secret,sessionKey,WT_URL);
			...
			if(!StringUtils.isEmpty(o.getService_remarks())){
				...
				if(StringUtils.isEmpty(o.getOrder_city_code()) && !StringUtils.isEmpty(phoneNo)){
					...
					try{
						...
						Map map = outterECSTmplManager.getPhoneInfo(phoneNo);
				...
				regionResp = regionService.getRegion(regionGet);//client.execute(regionGet, RegionsGetResp.class);//regionService.getRegion(regionGet);
...
//						com.ztesoft.net.outter.inf.util.ZhuanDuiBaoUtil.save(o.getOut_tid(), str);
			...
			//新增默认值=============mochunrun========2014-7-23=============================
			if(StringUtils.isEmpty(o.getOrder_city_code())){
				o.setOrder_city_code("440100");//归属地市：Order_city_code 订单归属属地有可能为空没有的默认为广州（440100）
			}
