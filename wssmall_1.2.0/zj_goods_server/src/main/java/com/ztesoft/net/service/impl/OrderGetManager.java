...
import com.ztesoft.net.server.OrderGetServlet.GoodsVo;
import com.ztesoft.net.server.OrderGetServlet.ParamList;
...
public class OrderGetManager extends ServiceBase {
	...
	public String taobaoOrderGet(String orderId,String toSysName,String serial_no) {
		...
		orderSql += " and  t.order_id = '" + orderId+"'";
			...
			List orders = this.baseDaoSupport.queryForList(orderSql);
...
//				order.setInvoice_content(getValue(m.get("invoice_content")));
			...
			for (int i = 0; i < orderlist.size(); i++) {
				...
				String order_id = order.getOrder_id();
				if (StringUtils.isNotEmpty(order_id)) {
					List items = this.baseDaoSupport.queryForList("select * from es_order_items i where i.order_id= '"+order_id+"' ");
					...
					List accepts = this.baseDaoSupport.queryForList("select * from es_taobao_accept a where a.order_id= '"+order_id+"' ");
	...
	String taobaoOrdertoJson(OrderTaobaoSyncResp resp ,String toSysName,String serial_no){
		...
		try {
			Map m = orderServiceLocal.queryMap(" select a.reserve0,a.reserve5,a.reserve9 from es_outer_accept a WHERE a.order_id = ? AND ROWNUM < 2 ", o.getOrder_id());
			...
			if(m.get("reserve1") != null)
				invoice_content = m.get("reserve1").toString();
			if(m.get("reserve2") != null)
				inventory_code = m.get("reserve2").toString();
			if(m.get("reserve3") != null)
				inventory_name = m.get("reserve3").toString();
			if(m.get("reserve4") != null)
				activity_list = m.get("reserve4").toString();
		} catch (Exception e) {
			e.printStackTrace();
		...
		try{
			Map map =orderServiceLocal.queryMap(" select a.params from v_product_phone_params a where a.a_goods_id = ? and rownum < 2", o.getGoods_id());
			if(map != null){
		...
		String out_plan_id_bss = null;
			...
			try{
				Map map =orderServiceLocal.queryMap(" select a.params from v_product_params a where a.a_goods_id = ? and rownum < 2", o.getGoods_id());
				if(map != null){
				String strpar = map.get("params").toString();
				strpar = strpar.substring(1, strpar.lastIndexOf("]"));
				ParamList pl = JsonUtil.fromJson(strpar, ParamList.class);
			...
			String act_protper = "";
			try{
				Map map =orderServiceLocal.queryMap(" select a.params from v_product_plan_params a where a.a_goods_id = ? and rownum < 2", o.getGoods_id());
				if(map != null){
				String strpar = map.get("params").toString();
				strpar = strpar.substring(1, strpar.lastIndexOf("]"));
				ParamList pl = JsonUtil.fromJson(strpar, ParamList.class);
    	...
    	jsonStr.append("\"source_from\":").append("\"").append(o.getSource_from()).append("\",");
    	jsonStr.append("\"order_city\":").append("\"").append(accept.getOrder_city_code()).append("\",");
    	jsonStr.append("\"channel_type\":").append("\"").append(accept.getOrder_channel()).append("\",");
    	...
    	jsonStr.append("\"reference_name\":").append("\"").append(accept.getRecommended_name()).append("\",");
        ...
        jsonStr.append("{\"order_amount\":").append("\"").append(accept.getOrder_realfee()).append("\",");
    	...
    	jsonStr.append("\"shipping_company_name\":").append("\"").append("").append("\",");
    	...
    	if("淘宝".equalsIgnoreCase(getValue(accept.getOrder_from()))){
    		jsonStr.append("<address>"+getValue(accept.getProvince())+" "+getValue(accept.getCity())+" "+getValue(accept.getDistrict())+" "+getValue(o.getShip_addr())+"</address>");
			
		}else{
			jsonStr.append("<address>"+getValue(o.getShip_addr())+"</address>");
...
//        jsonStr.append("{\"ship_addr\":").append("\"").append(o.getShip_addr()).append("\",");
    	...
    	jsonStr.append("\"ship_email\":").append("\"").append("").append("\",");
    	...
    	jsonStr.append("\"group_code\":").append("\"").append("").append("\",");
    	...
    	jsonStr.append("\"invoice_title\":").append("\"").append(invoice_title).append("\",");
    	jsonStr.append("\"invoice_content\":").append("\"").append(invoice_content).append("\",");
...
//    	    	jsonStr.append("\"invoice_content\":").append("\"").append(invoice_content).append("\",");
    	    	...
    	    	jsonStr.append("\"activity_list\":").append("\"").append(activity_list).append("\",");
    	    	//-------
	...
	public String orderStateSync(String orderId,String toSysName,String serial_no) {
		...
		String fromSysName = "ECS";
	    	...
	    	Date Current_date = new Date(System.currentTimeMillis());//获取系统当前时间
	    	String req_date = sdf_good.format(Current_date);
			Map m = this.baseDaoSupport.queryForMap("SELECT A.ORDER_ID, A.STATUS, B.SHIP_NAME, B.DELIVERY_ID, A.DEAL_MESSAGE FROM ES_ORDER A JOIN ES_DELIVERY B ON B.ORDER_ID = A.ORDER_ID WHERE A.ORDER_ID = '"+orderId+"'");
			result = result + "  {                                                                         " + 
					"    \"order_state_req\":                                                    " +
					"    {                                                                       " +
					"  \"serial_no\":\""+serial_no+"\",                                     " +
