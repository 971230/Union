 ...
 * @author Rapon
 */
public class RceiveICCIDServlet implements ICommonServlet {
	private final static Log log = LogFactory.getLog(RceiveICCIDServlet.class);
	private  static String interfaceName="【森锐ICCID单笔推送接口】";


	private String targetReqType;
	public RceiveICCIDServlet(String targetReqType) {
	...
	public String service(String reqString) throws Exception {
		...
		RceiveICCIDResp zteResponse = new RceiveICCIDResp();
		zteResponse.setResp_code("-1");//默认失败
		RceiveICCIDReq req = null;
		if(StringUtils.isNotBlank(reqString)) {
			Gson gson = new Gson();
			try {
				req = gson.fromJson(reqString, RceiveICCIDReq.class);
	 ...
	 * @return
	 */
	public RceiveICCIDResp dealOtherDealUser(RceiveICCIDReq req)throws Exception  {
		RceiveICCIDResp zteResponse = new RceiveICCIDResp();
		zteResponse.setResp_code("-1");//默认失败
		try {
			ZteClient client = ClientFactory.getZteDubboClient(ManagerUtils.getSourceFrom());
			zteResponse = client.execute(req, RceiveICCIDResp.class);//业务处理
		} catch (Exception e) {
