	...
	private void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (null == inJson || "".equals(inJson)) {
				outJson = "{\"respcode\":\"00000\",\"respdesp\":\"NO\",\"respmesg\":{\"ActiveNo\": \"\",\"RespCode\": \"9999\",\"RespDesc\": \"同步失败,接口报文为空.\",\"OrderId\": \"\"}}";
			}else {
				logger.info("总部商城请求报文:" + inJson);
...
//							把json参数set对Outer中
							...
							checkResult = setCenterMallOrderValue(centerMallOrder,out_order , extMap);
							if("".equals(checkResult)){							
								try {
									//同步块,确定不会重复重复插入
...
//									synchronized (this) {
										...
										if ("0".equals(resp.getError_code())) {
											outJson = "{\"respcode\":\"00000\",\"respdesp\":\"OK\",\"respmesg\":{\"ActiveNo\":\"" + centerMallOrder.getActiveNo() + "\",\"RespCode\":\"0000\",\"RespDesc\":\"订单[" + centerMallOrder.getOrderId() + "]同步成功\"}}";
										} else {
											...
											outJson = "{\"respcode\":\"00000\",\"respdesp\":\"NO\",\"respmesg\":{\"ActiveNo\":\"" + centerMallOrder.getActiveNo() + "\",\"RespCode\":\"9999\",\"RespDesc\":\"订单[" + centerMallOrder.getOrderId() + "]同步失败,原因:" + resp.getError_msg() + "\"}}";
										}
//									}
								} catch (Exception e) {
									errorFlag = true;
									logger.info(e.getMessage(), e);
									outJson = "{\"respcode\":\"00000\",\"respdesp\":\"NO\",\"respmesg\":{\"ActiveNo\":\"" + centerMallOrder.getActiveNo() + "\",\"RespCode\":\"9999\",\"RespDesc\":\"订单[" + centerMallOrder.getOrderId() + "]同步失败\"}}";
								}
								
							}else {
								errorFlag = true;
								outJson = "{\"respcode\":\"00000\",\"respdesp\":\"NO\",\"respmesg\":{\"ActiveNo\": \"" + centerMallOrder.getActiveNo() + "\",\"RespCode\": \"9999\",\"RespDesc\": \"订单[" + centerMallOrder.getOrderId() + "]同步失败,原因:"+checkResult+"\"}}";
							}
						}else {
							errorFlag = true;
							outJson = "{\"respcode\":\"00000\",\"respdesp\":\"NO\",\"respmesg\":{\"ActiveNo\": \"" + centerMallOrder.getActiveNo() + "\",\"RespCode\": \"9999\",\"RespDesc\": \"订单[" + centerMallOrder.getOrderId() + "]同步失败,原因:"+checkResult+"\"}}";
						...
						errorFlag = true;
						logger.info(e.getMessage(), e);
						outJson = "{\"respcode\":\"00000\",\"respdesp\":\"NO\",\"respmesg\":{\"ActiveNo\": \"\",\"RespCode\": \"9999\",\"RespDesc\": \"同步失败,json转换异常.\"}}";
					}
				} catch (Exception e) {
					errorFlag = true;
					logger.info(e.getMessage(), e);
					outJson = "{\"respcode\":\"00000\",\"respdesp\":\"NO\",\"respmesg\":{\"ActiveNo\": \"\",\"RespCode\": \"9999\",\"RespDesc\": \"同步失败,"+e.getMessage()+".\"}}";
				}
			}
		} catch (Exception e) {
			errorFlag = true;
			outJson = "{\"respcode\":\"00000\",\"respdesp\":\"NO\",\"respmesg\":{\"ActiveNo\": \"\",\"RespCode\": \"9999\",\"RespDesc\": \"同步失败,接口报文为空.\"}}";
			logger.info(e.getMessage(), e);
		}
		if (errorFlag) {
		...
		logger.info("CCCCCCCCCCCCCCCCCCCCCCCCCCCCC返回消息:" + outJson);
		out.print(outJson);
		out.close();
	}
	 ...
	 * @return
	 */
	private String setCenterMallOrderValue(CenterMallOrder centerMallOrder , Outer outer , Map extMap){
		String checkResult = "";
		/**
		 * 补充信息,总部没传但订单系统必传
		 */
		...
		String prov_name = goodServicesLocal.queryString("SELECT  a.province_name from es_zbmall_area a WHERE a.province_code = '" + centerMallOrder.getConsiGoodsProv() + "' AND ROWNUM < 2");
		outer.setProvince(prov_name);
		//ConsiGoodsCity  收货市
		outer.setCity_code(centerMallOrder.getConsiGoodsCity());
		String city_name = goodServicesLocal.queryString("SELECT  a.city_name from es_zbmall_area a WHERE a.city_code = '" + centerMallOrder.getConsiGoodsCity() + "' AND ROWNUM < 2");
		...
		if (MallUtils.isNotEmpty(centerMallOrder.getConsiGoodsDist())) {
			dist_name = goodServicesLocal.queryString("SELECT  a.district_name from es_zbmall_area a WHERE a.district_code = '" + centerMallOrder.getConsiGoodsDist() + "' AND ROWNUM < 2");
	 ...
	 * @return
	 */
	private String setCenterMallGoodsInfo(CenterMallOrder centerMallOrder , Outer outer , Map extMap){
		String checkResult = "";
				...
				/*ZX add 2015-12-25 集团ID start*/
						...
						if (EcsOrderConsts.ZB_GOODS_TYPE_ZHKL.equalsIgnoreCase(goodsType)
							...
							if (StringUtils.isNotBlank(userType)) {
								userType = CommonDataFactory.getInstance().getDictCodeValue("user_type", goodsAttInfo.getUserType());
...
//		型号编码
	 ...
	 * @return
	 */
	private String checkCenterMallOrder(CenterMallOrder centerMallOrder){
		...
		if (MallUtils.isEmpty(centerMallOrder.getConsiPhone())) {
			...
			if (centerMallOrder.getConsiPhone().length() != 11) {
...
//		}
	 ...
	 * @return
	 */
	private String checkGiftInfo(CenterMallGiftInfo giftInfo){
		//赠品面值
	 ...
	 * @return
	 */
	private String checkGoodsInfo(CenterMallGoodsInfo centerMallGoodsInfo){
		String strTmp = null;
		//商品编码
...
/*
	 ...
	 * @return
	 */
	private String checkFeeInfo(CenterMallFeeInfo feeInfo){
		//收费项编码
	 ...
	 * @return
	 */
	public String checkGoodsPhoneInfo(CenterMallGoodsPhoneInfo phoneInfo){
		String strTmp = null;
		//订购号码
...
//		}
	 ...
	 * @return
	 */
	private String checkGoodsAttInfo(CenterMallGoodsAttInfo goodsAttInfo , String goodsType){
		String strTmp = null;
		/**
		 * 商品类型goodsType说明
		 * ZHKL	总部号卡类
	 ...
	 * @return
	 */
	private String checkPackage(List<CenterMallPackage> packages){
		if (null != packages) {
	 ...
	 * @return
	 */
	private String checkCommon2(CenterMallGoodsAttInfo goodsAttInfo){
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		//号卡客户姓名
...
//		}
	 ...
	 * @return
	 */
	private String checkResourcesInfo(CenterMallResourcesInfo resourcesInfos){
		//终端品牌编码
	 ...
	 * @return
	 */
	private String checkCommonField(CenterMallGoodsAttInfo goodsAttInfo){
		//首月资费方式
	 ...
	 * @return
	 */
	private String checkSubProdInfo(List<CenterMallSubProdInfo> subProdInfos){
	 ...
	 * @return
	 */
	private String checkActivityInfo(List<CenterMallActivityInfo> activityInfos){
...
//				}
	 ...
	 * @return
	 */
	private String checkDiscountInfo(CenterMallDiscountInfo discountInfo){
		//优惠活动类型
	 ...
	 * @return
	 */
	private String checkPayInfo(CenterMallPayInfo payInfo){
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		//支付类型
