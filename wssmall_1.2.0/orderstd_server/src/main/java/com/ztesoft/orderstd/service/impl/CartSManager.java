...
import com.ztesoft.net.server.PromotionSServ;
import com.ztesoft.net.sqls.SF;
import com.ztesoft.orderstd.model.CartItemMapper;
...
public class CartSManager extends BaseSupport implements ICartSManager {
	private IDlyTypeManager dlyTypeSManager;

	private PromotionSServ promotionSServ;

	private CartPluginSBundle cartPluginSBundle;
	...
	public void add(Cart cart) {
		...
		if (count > 0) {
			...
			//先设置，再插入
			this.cartPluginSBundle.onAdd(cart);
	...
	public List<CartItem> listGoods(Member member,String sessionid,Coupons coupon) {
		...
		cartPluginSBundle.filterList(list, sessionid);
		...
		if (member != null && member.getLv_id() != null) {
			...
			this.promotionSServ.applyGoodsPmt(list, member.getLv_id(),coupon,stafs);
	...
	public void delete(String sessionid, Long cartid) {
		...
		this.cartPluginSBundle.onDelete(sessionid, cartid);
	}
		...
		if (member != null) {
			...
			for (Map map : list) {
				String goodsid = map.get("goods_id")
						.toString();
				List<Promotion> pmtList = promotionSServ.list(goodsid,
						memberLvId,null);
				for (Promotion pmt : pmtList) {

					// 查找相应插件
					String pluginBeanId = pmt.getPmts_id();
					IPromotionPlugin plugin = promotionSServ
	...
	public CartPluginSBundle getCartPluginSBundle() {
		return cartPluginSBundle;
	}

	public void setCartPluginSBundle(CartPluginSBundle cartPluginSBundle) {
		this.cartPluginSBundle = cartPluginSBundle;
	}
...
//		}
		...
		if(staff_no!=null && !"".equals(staff_no)){
			goodsItemList = cartSManager.listGoods(member,sessionid,staff_no,cp==null?null:cp.getCoupon());
		}else{
			goodsItemList = cartSManager.listGoods(member,sessionid,cp==null?null:cp.getCoupon());
		}
...
//			////////////////////add by wui团购秒杀加回
//			//计算捆绑商品的总价，并加入订单总价中
//			Double pgkTotal = this.countPgkTotal(sessionid);
//			//计算团购总价
//			Double groupBuyTotal = this.countGroupBuyTotal(sessionid);
//			
//			//计算秒杀总价
//			Double limitTotal = this.countLimitBuyTotal(sessionid);
//			
...
//		}
		...
		if(true || cp==null || !Const.ORDER_STAND_AUTO_SERVICE_CODE.equals(cp.getService_code())){
			if(regionid!=null &&shippingid!=null &&isProtected
					!=null  ){
				//计算原始配置送费用
				this.dlyTypeSManager.list();
				
				Object obj = this.dlyTypeSManager.countPrice(shippingid, weight, originalPrice, regionid, isProtected);
				...
				if(member!=null){ //计算会员优惠
					...
					String [] stafs = set.toArray(new String[0]);
					DiscountPrice discountPrice  = this.promotionSServ.applyOrderPmt(member,coupPrice, dlyPrice,point, member.getLv_id(),cp==null?null:cp.getCoupon(),stafs);
...
//					point = discountPrice.getPoint(); //优惠后的积分
		...
		orderPrice  = this.cartPluginSBundle.coutPrice(orderPrice);
		return orderPrice;
	}
...
//			this.applyMemPrice(list, memPriceList, discount);
			...
			this.promotionSServ.applyGoodsPmt(list, member.getLv_id(),coupon,staff_no);
		}
		return list;
	}
	...
	public PromotionSServ getpromotionSServ() {
		return promotionSServ;
	}

	public void setpromotionSServ(PromotionSServ promotionSServ) {
		this.promotionSServ = promotionSServ;
	}
