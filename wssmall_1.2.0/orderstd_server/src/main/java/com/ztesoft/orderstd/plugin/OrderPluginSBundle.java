...
import com.ztesoft.net.mall.core.model.AllocationItem;
 ...
 *
 */
public class OrderPluginSBundle extends AutoRegisterPluginsBundle {
	...
	public void onCreate(Order order,String sessionid){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[创建]事件错误", e);
			throw e;
		}
	}
	
	public void onFilter(String orderId,List<Map> itemList){
		try{
			if (plugins != null) {
				for (IPlugin plugin : plugins) {
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[filter]事件错误", e);
			throw e;
		}
	}	
	...
	public void onShip(Delivery delivery,List<DeliveryItem> itemList){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[ship]事件错误", e);
			throw e;
		}
	}	
	...
	public void onItemShip(DeliveryItem deliveryItem,AllocationItem allocationItem){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[onItemShip]事件错误", e);
			throw e;
		}
	}	
	...
	public void onReturned(Delivery delivery,List<DeliveryItem> itemList){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[returned]事件错误", e);
	...
	public void onDelete(Integer[] orderId){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[delete]事件错误", e);
			throw e;
		}
	}
	...
	public void onAllocationItem(AllocationItem allocationItem){
		try{
			if (plugins != null) {
				...
				AutoRegisterPluginsBundle.loger.info("调用订单插件[onAllocationItem]事件错误", e);
	...
	public void onAllocation(Allocation allocation){
		try{
			if (plugins != null) {
				...
				AutoRegisterPluginsBundle.loger.info("调用订单插件[onAllocation]事件错误", e);
...
/*	public String getAllocationHtml(OrderItem item){
		...
		try{
			...
			this.loger.info("调用订单插件[getAllocationHtml]事件错误", e);
...
/*	public String getAllocationViewHtml(OrderItem item){
		...
		try{
			...
			if(this.loger.isErrorEnabled())
			this.loger.info("调用订单插件[getAllocationViewHtml]事件错误", e);
			throw e;
		}
	 ...
	 * @throws SQLException 
	 */
	public void filterAlloItem(String catid,Map values,ResultSet rs) throws SQLException{
		try{
			if (plugins != null) {
				for (IPlugin plugin : plugins) {
					if (plugin instanceof IOrderAllocationItemEvent) {
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[filterAlloViewItem]事件错误", e);
			throw e;
		}
	...
	public void onPay(Order order ,boolean isOnline){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[pay]事件错误", e);
			throw e;
		}		
	}
	...
	public void onCanel(Order order){
		try{
			...
			if(AutoRegisterPluginsBundle.loger.isErrorEnabled())
			AutoRegisterPluginsBundle.loger.info("调用订单插件[canel]事件错误", e);
			throw e;
		}		
	}
	...
	public static void main(String[] args){
		OrderPluginSBundle  bundle = new OrderPluginSBundle();
