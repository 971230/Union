 ...
 *2010-4-1上午10:11:01
 */
public class DlyTypeSManager extends BaseSupport<DlyType> implements IDlyTypeManager{
	
	private RegionsInf regionsServ;
	// 本地缓存
	public static HashMap<String,DlyType> localCache = new HashMap<String,DlyType>();
	...
	public DlyType getDlyTypeById(String typeId) {
		String preKey = typeId+"SERVICE_DLY_TYPE_SELECT";
		DlyType dlyType = null;
		if(null != localCache.get(preKey)) {
			return localCache.get(preKey);
		} else {
			String sql  = SF.orderSql("SERVICE_DLY_TYPE_SELECT")+" and type_id=?";
			dlyType = this.baseDaoSupport.queryForObject(sql, DlyType.class, typeId);
			//配置了地区费用
