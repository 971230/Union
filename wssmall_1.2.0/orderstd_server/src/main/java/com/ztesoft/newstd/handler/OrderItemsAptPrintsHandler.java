...
public class OrderItemsAptPrintsHandler extends BaseSupport implements IAttrHandler {
	@Override
	...
	public AttrInstLoadResp handler(AttrSwitchParams params) {
		...
		/*
		 * mainContentOne 1、 套餐名称：《套餐名称》，详见业务协议 2、 《按合约数据文档中的产品描述。》
		 */
		String sms_send_num = "";
		...
		if (StringUtils.isEmpty(goods_id))
			return null;
		try {
			List<Map> list = this.baseDaoSupport.queryForList(
					"select a.params from v_product_params a where a.a_goods_id = ? and rownum < 2", goods_id);
			if (list != null && list.size() > 0) {
				...
				for (int s = 0; s < aa1.size(); s++) {
					...
					if (attrcode != null && !"".equalsIgnoreCase(attrcode)) {
						String dc_sql = querySqlResult(
								"select DC_SQL from es_dc_sql  a where  a.dc_name = '" + attrcode + "' and rownum < 2");
						String dc_sql_value = querySqlResult(
								"select value_desc from (select 'ECS' source_from, e.* from (" + dc_sql
										+ ") e)T where T.VALUE ='" + value + "' and rownum < 2");
						value = dc_sql_value;
					}
		...
		if ("".equalsIgnoreCase(sms_send_num) && "".equalsIgnoreCase(answering_free) && "".equalsIgnoreCase(call_times)
				&& "".equalsIgnoreCase(flow) && "".equalsIgnoreCase(visual_phone) && "".equalsIgnoreCase(other_business)
				&& "".equalsIgnoreCase(out_flow)) {
			mainContentOne_2 = sms_send_num + answering_free + call_times + flow + visual_phone + other_business
					+ out_flow;
		} else {
			mainContentOne_2 = "2、" + sms_send_num + answering_free + call_times + flow + visual_phone + other_business
					+ out_flow;
		}
		jsonStr.append("{\"mainContentOne\":").append("\"").append(mainContentOne_1).append(mainContentOne_2)
				.append("\",");
		 ...
		 * 可选业务包名称及描述：《可选活动名称》<br/> 号码信息：您选择的号码18502510173，具体规则详见业务协议 。<br/>
		...
		jsonStr.append("\"mainContentTwo\":").append("\"").append("基本通信服务及附加业务名称及描述：").append(ywmcms).append("\\r")
				.append("可选业务包名称及描述：").append("可选活动名").append("\\r").append("号码信息：您选择的号码")
				.append(getValue(orderAttrValues.get(AttrConsts.PHONE_NUM))).append("，具体规则详见业务协议").append("\",");
		...
		jsonStr.append("\"acctAddr\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.SHIP_ADDR)))
				.append("\",");// cust_addr
		...
		jsonStr.append("\"paperExpr\":").append("\"")
				.append(getValue(orderAttrValues.get(AttrConsts.CERT_FAILURE_TIME))).append("\",");// 证件有效期 传空值
		jsonStr.append("\"custName\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.PHONE_OWNER_NAME)))
				.append("\",");// 用户姓名 cust_name
		jsonStr.append("\"acctName\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.PHONE_OWNER_NAME)))
				.append("\",");// 用户姓名 cust_name
		jsonStr.append("\"agentPhone\":").append("\"").append("").append("\",");// 代理人电话号码，来源不清楚 空
		jsonStr.append("\"custType\":").append("\"").append(orderAttrValues.get(AttrConsts.CUSTOMER_TYPE))
				.append("\",");// 客户类型 字典值有（个人或集团） 要商城传，接口要改

		// 付费方式 payment_type 跟字典值做对应关系，字典值不清楚
		String payment_type = orderAttrValues.get(AttrConsts.PAY_METHOD).toString();
		String payment_name = querySqlResult(
				"select c.field_desc from es_mall_config c where c.field_name = 'payment_type' and c.field_value = '"
						+ payment_type + "'");
		if (StringUtils.isEmpty(payment_name)) {
			payment_name = payment_type;
		}
		jsonStr.append("\"payMethod\":").append("\"").append(payment_name).append("\",");
		jsonStr.append("\"emailAddr\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.SHIP_EMAIL)))
				.append("\",");
		jsonStr.append("\"contactMan\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.SHIP_NAME)))
				.append("\",");
		jsonStr.append("\"contactAddr\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.SHIP_ADDR)))
				.append("\",");
		jsonStr.append("\"paperAddr\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.CERT_ADDRESS)))
				.append("\","); // 证件地址 certi_addr
		jsonStr.append("\"paperType\":").append("\"").append(CommonDataFactory.getInstance()
				.getDcPublicDataByPkey(StypeConsts.CERTI_TYPE, getValue(orderAttrValues.get(AttrConsts.CERTI_TYPE))))
				.append("\","); // 证件类型 certi_type
		jsonStr.append("\"staffInfo\":").append("\"").append("").append("\","); // 受理人及工号 传空值
		jsonStr.append("\"contactPhone\":").append("\"")
				.append(getValue(orderAttrValues.get(AttrConsts.REVEIVER_MOBILE))).append("\",");// 客户联系电话 ship_phone
		jsonStr.append("\"bankAcctName\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.BANK_ACCOUNT)))
				.append("\",");// 银行帐号 bank_cust_code
		jsonStr.append("\"bankName\":").append("\"").append("").append("\",");// 银行名称 bank_name
		jsonStr.append("\"bankAcct\":").append("\"").append("").append("\",");// 该字段没有用到，来源不清楚 空
		jsonStr.append("\"paperNo\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.CERT_CARD_NUM)))
				.append("\",");// 证件号码 certi_num
		jsonStr.append("\"bankCode\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.BANK_CODE)))
				.append("\",");// 银行编码 bank_code
		...
		jsonStr.append("\"userNo\":").append("\"").append(getValue(orderAttrValues.get(AttrConsts.PHONE_NUM)))
				.append("\",");
		jsonStr.append("\"AcceptanceMode\":").append("\"").append("0").append("\",");// 默认“打套”，送“0”

		/*
		 * "RECEIPT_CODE  " 打印规则 presub_t 号码是预付费的合约机和号卡 postsub_t 号码是后付费的合约机和号卡
		 * wlpost_p_t 非集团用户的后付费无线上网卡 wlpost_g_t 集团用户的后付费无线上网卡 precard_t 预付费无线上网卡
		 * 
		 * 合约机：号码+合约计划+手机+套餐 号卡：号码+合约计划（可选）+套餐 上网卡：套餐+上网卡硬件（可选） 裸机：手机
		 */
		...
		// OrderItemsAptPrintsBusiRequest orderItemAptPrt = new
		// OrderItemsAptPrintsBusiRequest();
		List<OrderItemBusiRequest> orderItemBusiRequests = CommonData.getData().getOrderTreeBusiRequest()
				.getOrderItemBusiRequests();
		for (OrderItemBusiRequest orderItemBusiRequestsInfo : orderItemBusiRequests) {
			List<OrderItemsAptPrintsBusiRequest> OrderItemsAptPrintsBusiRequests = orderItemBusiRequestsInfo
					.getOrderItemsAptPrintsRequests();

			OrderItemsAptPrintsBusiRequest orderItemsAptPrintsBusiRequest = new OrderItemsAptPrintsBusiRequest();
			orderItemsAptPrintsBusiRequest.setOrder_id(order_id);
			orderItemsAptPrintsBusiRequest
					.setAcceptance_id(this.baseDaoSupport.getSequences("SEQ_ES_ORDER_ITEMS_APT_PRINTS")); // 手动设置主键值
			orderItemsAptPrintsBusiRequest.setItem_id(orderItemBusiRequestsInfo.getItem_id());
			orderItemsAptPrintsBusiRequest.setStatus(EcsOrderConsts.IS_PRINT_0);
			List<String> errors = parseStr(jsonStr.toString());
			for (int i = 0; i < errors.size(); i++) {
				if (i == 0)
					orderItemsAptPrintsBusiRequest.setAcceptance_html(errors.get(i));
				else if (i == 1)
					orderItemsAptPrintsBusiRequest.setAcceptance_html_2(errors.get(i));
				else if (i == 2)
					orderItemsAptPrintsBusiRequest.setAcceptance_html_3(errors.get(i));
			}
			orderItemsAptPrintsBusiRequest.setReceipt_mode("0");
			orderItemsAptPrintsBusiRequest.setReceipt_code(receipt_code);
			orderItemsAptPrintsBusiRequest.setCreate_time(Consts.SYSDATE);
			...
			OrderItemsAptPrintsBusiRequests.add(orderItemsAptPrintsBusiRequest);
