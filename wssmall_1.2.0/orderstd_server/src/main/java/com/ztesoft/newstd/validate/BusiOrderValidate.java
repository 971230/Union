...
public class BusiOrderValidate {

	private static Logger logger = Logger.getLogger(BusiOrderValidate.class);
...
//		source_system = mobileBusiOrder.getSource_system();
	...
	private static String setMobileBusiOrderValue(MobileBusiOrder mobileBusiMallOrder, Outer outer, Map extMap, DevelopOrderResp developOrderResp) {
		 ...
		 */
		outer.setType("");// 码上购没传
		outer.setOrder_deal_method(mobileBusiMallOrder.getOrder_deal_method());
		...
		if (null != developerInfo) {
			if (null != developOrderResp) {
				...
				if(!StringUtils.isEmpty(developerInfo.getDevelop_name())){
				    ...
				    extMap.put("develop_name_new", developerInfo.getDevelop_name());// 之前收单问题新增沉淀字段，方便以后移解
				}
			}
			...
			/*if (StringUtils.isEmpty(developerInfo.getDevelop_name())) {// 发展人姓名--bss不传
				// outer.setDevelopment_name(developerInfo.getDevelop_name());
				// develop_point_code->develop_code;develop_code->develop_name
				if (!StringUtils.isEmpty(developerInfo.getDevelop_point_code())) {// 发展点编码
					outer.setDevelopment_code(developerInfo.getDevelop_point_code());
				}
				if (!StringUtils.isEmpty(developerInfo.getDevelop_code())) {// 发展人编码
					outer.setDevelopment_name(developerInfo.getDevelop_code());
				}
			} else {
				extMap.put("develop_name_new", developerInfo.getDevelop_name());// 之前收单问题新增沉淀字段，方便以后移解
			}*/
		...
		if (null != seedUserInfo) {
			...
			if (!StringUtil.isEmpty(seedUserInfo.getTop_seed_professional_line())) {
				extMap.put("top_seed_professional_line", seedUserInfo.getTop_seed_professional_line());
			}
			if (!StringUtil.isEmpty(seedUserInfo.getTop_seed_type())) {
				extMap.put("top_seed_type", seedUserInfo.getTop_seed_type());
			}
			if (!StringUtil.isEmpty(seedUserInfo.getTop_seed_group_id())) {
				extMap.put("top_seed_group_id", seedUserInfo.getTop_seed_group_id());
			}
		}
		
		//获取extMap节点
		if (mobileBusiMallOrder.getExtMap() instanceof Map&&null!=mobileBusiMallOrder.getExtMap()) {
			extMap.put("is_no_modify", mobileBusiMallOrder.getExtMap().get("is_no_modify"));
		}
...
//		CommonData.getData().getOrderTreeBusiRequest().getOrderBusiRequest().setSn(mobileBusiMallOrder.getSerial_no());
		...
		outer.setProvinc_code(mobileBusiMallOrder.getOrder_prov_code());
		outer.setCity_code(mobileBusiMallOrder.getOrder_city_code());
		outer.setArea_code(mobileBusiMallOrder.getOrder_county_code());
		outer.setPost_fee("0");
		MobileBusiOrderContactInfo contact_info = mobileBusiMallOrder.getContact_info();
		if(!StringUtil.isEmpty(contact_info.getShip_province())){
			outer.setProvinc_code(contact_info.getShip_province());
		}
		if(!StringUtil.isEmpty(contact_info.getShip_city())){
			outer.setCity_code(contact_info.getShip_city());
		}
		if(!StringUtil.isEmpty(contact_info.getShip_district())){
			outer.setArea_code(contact_info.getShip_district());
		}
...
//						+ " f.source_from = '" + ManagerUtils.getSourceFrom() + "' and f.source_from = d.source_from "
...
//				promotionList = support.queryForList(sql, resp.getData().get("goods_id").toString(), mobileBusiOrder.getOrder_city_code(), user_type);
                ...
                if (null != promotionList && promotionList.size() > 0) {
                    for (int i = 0; i < promotionList.size(); i++) {
                        ...
                        String create_time = mobileBusiOrder.getCreate_time();
                        ...
                        if (create_time_stamp > begin_time_stamp && create_time_stamp < end_time_stamp) {
                        	//活动在参与省份城市
                        	if(String.valueOf(m.get("region")).indexOf(province_region)>0||String.valueOf(m.get("region")).indexOf(mobileBusiOrder.getOrder_city_code())>0) {
...
//		    String SOURCE_SYSTEM_CACHE_KEY = "SOURCE_SYSTEM_CACHE_KEY:" + mobileBusiOrder.getSource_system();
//            Boolean source_system_check = (Boolean) cache.get(space, SOURCE_SYSTEM_CACHE_KEY);         
//            if (source_system_check == null) {
//                source_system_check = dbUtils.checkFieldValue("source_system", mobileBusiOrder.getSource_system());
//                cache.set(space, SOURCE_SYSTEM_CACHE_KEY, source_system_check,EXPIRE_TIME);
//            }
			//改造dbUtils方法实现缓存 ding.xiaotao
            Boolean source_system_check = dbUtils.checkFieldValue("source_system", mobileBusiOrder.getSource_system());
...
//		    String SOURCE_SYSTEM_TYPE_CACHE_KEY = "SOURCE_SYSTEM_CACHE_KEY:" + mobileBusiOrder.getSource_system_type();
//            Boolean source_system_type_check = (Boolean) cache.get(space, SOURCE_SYSTEM_TYPE_CACHE_KEY);
//            if (source_system_type_check == null) {
//                source_system_type_check = dbUtils.checkFieldValue("source_from_system", mobileBusiOrder.getSource_system_type());
//                cache.set(space, SOURCE_SYSTEM_TYPE_CACHE_KEY, source_system_type_check,EXPIRE_TIME);
//            }
			//改造dbUtils方法实现缓存 ding.xiaotao
            Boolean source_system_type_check = dbUtils.checkFieldValue("source_from_system", mobileBusiOrder.getSource_system_type());
