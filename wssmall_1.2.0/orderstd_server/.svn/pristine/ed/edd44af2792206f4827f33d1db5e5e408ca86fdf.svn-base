...
public class OutterSTmplManager extends BaseSupport implements IOutterTmplManager {
	// 本地缓存
	public static HashMap localCache = new HashMap();
	/*@Override
	...
	public PublicConst queryPublicConst(String order_from, String object_type,
			String outer_code) {
		String preKey = order_from + "_" + object_type + "_" + outer_code;
		List<PublicConst> list = null;
		String sql = SF.orderSql("QUERY_PUBLIC_CONST_BY_SYS");
		if(localCache.containsKey(preKey+"QUERY_PUBLIC_CONST_BY_SYS")) {
		    if(null != localCache.get(preKey+"QUERY_PUBLIC_CONST_BY_SYS")) {
			return (PublicConst)localCache.get(preKey+"QUERY_PUBLIC_CONST_BY_SYS");
		    }else {
		        return null;
		    }
		} else {
			list = this.baseDaoSupport.queryForList(sql, PublicConst.class, order_from,outer_code,object_type,ManagerUtils.getSourceFrom());
			if(list!=null) {
			    localCache.put(preKey+"QUERY_PUBLIC_CONST_BY_SYS", (list.size()==0)?null:list.get(0));
			}
		}
