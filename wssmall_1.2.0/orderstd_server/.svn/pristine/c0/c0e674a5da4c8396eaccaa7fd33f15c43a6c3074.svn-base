...
public class AttrGiftInfoHandler extends BaseSupport implements IAttrHandler {
	@Override
	...
	public AttrInstLoadResp handler(AttrSwitchParams params) {
		...
		List<AttrGiftInfoBusiRequest> attrGiftInfoBusiRequests = (List<AttrGiftInfoBusiRequest>) params
				.getBusiRequest();
		// 在orderAttrValues获取
		...
		String service_remarks = (String) orderAttrValues.get(AttrConsts.SERVICE_REMARKS);
		...
		if (isZbOrder) {// 总部赠品
			if (!StringUtils.isEmpty(giftInfos) && !"-1".equals(giftInfos)) {
				saveZBGiftInfo(order_id, inst_id, giftInfos, attrGiftInfoBusiRequests, source_from);
			}
			if (StringUtils.isNotEmpty(reserve9)) {
				// 公共方法,ES_ATTR_GIFT_INFO
				saveECSGift(order_id, inst_id, reserve9, attrGiftInfoBusiRequests, source_from);
			}
		} else if (isTbOrder/* EcsOrderConsts.ORDER_FROM_10001.equals(order_from) */) {// 淘宝赠品

			saveTbGiftInfo(order_id, inst_id, service_remarks, attrGiftInfoBusiRequests, source_from);
			// 公共方法ES_ATTR_GIFT_INFO
			saveECSGift(order_id, inst_id, reserve9, attrGiftInfoBusiRequests, source_from);
		} else if (EcsOrderConsts.ORDER_FROM_10002.equals(order_from)) {
			saveLocalMallGiftInfo(order_id, inst_id, service_remarks, attrGiftInfoBusiRequests, source_from);
		} else {// 商城赠品
			saveGiftInfo(order_id, inst_id, activity_list, attrGiftInfoBusiRequests, source_from);
		}
		return null;
	}

	public void saveTbGiftInfo(String order_id, String inst_id, String service_remarks,
			List<AttrGiftInfoBusiRequest> attrGiftInfoBusiRequests, String source_from) {
		...
		for (String remark : remarks) {
			if (!StringUtils.isEmpty(remark)) {
				...
				if (remarkInfo == null && remarkInfo.length != 2)
					continue;
				...
				if (EcsOrderConsts.TB_GIFT_REMARK_PREFIX.equals(name) && StringUtils.isNotEmpty(giftStr)) {
					...
					for (int i = 0; i < giftInfo.length; i++) {
						...
						String is_yewubao = CommonDataFactory.getInstance().getDictCodeValue(StypeConsts.ISYEWUBAO,
								cat_id);
						...
						String isvirtualgift = CommonDataFactory.getInstance()
								.getDictCodeValue(StypeConsts.IS_VIRTUAL_GIFT, lpMap.get("cat_name").toString());
						...
						String giftType = EcsOrderConsts.PRODUCT_CAT_ID_1.equals(cat_id)
								? EcsOrderConsts.CARD_SPECIES_01
								: EcsOrderConsts.PRODUCT_CAT_ID_5.equals(cat_id) ? EcsOrderConsts.CARD_SPECIES_03
										: EcsOrderConsts.CARD_SPECIES_02;
						...
						giftMap.put("source_from", source_from);
						...
						List<AttrGiftParamBusiRequest> attrGiftParamBusiRequests = attrGiftInfoBusiRequest
								.getGiftParamBusiRequests();
						...
						try {
							...
							attrGiftInfoBusiRequests.add(attrGiftInfoBusiRequest);
						} catch (Exception e) {
							e.printStackTrace();
						}
	...
	public void saveLocalMallGiftInfo(String order_id, String inst_id, String service_remarks,
		...
		String isvirtualgift = CommonDataFactory.getInstance().getDictCodeValue(StypeConsts.IS_VIRTUAL_GIFT,
				lpMap.get("cat_name").toString());
		if (EcsOrderConsts.IS_DEFAULT_VALUE.equals(isvirtualgift)) {
			giftMap.put("is_virtual", "1");
		} else {
			giftMap.put("is_virtual", "0");
		...
		String giftType = EcsOrderConsts.PRODUCT_CAT_ID_1.equals(cat_id) ? EcsOrderConsts.CARD_SPECIES_01
				: EcsOrderConsts.PRODUCT_CAT_ID_5.equals(cat_id) ? EcsOrderConsts.CARD_SPECIES_03
						: EcsOrderConsts.CARD_SPECIES_02;
		giftMap.put("is_gift", "1");
			...
			if (!StringUtils.isEmpty(giftInfos) && !"[]".equals(giftInfos)) {
				try {
					...
					for (int i = 0; i < jsonArray.size(); i++) {
						...
						String giftType = jsonObject.get("giftType") == null ? ""
								: jsonObject.get("giftType").toString();
						String giftValue = jsonObject.get("giftValue") == null ? ""
								: jsonObject.get("giftValue").toString();
						String giftName = jsonObject.get("giftName") == null ? ""
								: jsonObject.get("giftName").toString();
						String giftDesc = jsonObject.get("giftDesc") == null ? ""
								: jsonObject.get("giftDesc").toString();
						String giftNum = jsonObject.get("giftNum") == null ? "" : jsonObject.get("giftNum").toString();
						String giftBrand = jsonObject.get("giftBrand") == null ? ""
								: jsonObject.get("giftBrand").toString();
						String params = null;
						...
						if (null != giftName && !"".equals(giftName)) {
							...
							if (null != lpMap && lpMap.size() > 0) {
								...
								if (StringUtils.isEmpty(giftBrand)) {
									giftBrand = lpMap.get("brand_name") == null ? ""
											: lpMap.get("brand_name").toString();
								}
								params = lpMap.get("params").toString();
							}
						}
						giftType = EcsOrderConsts.PRODUCT_CAT_ID_1.equals(goodsCategory)
								? EcsOrderConsts.CARD_SPECIES_01
								: EcsOrderConsts.PRODUCT_CAT_ID_5.equals(goodsCategory) ? EcsOrderConsts.CARD_SPECIES_03
										: EcsOrderConsts.CARD_SPECIES_02;
						Map giftMap = new HashMap();
						String gift_id = this.baseDaoSupport.getSequences("seq_attr_gift_info");
						giftMap.put("gift_inst_id", gift_id);
						...
						giftMap.put("goods_desc", giftName);
						String isvirtualgift = CommonDataFactory.getInstance()
								.getDictCodeValue(StypeConsts.IS_VIRTUAL_GIFT, goodsCategory);
						if (EcsOrderConsts.IS_DEFAULT_VALUE.equals(isvirtualgift)) {
							giftMap.put("is_virtual", "1");
						} else {
							giftMap.put("is_virtual", "0");
						...
						paramMap.put("source_from", source_from);
						...
						attrGiftParamBusiRequests.add(attrGiftParamBusiRequest);
	...
	private void saveGiftInfo(String order_id, String inst_id, String activity_list,
		...
		if (StringUtils.isEmpty(activity_list) || "-1".equals(activity_list) || "[]".equals(activity_list))
			return;
		try {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				for (int j = 0; j < giftArray.size(); j++) {
					...
					String is_process = giftObject.get("is_process") == null ? ""
							: giftObject.get("is_process").toString();
					String process_type = giftObject.get("process_type") == null ? ""
							: giftObject.get("process_type").toString();
					...
					List<AttrGiftParamBusiRequest> attrGiftParamBusiRequests = giftInfo.getGiftParamBusiRequests();
					Map paramMap = null;
					...
					if (null != goods) {
						...
						if (EcsOrderConsts.PRODUCT_TYPE_SP_PRODUCT.equals(type_id)) {// SP产品
							saveSPGift(order_id, goods, source_from);
							continue;
						} else if (EcsOrderConsts.PRODUCT_TYPE_SUB_PRODUCT.equals(type_id)) {// 附加产品
							saveSubProductGift(order_id, goods, subProducutList, source_from);
							continue;
						...
						String isvirtualgift = CommonDataFactory.getInstance()
								.getDictCodeValue(StypeConsts.IS_VIRTUAL_GIFT, cat_id);
						if (EcsOrderConsts.IS_DEFAULT_VALUE.equals(isvirtualgift)) {
							giftMap.put("is_virtual", "1");
						} else {
							giftMap.put("is_virtual", "0");
						}
						String gift_type = CommonDataFactory.getInstance().getDictCodeValue(StypeConsts.GIFT_TYPE,
								cat_id);
						...
						if (null != paramAr && isGift) {
							for (int k = 0; k < paramAr.length; k++) {
								...
								for (GoodsParam param : params) {
									...
									paramMap = new HashMap();
									AttrGiftParamBusiRequest attrGiftParamBusiRequest = new AttrGiftParamBusiRequest();
									paramMap.put("param_inst_id",
											this.baseDaoSupport.getSequences("seq_attr_gift_param"));
									paramMap.put("gift_inst_id", id);
						...
						attrGiftInfoBusiRequests.add(giftInfo);
						// giftInfo.store();
					}
				}
			}
	...
	private void saveECSGift(String order_id, String inst_id, String pmt_codes,
			List<AttrGiftInfoBusiRequest> attrGiftInfoBusiRequests, String source_from) {
		if (null == order_id || "".equalsIgnoreCase(order_id) || null == pmt_codes || "".equalsIgnoreCase(pmt_codes))
			return;
	 ...
	 * @param goods
	 */
	public void saveSPGift(String order_id, Goods goods, String source_from) {
		try {
			...
			spProduct.setSource_from(source_from);
			spProduct.setIs_dirty(ConstsCore.IS_DIRTY_YES);
			spProduct.setDb_action(ConstsCore.DB_ACTION_INSERT);
			// spProduct.store();
			CommonData.getData().getOrderTreeBusiRequest().getSpProductBusiRequest().add(spProduct);
		} catch (Exception e) {
			e.printStackTrace();
			logger.info(e.getMessage(), e);
		}
	...
	public void saveSubProductGift(String order_id, Goods goods, Map<String, String> subProducutList,
		...
		try {
			...
			if (!subProducutList.containsKey(cbss_product_code)) {
				...
				orderSubProductBusiRequest.setSource_from(source_from);
				...
				CommonData.getData().getOrderTreeBusiRequest().getOrderSubProductBusiRequest()
						.add(orderSubProductBusiRequest);
			...
			attrPackageSubProdBusiRequest.setSource_from(source_from);
			...
			CommonData.getData().getOrderTreeBusiRequest().getAttrPackageSubProdBusiRequest()
					.add(attrPackageSubProdBusiRequest);
		} catch (Exception e) {
			e.printStackTrace();
			logger.info(e.getMessage(), e);
		}
	...
	public void saveProductPackageGift(String order_id, Goods goods, String inst_id) {
		...
		try {
			...
			CommonData.getData().getOrderTreeBusiRequest().getPackageBusiRequests().add(attrPackageBusiRequest);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
