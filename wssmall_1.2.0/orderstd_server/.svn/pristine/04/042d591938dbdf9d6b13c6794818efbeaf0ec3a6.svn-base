...
public class SDBUtils extends BaseSupport {
	
	private Logger logger = Logger.getLogger(SDBUtils.class);
	private String sql = "";
...
//		String cacheed = cache.get(CacheUtils.MALL_DATA_CONFIG+"YES");
		Object obj=cache.get(CacheUtils.MALL_DATA_CONFIG+"YES");
		String cacheed = obj==null?"":obj.toString();
		if(!"yes".equals(cacheed)){
			refreshPkConfig(cache);
		}
		
		obj= cache.get(CacheUtils.MALL_DATA_CONFIG+fieldName+":"+fieldValue);
...
//		Map value = cacheMallConfig.get(CacheUtils.MALL_DATA_CONFIG+fieldName+":"+fieldValue);
		...
		if(!StringUtil.isEmpty(value) && fieldValue.equals(value))
			...
			try {
				...
				if (null != l && l.size() > 0) {
					//dingxiaotao 增加此处为field_name不匹配增加缓存设置与数据库对齐
					cache.set(CacheUtils.MALL_DATA_CONFIG+fieldName+":"+fieldValue,fieldValue);
					flag = true;
				}
			} catch (Exception e) {
				flag = false;
...
//		return flag;
	...
	public void refreshPkConfig(INetCache cache){
...
//				cacheMallConfig.put(field_name+":"+field_value, m);
				cache.set(CacheUtils.MALL_DATA_CONFIG+field_name+":"+field_value,field_value);
				
			}
			cache.set(CacheUtils.MALL_DATA_CONFIG+"YES","yes");
		}
	}
	 ...
	 * @return boolean 
	 */
	public boolean checkFieldValueExists(String fieldName, String provinc, String city, String county) {
		boolean flag = false;
		try {
			// 省
			if ("order_provinc_code".equals(fieldName)) {
				// 省查询
				String provincSql = "select z.code,z.name from es_regions_zb z where z.add_level='1' and z.code='" + provinc+ "'";
				List listMap = this.baseDaoSupport.queryForList(provincSql, null);

				if (listMap != null && listMap.size() > 0) {
					flag = true;
				}

			} else if ("ship_city".equals(fieldName)) {
				// 地市查询
				String citySql = "select z.code,z.name from es_regions_zb z where z.add_level='2' and z.pre_code='" + provinc+ "' and z.code='" + city + "'";
				List listMap = this.baseDaoSupport.queryForList(citySql, null);

				if (listMap != null && listMap.size() > 0) {
					flag = true;
				}

			} else if ("ship_country".equals(fieldName)) {
				// 县分查询
				String countySql = "select z.code,z.name from es_regions_zb z where z.add_level='3' and z.pre_code='" + city+ "' and z.code='" + county + "'";
				List listMap = this.baseDaoSupport.queryForList(countySql, null);
